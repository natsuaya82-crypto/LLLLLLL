...
その「通報」の中に**「管理」**があります。人数・投稿数・言語数・通報数と、通報の一覧が
一枚に出ます。

**「管理」を開くと、もう一度パスワードを訊かれます。** これはアプリの中で合言葉を
照らし合わせているのではなく、**そのパスワードを Supabase に送って、Supabase に
答えさせています**（設定の「パスワードを変える」が古いパスワードを確かめるのと同じ道です）。
アプリの中には合言葉がどこにも書かれていないし、どこにも保存されません ──
`www/` のファイルは全部、アプリを落とした人が読めるからです。

これが守っているのは**画面だけ**です。データを守っているのは `is_staff()` で、
これは上で立てた `profile.staff` を、**サーバが**毎回訊いています。パスワードを
知らない人が管理画面を無理やり開けたとしても、数も通報も何ひとつ返ってきません。
このパスワードは「サインインしたままの電話を人に渡したとき」のためのものです。

アプリを閉じると、また訊かれます。**Apple や Google でサインインしている
アカウントには訊きません** ── 入れ直すパスワードが存在しないからで、その場合は
`is_staff()` だけが門になります。
