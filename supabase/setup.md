...
**どこで見るかは変わっていません。アプリの中です。**
2026-08-26 の決定 ──「売り上げもアナリティクスも見れるようにしたい」「アプリの
中で見たい」「画面を開いたときに毎回」── はそのまま生きています。今日変わったの
は**数字の出どころ**だけで、App Store Connect に直接聞くのをやめた、という一点
です。「RevenueCat の画面で見る」とは誰も言っていません。

**コードはまだ残っています。**`supabase/functions/appstore/` と、それを呼ぶ
`www/net.js` の売上まわり、`www/mod.js` の管理画面の数字。鍵が無いので数字は
空欄のままです。**管理画面の売上の欄は残します**（アプリの中で見る、が決定
だから）。空欄を埋める線をどう引くかは決まっていません ── `docs/BACKLOG.md`。
