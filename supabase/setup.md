...
した。**そのとおりに起きました** ── 登録のコードは届き、パスワード再設定だけが
リンクで届き続けました（OWNER 2026-08-26「6桁の数字がそもそも届かない。リンクで
くるのをやめて欲しい」）。**押す一覧に無い手順は、押されません。**

**そして 2026-09-03 に、同じことが 7 で起きました。**扉が
`/auth/v1/signup` から `/auth/v1/otp` に変わった日（「1アドレス1アカウント」）、
使われるひな形も Confirm signup から Magic Link に変わりました。二つ直して
あって、三つ目が既定のままリンクを送っていました ──「メアド登録しようとしたら
サインインリンク来たけど6桁のコードじゃないの？」OWNER 2026-09-03。**この乗り換え
が、桁数が 8 になった理由でもあります ── 下の「桁数は 8」。**
...
**8 桁は事故で始まりました。** 2026-09-03 に扉が `/auth/v1/signup` から
`/auth/v1/otp` に変わった日（「1アドレス1アカウント」）、使われるひな形が
Confirm signup から Magic Link に変わり、Magic Link 側の既定が 8 桁だったので、
その日から 8 桁が届くようになりました。オーナーが実機で見つけたのがそれです。
**そして 8 で行くと決まりました**（同日）。この一覧はそれまで 6 桁と書いていて、
その一文は消しました。

**有効期限はまだ決まっていません。**「知らん」ですので、ここには書きません。
Supabase 側の既定のままです。**勝手に決めないでください。**

**まだ入っていないもの**: 再送信の 60 秒。`obMailAgain()`（`www/onboard.js`）は
何度でも押せます。あの画面は別のセッションの持ち物なので、ここには印だけ。
...
`www/net.js` の `netStore()`、`www/mod.js` の五ページと六行。管理画面に残って
いるのは**通報とスタッフ**だけです ── 数字は一つもありません。

手順は下に残してありますが、**やることではありません**。
...
## 11. バグで言語の中身が消えたとき ── 調べて、戻す

**「言語の単語が全部消えた」と言われたときに、ここを見ます。**
...
言語の中身（単語・文字・キーボードなど 12 種類）は、サーバーでは
`slice` という表に **1 種類につき 1 行** で入っています。アプリは保存のたびに
この行を **上書き** します。つまり、**前の中身はどこにも残っていませんでした。**
バグが一度でも変な中身を書けば、その瞬間に唯一の版が消えていました。

**上書きされる直前の中身を、`slice_past` という別の表に残すようにしました。**
アプリは一行も変わっていません。サーバー側だけの話です。

**戻せるのは、上書きで消えたものだけです。**

| | 戻せるか |
|---|---|
| バグで中身が空になった・変な文字列になった | **戻せます** |
| その人が自分で言語を消した | 戻せません（消えたら消えたままです） |
| その人がアカウントを削除した | 戻せません。**本当に全部消えます** |
| 投稿・下書き・写真・声 | 戻せません（この節の範囲外です） |
| 設定・テーマ・書記体系（`wsys`） | 戻せません（サーバーに写しがありません） |
| 一度もサーバーに上がっていない言語 | 戻せません（サーバーに無いものは残りようがありません） |

### 0. 先に一度だけ ── 入っているか確かめる

`schema.sql` を流してあれば（§2）、もう入っています。確かめるなら:

| 見る場所 | あるべきもの |
|---|---|
| Table Editor | `slice_past` という表がある |
| Database → Triggers | `slice` の下に `slice_keep` がある |

**`slice_keep` が無ければ何も残りません。**まず §2 をやり直してください。

**この表は本人と運営（@lingua）しか読めません。**他人からは一行も見えません。

### 1. その人の言語を見つける

**SQL Editor → New query → 貼る → Run。**

`@` のところを、その人のユーザー名に書き換えてください（`@` は付けません）。

```sql
select l.id as language, l.name as 言語名, p.handle as ユーザー名,
       l.created_at as 作った日
  from language l
  join profile p on p.id = l.owner
 where p.handle = 'ここにユーザー名';
```

**`language` の欄に出る長い文字列（uuid）をコピーします。**下の手順で何度も使います。
行が出てこなければ、そのユーザー名の人はいないか、言語を持っていません。

### 2. いま何が入っているかを見る

`ここに language` を、1 で取った uuid に書き換えて Run。

```sql
select kind as 種類,
       length(body) as いまの文字数,
       no as 版番号,
       at as アプリが言う時刻
  from slice
 where language = 'ここに language'
 order by kind;
```

**`いまの文字数` が 0 や極端に小さい行が、消えた種類です。**
単語は `words`、文字は `letters`、キーボードは `kb`、文法は `phases` と `gram2`、
言語の紹介は `wld` です。

### 3. その種類の、前の版を並べる

`ここに language` と `words` を書き換えて Run。`words` のところは 2 で見つけた
種類の名前です。
...
select id as 版,
       no as 版番号,
       kept_at as 上書きされた時刻,
       length(body) as そのときの文字数,
       left(body, 60) as 中身のあたま
  from slice_past
 where language = 'ここに language'
   and kind     = 'words'
...
上から新しい順に並びます。**`そのときの文字数` が大きいまま並んでいて、
どこかで急に小さくなっていれば、その一つ前が「消える直前」です。**

**`上書きされた時刻` はこのサーバーの時計です。**表 `slice` のほうの
`at` はアプリが送ってきた時刻で、その人の端末の時計なので、ずれていることが
あります。**並べ替えは `kept_at` でやってください。**

**戻したい行の `版` の数字を控えます。**

### 4. 戻す

`0` のところに、3 で控えた `版` の数字を書いて Run。**他は書き換えません。**
...
update slice s
  ...
  from slice_past p
 where p.id = 0
   and s.language = p.language
   and s.kind     = p.kind
 returning s.language, s.kind as 戻した種類, length(s.body) as 戻した文字数;
```

**一行返ってくれば戻っています。**`戻した文字数` が、3 で見た
`そのときの文字数` と同じ数になっているのを確かめてください。
**何も返ってこなければ、版の数字が違います。**3 に戻ってやり直してください。

この文は `slice_past` の行から言語と種類を読み直すので、**数字を打ち間違えても
別人の言語に書き込むことはありません。**その番号の行が持っている言語の、
その種類にしか届きません。

**戻した操作そのものも残ります。**戻す直前に入っていた（壊れた）中身が
`slice_past` に一行増えるので、**間違った版を戻しても、同じ手順でやり直せます。**

### 5. 戻ったか確かめる

2 の SQL をもう一度 Run。**`いまの文字数` が戻っていれば済んでいます。**

そのあと、**その人にアプリを開いてもらってください。**

### 気をつけること

**その人の端末に壊れた写しが残っていると、また上がってきます。**
アプリは同期のとき、端末側とサーバー側を突き合わせて、**どちらかが読めない
形になっていると端末側を採ります**（`syMerge()`）。だから、消えた原因が
端末の中の壊れた写しだった場合、サーバーを戻しても**次の同期でまた壊れた版が
上がってきます。**

そうなったかどうかは、**3 の SQL をもう一度見れば分かります** ──
戻した直後より新しい行が増えていて、文字数がまた小さくなっていれば、
上がってきています。そのときはアプリ側の話になるので、こちらに知らせてください。
**この節の SQL では直せません。**

**前の版は、消えずにずっと残ります。**オーナー決定 2026-09-04
「そもそもバグで消えるなら一生残るはずだよね？自分で消してるわけじゃないし」
「基本一生残るよな」。**古い版を消す仕組みは作っていません。**期限もありません。
だから置き場は増える一方です ── 5000 語の言語なら、一語足して保存するたびに
その時点の単語ぜんぶ（約 685 KB）が一行残ります。**Supabase の使用量が
気になってきたら、消すのではなく、まずこちらに知らせてください。**
