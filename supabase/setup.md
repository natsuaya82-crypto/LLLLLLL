...
売上とアナリティクスがどこから来るかは RevenueCat 側になりました。
2026-08-26 の決定（「売り上げもアナリティクスも見れるようにしたい」「アプリの
中で見たい」「画面を開いたときに毎回」）は**どこで見るかだけ**が置き換わった
ものとして残ります。

**コードはまだ残っています。**`supabase/functions/appstore/` と、それを呼ぶ
`www/net.js` の売上まわり、`www/mod.js` の管理画面の数字。鍵が無いので数字は
空欄のままです。**これを消すか、RevenueCat に付け替えるかは決まっていません**
── `docs/BACKLOG.md` にあります。手順は下に残してありますが、**やることでは
ありません**。
