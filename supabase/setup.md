...
**一つだけ例外があります ── § 11 のバックアップです。**あれは順番と関係が
ありません。**一番先にやってください。**入っているかどうかが誰にも分からない
まま、サーバーが唯一の本物になろうとしています。
...
実機 143「なんか全体的に遅くない？」で、人の一覧が往復四回になっていたのを
二回にしました。そのために `schema.sql` の view を二つ触っています ──
`follow_seen`（新）と、`profile_seen`（言語の三列が増えた）。
...
**いまどうなっているか。**
**分かりません。**この文書に、バックアップのことが**一行も書かれていません**
── `backup` `PITR` `バックアップ` の三つで検索して、**どれも 0 件**でした
（2026-09-04 確認）。**セッションからはダッシュボードに入れません。読んで
確かめただけで、画面は誰も見ていません。**入れたのか入れていないのか、
記録が残っていません。

**なぜ一番大事か。**
2026-09-04 に**オンライン前提に切り替える**と決まりました ──
「オフラインをなくそう。写しも別に今はいらなくない？」。
**サーバーが唯一の本物になります。**iPhone には言語を一つも置きません。

SNS の分（投稿・下書き・プロフィール・フォロー）は元からサーバー一箇所に
しかありません。それが普通で、X もインスタも同じです ──
