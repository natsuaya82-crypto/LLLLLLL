...
Deno.serve(async (req: Request) => {
  ...
  /* そして書きます。**読んでから書くのではなく、一つの文で。**
     -----------------------------------------------------------------------
     OWNER 2026-09-12「オンラインで出してね流石に」「4 起動の時に表示して
     ☑️今後表示しない 閉じる みたいなポップにしたくない？」。下がったなら
     下がる前の段（`was`）を行に書き、上がったら消し、同じなら触らない ──
     その三つは `supabase/schema.sql` の `plan_put()` が、**行を握ったまま**
     前の段と比べて決めます。ここで `plan` を読んでから書いていた間は、二つの
     答えが同時に来るとどちらも古い段を読み、`was` を取り違えました
     （r63-audit SQ7）。段の梯子は `verify.mjs` の `ORDER` 一つで、それを渡します。

     書いた行が返ってきます。**答えに載せる段も `was` も `lapse_seen` も、
     行が今持っているもの**で、ここで組み立て直したものではありません。
     「スタッフは消えないんじゃねえの？」 OWNER 2026-09-12、実機。段は
     `purchase` の行から決まり、staff の人には purchase がありません ──
     ここが決めるのは 'free' です。行は 'pro' になります（`plan_staff_hold()`、
     2026-09-05「管理の画面でスタッフ設定を@でできるでしょ？そこに記載されてる
     人だけずっとプロに」）。**段を決める場所は表とトリガーの一箇所で、この函数は
     自分の計算を返さない。** staff の条件をここに足さないのも同じ理由です。
     `until` だけは行にありません ── Apple の日付で、この表の列ではないので、
     決めたものをそのまま返します。staff の 'pro' には日付が無く、それは
     「期限が分からない」であって「期限が無い」ではない（www/store.js）。 */
  const wrote = await fetch(`${url}/rest/v1/rpc/plan_put`, {
    method: 'POST',
    headers: head,
    body: JSON.stringify({ who: uid, rung: plan, ladder: ORDER }),
  });
