...
const SUBS = [
  ...
  'Lingua\tPlus\t4.99\tUSD\t3.49\tUSD\t\tUS\t5\t0\t5',
].join('\n');
const EVENTS = [
...
let ADMIN = true, DAYS_READY = 1, SEEN = [];
globalThis.fetch = async (url, opt) => {
  ...
  const ready = new Date(Date.now() - DAYS_READY * 86400000)
    .toLocaleDateString('en-CA', { timeZone: 'America/Los_Angeles' });
  if (day !== ready) return new Response('', { status: 404 });
...
is('downloads are first-time only', d.sales.downloads, 3);
is('an update is not a download', d.sales.updates, 9);
is('a re-download is its own number', d.sales.redownloads, 2);
is('two currencies stay two rows', d.sales.money,
   [{ cur: 'JPY', proceeds: 700 }, { cur: 'USD', proceeds: 3.49 }]);
is('live subscriptions add the Active columns', d.subs.live, 21);
is('both kinds of cancel count', d.events.cancel, 3);
is('renewals', d.events.renew, 9);
is('reactivations', d.events.back, 1);
is('the day comes back with the numbers', d.sales.day, d.events.day);

/* 6. yesterday is not ready yet -- it walks back */
DAYS_READY = 3; SEEN = [];
r = await call('Bearer x');
const e = await r.json();
const want3 = new Date(Date.now() - 3 * 86400000)
  .toLocaleDateString('en-CA', { timeZone: 'America/Los_Angeles' });
is('walks back to the day Apple has', e.sales.day, want3);
is('and tried the two days before it', SEEN.filter((s) => s.indexOf('SALES') === 0).length, 3);

/* 7. nothing ready at all -- blank, never nought */
DAYS_READY = 99;
const f = await (await call('Bearer x')).json();
is('nothing ready is null and not 0', [f.sales, f.subs, f.events], [null, null, null]);
