...
/* The browser comes from tools/browser.mjs and so does LAUNCH. This file had
   its own copy of the loader and its own LAUNCH, and the copy is what broke
   it: browser.mjs turns Chromium's own 「swipe back」 off in ARGS, and a
   LAUNCH built here carries none of them -- so the drag from the left edge
   below was taken by the browser, which went back to about:blank, and every
   evaluate after that failed on a page with no app in it. */
...
/* The browser's own 「swipe back」 is off at launch (tools/browser.mjs), which
   is what makes the drag from the left edge below reach the app at all. */
