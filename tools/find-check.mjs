...
/* ---- 10. 当たるのは投稿の四つの場所 ------------------------------------
   綴った行と、意味と、言語の名前と、**タグ**。書いた人の名前と @ には
   当たりません ── 人は人の検索が答えるもので、同じ言葉が二つの答えを持つと、
   どちらが出たのか誰にも分からなくなります。

   タグが四つ目になったのは 2026-09-15、「#はべつで」でタグが本文の外に出た
   日です ── **タグは投稿の物**（`body.tags`）なので投稿の検索が答えます。
   書いた人は投稿の物ではないので、その線は動いていません。

   `#` の有る無しはここでは見ません。箱に入った言葉から `#` を落として枠に
   当て、付けたまま本文に当てる ── その二本立ては `tools/post-check.mjs` の
   § 26 が持っています。ここが見るのは**どの欄に当たるか**だけです。 */
say(p1.typing.fields.join(',') === 'ln,mn,lname,tags',
    '当たるのは行と意味と言語の名前とタグの四つ (' +
    p1.typing.fields.join(',') + ')');
...
/* 4. お題から書き始めると、**タグの枠の先頭**に札が入っている ── これが
      「前の日も出る」を支えている一つです。札がどの投稿にも乗っていなければ、
      その日のお題を名指しする問い合わせだけが答えることになり、それが
      「今日しか出ない」の正体でした。札は今も投稿に乗ります ── 乗る場所が
      本文から枠（`body.tags`）に変わっただけで、検索がそこにも当たることは
      § 10 が持っています。

      **本文には入りません。**「#はべつで」 OWNER 2026-09-15 ── 入れていた頃は
      アプリの言葉がその人の 140 文字から引かれていました。

      そして**外せる**こと。枠は打てる欄なので、消したら消えます。意味のほうは
      お題の下では読み取り専用のままです（OWNER DECISION 2026-08-23 #5
      「消せないようにしよう そこからのやつは」）。 */
const composed = await pg.evaluate(() => {
  PW = pwBlank();
  openPost('day');
  /* 枠が持つのは**綴り**（`#` 無し）です ── 見える字は読む人の言葉ですが、
     貯まるのは一つ（www/sns.js § THE TAG）。綴りそのものはページに訊きます
     ── ここで書き写すと、この検査が二つ目の綴りになります。 */
  var out = { tag: (PW.tags || [])[0] || '', want: tagClean(DAY_TAG),
              ln: PW.ln, mn: PW.mn, pr: PW.pr };
  /* 外せる ── 枠の一つ目を空にするのは、人が欄を消すのと同じ道
     （pwSetTag）。pwTagsNoDay() は「+ から入り直したときにアプリが外す」道
     なので、ここでは使いません ── 人が消せることを見たいので。 */
  pwSetTag(0, '');
  out.after = (PW.tags || []).join('|');
  out.prAfter = PW.pr;
  /* 意味のほうは読み取り専用のまま。 */
  pwSetMn('べつのこと');
  out.mnAfter = PW.mn;
...
say(composed.tag === composed.want,
    'お題から書き始めると枠の先頭に札が入っている (' +
    JSON.stringify(composed.tag) + ')');
say(composed.ln === '',
    '本文は空のまま ── 札は本文に入らない (' + JSON.stringify(composed.ln) + ')');
say(composed.after === '' ,
    'その札は枠から外せる (' + JSON.stringify(composed.after) + ')');
say(composed.prAfter === 7,
    '札を外しても、その日に答えたことは投稿に残る ── 集めるのは列 (' +
    composed.prAfter + ')');
