...
(function(root){
  ...
  function derivation(data){ data=data||{}; return {id:data.id||id('deriv'),sourcePartOfSpeech:data.sourcePartOfSpeech||null,targetPartOfSpeech:data.targetPartOfSpeech||null,operation:data.operation||'suffix',morphemeId:data.morphemeId||null,form:data.form||null,separator:data.separator===undefined?'-':data.separator,drop:Math.max(0,parseInt(data.drop,10)||0),conditions:object(data.conditions),metadata:object(data.metadata)}; }
  /* `drop` is how much of the stem goes before the piece is added, counted in
     characters off the END -- the same place whichever side the piece lands on,
     because that is where a stem changes. "carry" + drop 1 + "ied" is the case
     it exists for. Without it a language that changes its stem could be written
     down here and simply not happen. */
