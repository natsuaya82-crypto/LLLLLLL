...
| `script` | `SCRIPT` | roman → strokes, letters no word uses yet, **which way the language is written** (`dir`), and **what stands between two letters** (`sp`, in steps of the lattice — absent means 1, which every language was before 2026-09-23; written only when somebody chooses one in 設定 → 言語 → 字間), and and `blk` -- **how a block's square WAS cut** (`{"o":"tb","a":"q"}`), written by 1.0.3 builds before 2026-09-26 r105 and read by nothing now: a block's pieces stand where they were drawn (`wsParts()`), so nobody chooses a cut. It is not rewritten and not removed. `langRead()` keeps every field the slice holds | object |
