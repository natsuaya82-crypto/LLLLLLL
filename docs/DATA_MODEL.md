...
| `script` | `SCRIPT` | roman → strokes, letters no word uses yet, **which way the language is written** (`dir`), and **what stands between two letters** (`sp`, in steps of the lattice — absent means 1, which every language was before 2026-09-23; written only when somebody chooses one in 設定 → 言語 → 字間), and **how a block's square is cut** (`blk`, the vowels not cut side by side, by how: `tb` under what comes before them, `q` the square in four (田) — `{"o":"tb","a":"q"}`; a vowel it does not name, or names with a value a build does not know, goes beside; written only from the vowel's own letter page, 置き方). `langRead()` keeps every field the slice holds | object |
...
| `tags` | **what the post is filed under** — up to four, as they were typed, with the `#` off and the case left alone. **Absent means none**, which is what every post written before 2026-09-15 looks like: until that day a tag was characters inside `ln`, and those are not rewritten (「タグは別の枠」 OWNER 2026-09-15). Frozen like everything else here — the row under the translation draws these and asks nothing of the open language. `tagsOf()` in `www/sns.js` is the one place that reads them and it applies the ceiling again on the way out. The day's tag is stored as the one spelling (`DAY_TAG`) and drawn in the reader's own language; what GATHERS a day's answers is still the `pr` column and never this |
