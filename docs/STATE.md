...
2. **Plus と Pro を一つのサブスクリプショングループにまとめる。**
   ...
   が、グループの件はアプリでは直せません。
3. **サンドボックスで購入・復元・更新を試す。**一度も試されていません。
4. **次のビルドで、料金が $4.99 と出る原因を見る。**アプリが画面で言うように
   してあります ── 問い合わせが「落ちた」のか「空で返った」のか。そのどちらかが
   出れば原因が二つに絞れます。

**私からは見えないので、確認してほしいもの。**済んでいるなら消してください。

5. ウィジェットのプロビジョニングプロファイル（`Lingua Widget Distribution`、
   `com.tokinets.lingua.widget`）── `docs/apple.md` § 4。無いと署名が通りません。
6. Supabase の Apple / Google サインインが ON か ── `supabase/setup.md` § 4。
7. メールの送信設定と DNS ── `supabase/mail.md`。
8. 通報を読む人（スタッフ）── `supabase/setup.md` § 5。
9. 請求の上限 ── `supabase/setup.md` § 6。
10. App Store Connect の売上キー ── `supabase/setup.md` § 10。
