...
3. **審査用のデモアカウント。**扉を通らないと何もできないので、審査員が入れ
   ...
   二つ三つ入れておくこと。
4. **App Store Connect のプライバシー質問票。**`PrivacyInfo.xcprivacy` は
   2026-09-03 に入りましたが、そこに書いてあるのは**覗いている API の申告だけ**で、
   「何を集めているか」は書いていません。空の配列は「何も集めていない」という
   嘘になるので、意図的に省いてあります。答えるのは App Store Connect の質問票で、
   そこはこのリポジトリから触れません。集めているもの: メールアドレス、ハンドル、
   名前、写真、声、書いたもの。
