...
リーダーが直接書いています（MCP の投げ口が途中で落ちたため）。枝は
`claude/reader-gweflc`。他の枝は畳んであります ── `git branch -r` に並んでいる
ものは古い作業で、**取り込まれているかどうかは
`git merge-base --is-ancestor <枝> origin/master` で訊くこと**。名前から
推測しないこと。

### 今日、実機から来て直したもの（ビルド未確認）

`docs/CHANGELOG.md` の 2026-09-01 の節が全部です。読む値打ちがあるのは、
**どれも何も投げず、検査が全部緑のまま出ていた**という一点：

- **ポップのボタンが一つも効いていませんでした。**`#pop` は `#sbg` の中で、
  押されたものを聞く仕掛けは `#app` と `#tabs` にしか付いていない。名前は
  登録済み、関数も存在、聞き手だけ不在。`press` は `#app` の中を押すので緑。
  → `boot.js` が `#sbg` にも付けます。`press` に「立っている場所で押す」claim。
- **無料に戻すと、隠れた文字の音が空のマスに化けて**居座っていました。
- **PDF から取り込んだ「1」がアルファベットに入っていました**（手書きシートの
  道だけが振り分けていた）。
...
`confirm()` からアプリ自前のポップ（`popAsk`）に替えた時、`plan-check` は
`window.confirm` を差し替えて質問文を読む作りのままでした。ゲート全部を
回さずに取り込んだのが理由です。**ゲートは取り込んだ者が一度回す** ──
`CLAUDE.md` のこの行は、今日それを守らなかったことで一度証明されました。

### オーナーの側に残っているもの ── 2026-09-02 現在

**確実に残っているもの。**私にはできない作業です。

1. **`supabase/schema.sql` をダッシュボードに流す。**手順は
   `supabase/setup.md` § 2。当たるまで動かないもの：プラン表、`netLangsDown`、
   通知のまとめ、フォロワー数、**そして 2026-09-02 に足した単語と文法の DL**
   （`slice_dl()` と広げた `slice_read`）。流すまで、人の言語のページで単語と
   文法の ↓ を押しても何も落ちてきません。何も壊れません。
2. **Plus と Pro を一つのサブスクリプショングループにまとめる。**
   App Store Connect → Lingua → 収益化 → サブスクリプション。いまは別グループに
   なっていて、Plus の人が Pro を買うと**二本走ります**（Plus は止まらず、両方
   課金される）。同じグループなら Apple が格上げとして扱い、Plus の残りを日割り
   で返し、Pro がその日から始まります。`docs/apple.md` § 4 は最初から一つの
   グループと書いています。アプリ側は同じか下のプランの二度目の購入を断ります
   が、グループの件はアプリでは直せません。
3. **次のビルドで、料金が $4.99 と出る原因を見る。**アプリが画面で言うように
   してあります ── 問い合わせが「落ちた」のか「空で返った」のか。そのどちらかが
   出れば原因が二つに絞れます。**まだどのビルドにも入っていません**（最後に出た
   のは #106 で、この作業は全部その後）。

**私からは見えないので、確認してほしいもの。**済んでいるなら消してください。

4. ウィジェットのプロビジョニングプロファイル（`Lingua Widget Distribution`、
   `com.tokinets.lingua.widget`）── `docs/apple.md` § 4。無いと署名が通りません。
5. Supabase の Apple / Google サインインが ON か ── `supabase/setup.md` § 4。
6. メールの送信設定と DNS ── `supabase/mail.md`。
7. 通報を読む人（スタッフ）と、**自分に admin を付けること** ──
   `supabase/setup.md` § 5。運営専用ページは設定画面の見出しを7回叩くと開くが、
   `NET_ADMIN` が false だと無反応で、それはサーバーが答える。
8. 請求の上限 ── `supabase/setup.md` § 6。
