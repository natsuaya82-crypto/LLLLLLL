...
（Actions「Supabase Schema」apply、2026-09-26 16:27 UTC ── トリガーは `push_on_post` に、`posts_by()` あり）、`push-send` も置き直した。
...
- **r104〜r106 と輪**: 組み合わせの置き方は描いた位置のまま重ね、終声だけ下の段へ（r105、置き方の選択は消した）、文字の書き出しは
  ...
  字を描く画面の「?」は字の名前の横（r106）、押した所に金の輪（`actRing()`・`#ring`）。
- ストアのキーワード 10 言語（`store/*.json`）。カテゴリはグラフィック&デザイン（1.0.3 の版を作る時に App Store Connect で）。

**本番のサーバー**: 済み（上）。Actions に「Supabase Schema」（`check` 読むだけ／`apply` 流す）がある ── スマホから押せる。
...
**まだ終わっていないこと**: iPhone の通知が Apple に 403 で断られている（`push-send` の答えの `left`、理由は次の一回で出る形にした）
── オーナー「通知そんな大事じゃない」で後回し。again-check の改名の段が一度だけ赤になった原因は再現できていない（待ちを条件に書き直した）。
