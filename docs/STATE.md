...
## 2026-09-26 ── 1.0.3 の中身が integ-0905 に揃った（**ビルドはオーナーの「出して」待ち**）

ゲートは r103 を入れた所で 60 のうち `press` 一つが赤（r103 の「?」の中で行に `margin-top` を付けていた）── `.grpsep` に書き直して
回し直している。`rls` は r100 を入れた所で緑（600/0 台、`posts_by()` を足した後）。

入った物（決定ログ 2026-09-25・26 の各項、写真は `shots/r94/`・`shots/r96-*`・`shots/r97-*`・`shots/r99-*`・`shots/r100-*`・
`shots/r101/`・`shots/r102/`・`shots/r103/`）:
- **r94**: いいね・リポストした人の一覧（長押し、`react_seen`）、引用リポスト（`post.quote_of`、引用の通知と iPhone の通知 `push_quote`）、
  リポスト・フォローを外す時の確認、スレッドの詳しい時刻、意味を出さない投稿（`body.nm`）、本文は 15px・意味は 0.8 倍。
...
- ストアのキーワード 10 言語（`store/*.json`）。カテゴリはグラフィック&デザイン（1.0.3 の版を作る時に App Store Connect で）。

**オーナーがやること（ビルドの前）**
1. 本番の SQL Editor に `supabase/schema.sql` を全部貼って Run（引用の列 `quote_of`・`react_seen`・引用の通知とトリガー・引用を数える
   `post_seen.boosts`・プロフィールの `posts_by()`）。
2. そのあと Actions → Supabase Deploy → `push-send`（引用の iPhone の通知の種類が増えた）。順番は 1 → 2。

**実機でしか見られない物**（Swift はここでビルドしていない）: 投稿画面をキーボードが出た後に揺らしても動かないか・キーボードの切り替え、
評価のお願い（TestFlight では iOS が出さないことがある）、手書きのキーボード、長押しの一覧と長押しのコピー、SVG を共有シートから出して開けるか、
組み合わせの一マスがフォント・キーボードに乗るか、`docs/scope/r97-still.md` § 実機で見ること。

**オーナーに訊いていること**: SVG は文字（全部・一文字）で作った ── 決定の「一行を自分の字で」の SVG も要るか。組み合わせの一マスの
区画の割合（左右は半々、終声の段は 4 割）。二択のポップの二つ目（「SVG」「引用」）が「いいえ」と同じ灰色で出ること。
「〇〇がリポスト」の印をタイムラインとプロフィールに出すか（今はどこにも無い）。script-maker（kerbash.github.io）から取り入れる物 ──
この環境のネットワークが開くのを待っている。

**press の「could not be built」**: 「a word, read」の面は二度目から作れない（一つ前の押しで開いた編集の KEEP が seed() で消えず、
`wdSigEdit()` が空の `wEdit` を読む）。検査の側の穴で、その面の二つ目以降のボタンは押されていない。`docs/BACKLOG.md` にある。
...
master、run 36111745883）。入った物: r84（保存を押したら・3 つ前まるごと・開いた時に読む五つ・親の承認・上限の文）、
r85（ミュートと非表示リスト・ブロックの両向き・前の版の用紙と声を消す）、r86・r89・r90・r92（検査の側の直し）、
r87（誰が言語を読めるかを `lang_readable()` 一つに）、r88（タブで出る時も保存を訊く・Pro の上限で＋を出さない・
ミュートはリポストと通知も・ミュート中は赤い斜線）、r91（課金を RevenueCat SDK に ── Shipaton、公開キー入り）、
schema.sql の 42501 の直し。**RevenueCat の Swift はこのビルドで初めて組み立てる。**
オーナーの手: Supabase で schema.sql をもう一度流す／届いたら実機で買えるか見る → 審査（Shipaton、日本時間 10/1 15:45）。
