...
4. **ミュートのマーク** ── 投稿や人のページの「…」を押すと出る一覧の「ミュート」の行の左に付く小さな絵。今は音を鳴らす時と同じスピーカーの絵を仮に使っている。斜線の入ったスピーカーにするか。

### 9月上旬から残っていて、今のコードで測り直してから訊くもの
5. **消えるのを防ぐ「小さくなったら書かない」守り** ── 版が3つ前まで残るようになったら外してよいか。
6. **一度も触っていない設定** ── 既定の値を「その人の答え」として書くか、空のままにするか。
7. **★を50件より多く付けている人** ── 51件目より古い★が画面から消え、続きへ行く道が無い。
8. **まれに起きる二つ** ── 検索の履歴が一つ消えることがある／同じ言語が一覧に二つ並ぶことがある。
