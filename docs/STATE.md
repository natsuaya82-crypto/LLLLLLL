...
## 2026-09-26 ── 1.0.3 の中身が integ-0905 に揃った（ゲート全部緑・rls 581/0、**ビルドはオーナーの「出して」待ち**）

入った物（決定ログ 2026-09-25 の各項、写真は `shots/r94/`・`shots/r96-*`・`shots/r97-*`）:
- **r94**: いいね・リポストした人の一覧（長押し、`react_seen`）、引用リポスト（`post.quote_of`、引用の通知と iPhone の通知 `push_quote`）、
  リポスト・フォローを外す時の確認、スレッドの詳しい時刻（本文の下の右）、意味を出さない投稿（`body.nm`）、本文は 15px・自作文字は今のまま・意味は 0.8 倍。
...
- **r96**: 字を選ぶ画面は一つ（`pkKindsHTML()`・`pkKind()`、種類は `WORLD_SCRIPTS`）、複数のキーに①②③の順で確定で一度に、
  フォントの書き出しは文字の画面の右上（`ltFontOut()`）、手書きのキーボード（候補 8 つ、近さは `ios/App/LinguaKeyboard/hand.js`）。
- **r97**: 投稿の画面は揺れない（`keepStill()` がキーボードの分だけアプリの画面を縮める、追いかける JS は消えた）、App Store の評価のお願い（`rateOpen()`、開いた五回目）。
- ストアのキーワード 10 言語（`store/*.json`、1.0.3 の版と一緒に送る）。カテゴリはグラフィック&デザイン（1.0.3 の版を作る時に App Store Connect で）。

**オーナーがやること（ビルドの前）**
1. 本番の SQL Editor に `supabase/schema.sql` を全部貼って Run（引用の列 `quote_of`・`react_seen`・引用の通知とトリガー）。
...
メモの画面、評価のお願い（TestFlight では iOS が出さないことがある）、手書きのキーボード（候補が出て押すと入る）、長押しの一覧、
`docs/scope/r97-still.md` § 実機で見ること。

**オーナーに訊くこと**（作った形のまま待っている）: 端末に残るキーボードの高さの測り（もう誰も読まない）を消すか（DELETE REVIEW は CHANGELOG 2026-09-25）、
評価の「開いた」にバックグラウンドからの復帰を数えるか、文字を打つ間は下のタブ・＋・下の帯を隠す形でよいか、意味の切り替えの置き場所、
引用の数を出すか、ミュートした人の投稿を引用した物の中身を出すか、キーの数より少なく字を選んで確定した時の扱い。

**press の「could not be built」**: 「a word, read」の面は二度目から作れない（一つ前の押しで開いた編集の KEEP が seed() で消えず、
`wdSigEdit()` が空の `wEdit` を読む）。1.0.2 の時から在る検査の側の穴で、その面の二つ目以降のボタンは押されていない。`docs/BACKLOG.md` へ。
