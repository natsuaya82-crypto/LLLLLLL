...
- **言語にアカウントの印。**`langNew()` が `SESS.uid` を押します。印の無い言語は
  **別のアカウントがこの端末に来ていない限りその人のもの**（`SET.uidWas`）──
  印が付き始めたのは今日なので、それ以前の言語を「誰のものでもない」にすると
  既にある端末のほとんどで持ち主から見えなくなります。
...
2. **Plus と Pro を一つのサブスクリプショングループに。**実機で証拠が出ました
   ── Plus を買って「Pro になりました」と出たのは、Pro が同時に生きているから。
   いまは両方走って両方請求されます。アプリでは直せません。
3. **メールの送信設定と DNS**（`supabase/mail.md`）。**これが先です** ── 六桁が
   飛ばないと審査用のデモアカウントも作れません。
4. **審査用のデモアカウント。**扉を通らないと何もできないので、審査員が入れ
   なければ即リジェクト。`natsuaya82+demo@gmail.com` のように自分のアカウントと
   分けること（審査員にパスワードを渡すため）。言語一つ、文字を何個か、投稿を
   二つ三つ入れておくこと。
5. **Supabase の Authentication → Sessions の二つの値**を見て
