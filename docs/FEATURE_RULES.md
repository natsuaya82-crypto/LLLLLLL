...
### アプリは、人が打っていない値を書き込まない
- Date: 2026-09-04
- Area: 保存されるものぜんぶ。発音の推測、文法の既定、その他すべての既定値
- Decision:
...
- Reason: **人が触っていない欄に、アプリが値を書き込んでいます。**文法の語順を
  一度も触っていない言語に既定の値が書かれ、発音を入れていない単語に綴りから
  読んだ音が書かれます。**書き込んだあとは、「その人が選んだ」のか「アプリが
  入れた」のかが見分けられません。**

  **見せるのと、書き込むのは別です。**推測は今までどおり**画面に出ます** ──
  発音を入れていない単語には綴りから読んだ音が出ます。**出るけれど、保存は
  しません。**だから、あとから見ていつでも見分けられます。

  **これは同じ日の「推測も含めて入れる」を差し替えます。**（下の該当項目に
  SUPERSEDED を付けました。）リーダーが二つを別の話として訊いたのが誤りで、
  **同じ形の話でした。**オーナーの答えは一つです ──
  **誰も頼んでいないものを、アプリが書き込まない。**
- Affected features: 単語の発音、文法の語順、その他の既定値
- Affected data: **減ります。**人が打っていない欄は空のまま保存されます
- Affected docs: この項目、`docs/EXPIRY.md`、`docs/DATA_MODEL.md`、`docs/ONE.md`
- Implementation status: **未実装。**オンライン前提への書き換えと同じ回で。

  **「空」と「まだ答えていない」が同じ状態になります。**それでよい、という
  ことです ── **アプリが埋めないので、空はいつでも「人がまだ答えていない」
  を意味します。**
