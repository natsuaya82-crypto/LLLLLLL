...
### 2026-09-26 組み合わせの置き方は母音の文字のページで、四分割まで。書き出しは画面で、カードは作らない（1.0.3）
- Date: 2026-09-26
- Area: 書き方「組み合わせ」（`block`）の置き方、文字の画面の書き出し
- Decision:
  - 置き方は、母音の文字の一文字のページ（文字設定）で、見本の絵のある行から選ぶ。独立した一覧と母音の画面は無い。
  - 置き方は四つに分けるまで: 左右／上下／左右＋下／上下＋下／田の字。母音ごとに選ぶのは左右・上下・田の字の三つで、
    終声があれば左右・上下は下に一段、田の字は下の二つに左から（終声一つなら左下だけ）。四つより多い字の音節は組まない。
  - 文字の画面の共有マークはポップを出さず、書き出しの画面へ進む。そこに「フォント」「SVG」の二行、押すとそのまま書き出す。
  - 書き出すのは描いた文字。カード（一行）の SVG は作らない。
- Reason: オーナーの言葉「4分割までで作れればいいんちゃう？組み合わせは文字のページでうまくできるように」「ますの形ってなに？」
  「それは文字のページの文字設定の時に作れれば良くない？」「そのポップでフォントとsvg出すのはやめてくれ」「カードはいらん。文字書いた後の書き出し。」
- Affected features: 文字の一文字のページ（`vLetter`、`ltCutRows()`）、文字の画面の共有マーク、書き出しの画面（`vLtOut`、route `ltout`）。
- Affected data: `SCRIPT.blk` に値 `q`（田の字）。今ある値は書き換えない（`docs/CHANGELOG.md`）。
- Affected docs: この項、上の「他の道具の強いところを全部入れる」の二行、`docs/DATA_MODEL.md`（`blk`）、`docs/FEATURES.md`。
- Implementation status: 実装（r104、`claude/r104-blk`、CODE CONFIRMED のみ・実機未確認）。区画の割合（間 0.06、左右・上下の終声の段 0.7、
  田の字は四等分）は見た目で仮に決めた。
...
- Decision:
  ...
  - **字を組み合わせた一マス**: ハングルのように、役割（初声・中声・終声など）と型で字を組み合わせて一マスにする書き方。
    置き方は母音の文字のページで選ぶ ── 下の 2026-09-26「組み合わせの置き方は母音の文字のページで、四分割まで」。
...
- Implementation status: 字を組み合わせた一マス（書き方 `block`、`SCRIPT.blk`）と SVG の書き出し（文字ぜんぶ・一文字）は r102
  （`claude/r102-block`）、置き方と書き出しの画面は r104（`claude/r104-blk`）── コード確認、実機未確認。単語の自動生成（`vGen`・`vGenSyl`、`STG.syl`）と語源の系統図
