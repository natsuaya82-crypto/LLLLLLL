...
### 画面の基準、十。そして下から出るシートの禁止が狭まった
- Date: 2026-09-01
- Area: 画面ぜんぶ
- Decision:

  **原文のまま**（`docs/CHANGELOG.md` 2026-09-01 に同じものが入っています。
  あちらは書き換えないので、こちらが決定ログの側の写しです）:
  ...
  「これは Shape の規則を狭めますか」と直接訊いて「**1はイエス**」。

  **禁止のまま残るのは、画面の代わりに使うシート** ── 一画面ぶんの操作を、
  ページを作りたくないからという理由で下から持ち上げたもの。**許すのは、
  指の下にある一つのものを消すか変えるかを訊く、iOS 自身が出す二〜三行のもの
  だけ**、しかも **iOS 自身のもの**だけです（基準1）。**HTML で似せて描いた
  シートは、禁止されている側が許された側の名前を着ているだけ**なので、
  そこは閉じています。`UIAlertController` が要るので `www/` だけでは持てません。

- Reason: 基準1がほかの九つの土台です ──「標準では実現できない場合のみ」独自に
  する、と言っている以上、**「独自のほうが良い」は理由になりません。**
  シートの狭めかたも同じ一行から出ています。
- Affected features: 画面ぜんぶ。**今日ここで直したのは規則だけで、画面は
  一つも触っていません。**
- Affected data: 無し。
- Affected docs: `CLAUDE.md` § Shape（直した）、この項、
  `docs/CHANGELOG.md`（`claude/fo2` が原文を入れた。書き換えない）。
- Implementation status: **規則は直した。コードは誰も触っていない。**

#### コードと合っていない所。直していない ── **二つあります**

**どちらも「見つけたから直す」で触ってよいものではありません。**
片方は書かれた決定と、片方は全画面の作り直しと衝突します。
...
`CLAUDE.md`「Deciding」のとおり**オーナーのもの**です。`tools/face-check.mjs`
が「`:root` が宣言していない顔は使えない／誰も着ていない顔は在ってはいけない」
の両方向を押さえているので、**減らすなら検査と同じコミットで**。
...
### 端末に置くものはほとんど無い。課金とキーボードはアカウントに結びつく
- Date: 2026-09-01
...
- Decision:
  ...
  そしてリーダーが渡した一行 ──「課金とアカウントとキーボードはアカウントに
  結びつく」。
  ...
  4. **プラン（課金）はアカウントのもの。**設定の中ではない。サインインした
     先の端末について来る。
  5. **キーボードは言語の一部**なので、言語と一緒にサーバー（`slice` の `kb`）。
  6. **古い記載は消す。**「これは歴史です」と前置きして残さない ── 読まれる。

- Reason: **書いてあるものが読まれて、間違った説明がオーナーに届いた。**
  `docs/STATE.md` § 3 は「言語はまだ `localStorage` だけ、サーバーに載せるのは
  これから」と書いたままで、`CLAUDE.md` の一覧は「タイムラインは端末に在る」と
  書いたままだった。どちらも逆である。リーダーがそれを読み、そのとおりに
  説明した。**直すのは説明した人ではなく、その文である。**
- Affected features: 無し（この項は仕様の言い直しであって、新しい振る舞いでは
  ない）。ただし 4 はコードと合っていない ── 下を見ること。
- Affected data: 何も消えない。
- Affected docs: `CLAUDE.md`、`docs/STATE.md`、`docs/ARCHITECTURE.md`、
  `docs/DATA_MODEL.md`、`docs/FEATURES.md`、`docs/PAID_FEATURES.md`、
  この項。**`docs/CHANGELOG.md` は書き換えない。**
- Implementation status: **1・2・3・5・6 はコードに在る。4 は無い。**
  プランは `SET.plan`（端末の設定ファイル、実機では Keychain）に在り、
  `profile` に列が無く、`www/net.js` はプランを一度も送らない。
  差は `docs/STATE.md` § 3 の項目4 に書いた。
...
この枝の持ち物ではない。**プランを `profile` に載せるのは購入がサーバーに
