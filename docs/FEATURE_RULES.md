...
- Decision:
  ...
  1. **何か変えたら、右上に保存のボタンが出る。**変えていなければ出ない
...
- Implementation status: **未実装。**`www/shell.js` は `claude/swipe`、
  `www/me.js` は `claude/me3`、`www/keyboard.js` は `claude/keysel` が
  持っている。**三つに半分ずつ配線するのが一番危ないので、三つが戻ってから
  一つの担当でまとめて入れる**（リーダーの判断、2026-09-03）
