...
### 2026-09-25 キーの画面 ── 字は種類の一覧から選ぶ、「なし」は消す、スペースは記号の中（1.0.3）
- Date: 2026-09-25
- Area: キーボードの編集（`www/keyboard.js` のキーの画面）
- Decision:
  - キーに載せる字は、キーの画面の**種類の一覧**から選ぶ: 自作文字・アルファベット・数字・記号・句読点・ひらがな・カタカナ・ギリシャ文字・
    キリル文字・絵文字。行を押すとその種類の字が並んだ画面に行き、押した字がキーに載る。一覧に無い字は上の検索欄に打つと出て、そこから選ぶ。
  - **「なし」は消す。** **スペースは記号の中に一つの字として置く。**
  - キーボードの編集画面に r95 が足した「文字を入力」欄は消す ── 字を選ぶ場所はキーの画面の一つだけ。
- Reason: オーナーの言葉「そもそもキーの編集ボタン押したら、文字選択できたやん。なんで勝手に追加するの？」
  「自作文字 アルファベット 〇〇 〇〇 って並べて開いたらそこから選べるようにするのは？」「なし消して。スペースキーを記号のとこに追加すればいい」。
- Affected features: キーの画面、キーボードの編集画面の入力欄（消す）。
- Affected data: 無し（キーに載る字の形は今のまま）。
- Affected docs: この項、`docs/keyboard.md`、2026-09-25「キーボードはプランで分けない」の「既存の文字はキーボードの編集画面で直に」。
...
- Decision:
  ...
  - **キーに置ける字は二種類**: その言語の自作文字と、既存の文字（Unicode のどの字でも）。既存の文字はキーの画面の
    種類の一覧から選ぶ（2026-09-25「キーの画面」の項）── 文字の画面を通さない。自作文字だけが文字の画面で描いて登録する。
