...
- Decision:
  - キーを複数選んでいる時もペンのマークが押せる。
  - 選んだキーの右上に、選んだ順の番号（①②③…）を小さく出す。
  - ペンで字を選ぶ画面に行き、キーの数だけ字を選ぶと、選んだ順に①のキー・②のキー…に入る。
  - **字は押してもすぐには入らない。** 右上の確定を押すと入り、キーボードの編集画面に戻る。キーが一つの時も同じ
    （誤タップを直す時に困らないように）。
- Reason: オーナーの言葉「複数キーでも鉛筆マークつくようにして、編集でキー分文字選んだら一括で設定できるようにしたいな。
  選択した順に右上に小さく①②とかついてその順に選択した文字がキーに入る」「文字選んだらすぐ入るんじゃなくて右上の確定押したら
  文字が入ったキーボード設定の画面に戻る これで。1つの場合も一緒。じゃないと誤タップで直す時ストレスになる」。
...
### 2026-09-26 組み合わせの一マスは描いたままの位置で重ねる、終声だけ下の段へ。書き出しは画面で、カードは作らない（1.0.3）
- Date: 2026-09-26
- Area: 書き方「組み合わせ」（`block`）の一マス（`wsParts()`・`wsInto()` in `www/wsys.js`）、文字の画面の書き出し
- Decision:
  - 一マスは、部品（初声・母音・終声の描いた字）を**描いたままの位置と大きさで重ねる**。縮めない、区画に押し込まない。字を描く画面は
    もともと田の字に四つに区切られていて、どこに描いたかがそのまま置き場所になる。置き方を選ぶ所は無い。
  - 例外は終声だけ: 一つ目は下の段の左、二つ目は下の段の右。上半分だけに描かれた終声は半マス下へ、二つ目は左半分だけに描かれていれば
    さらに半マス右へ。下半分・右半分にかかっている字はその向きには動かさない。
  - 組まない音節: 終声の後に母音、終声が三つ以上、五つ以上の字。
  - 文字の画面の共有マークはポップを出さず、書き出しの画面へ進む。そこに「フォント」「SVG」の二行、押すとそのまま書き出す。
  - 書き出すのは描いた文字。カード（一行）の SVG は作らない。
- Reason: オーナーの言葉「せっかく4つに区切ってるから、それうまく利用しない？そうすれば置き場所指定しなくても入れるやん？」
  「そのポップでフォントとsvg出すのはやめてくれ」「カードはいらん。文字書いた後の書き出し。」
- Affected features: 組み合わせで書いた語の一マス（見出し・辞書・フォント・キーボード）、母音の文字のページ（置き方の行が無くなる）、
  字を描く画面の「?」、文字の画面の共有マーク、書き出しの画面（`vLtOut`、route `ltout`）。
- Affected data: 無し。`SCRIPT.blk`（前の置き方）は書き換えず消さず、読まなくなるだけ（`docs/CHANGELOG.md`）。
- Affected docs: この項、上の「他の道具の強いところを全部入れる」の二行、`docs/DATA_MODEL.md`（`blk`）、`docs/FEATURES.md`。
- Implementation status: 実装（一マスは r105、`claude/r105-quad`。書き出しの画面は r104）── CODE CONFIRMED のみ・実機未確認。

### 【差し替え済み 2026-09-26】組み合わせの置き方は母音の文字のページで、四分割まで。書き出しは画面で、カードは作らない（1.0.3）
- 差し替えた決定: 「組み合わせの一マスは描いたままの位置で重ねる、終声だけ下の段へ。書き出しは画面で、カードは作らない」（2026-09-26）

### 2026-09-26 リポストのポップは今のまま（1.0.3）
- Date: 2026-09-26
- Area: 投稿のリポストのマーク（`postBoost()`、`www/post.js`）
- Decision: リポストのマークを押すと「リポスト／引用」のポップ（今の形、二つ目の「引用」は灰色のまま）。一回押す・長押しで分ける形にはしない。長押しは今どおりリポストした人の一覧。
- Reason: オーナーの言葉「リポストのポップそれで行こう。ごめん忘れて」。
- Affected features: 投稿のリポスト・引用。
- Affected data: 無し。
- Implementation status: 今の形のまま（変更なし）。
...
- Decision:
  - **単語の自動生成**: その言語の音と音節の形から、合う単語を作って並べ、選んだ物を辞書に入れる。新しい章として作る（前の Make を戻すのではない）。
  - **語源の系統図**: 語と語の由来（派生・借用・合成など）をつないで、家系図として見られる画面。
  - **字を組み合わせた一マス**: ハングルのように、役割（初声・中声・終声など）で字を組み合わせて一マスにする書き方。
    部品は描いたままの位置で重ね、終声だけ下の段へ ── 下の 2026-09-26「組み合わせの一マスは描いたままの位置で重ねる」。
  - **SVG の書き出し**: 描いた文字を SVG で書き出す（文字ぜんぶ・一文字）。カード（一行）の SVG は作らない ── 同じく下の項。
  - **使い方は各画面の「?」の中にたくさん描く**（画面そのものに説明を書かない規則はそのまま）。
  - 無料・ブラウザ版は入れない ── Lingua の強みはスマホのアプリで、他の強いところを取り込んで差を付ける。
  - 全部 1.0.3。
- Reason: オーナーの言葉「向こうの強いところ全部パクろう」「単語自動生成もやるか。足そう。組み合わせてやるのも作ろう svgも足そう。
  ？の中に描きまくろう」「linguaの強いところはスマホアプリだから他からパクって優位性を確立した方がいい」「1.0.3でいいよ」。
...
### 2026-09-26 「〇〇がリポスト」は投稿者の名前の下、字を描く画面の「?」は字の名前の横（1.0.3）
- Date: 2026-09-26
- Area: 投稿の行（`postRow()` in `www/post.js`）、フォロー中のタイムラインとプロフィールの一覧、字を描く画面（`vGlyph()` in `www/glyph.js`）
- Decision:
  - リポストとして並んでいる行には、**投稿者の名前の行のすぐ下**に小さい一行「↻ 〇〇がリポスト」（〇〇はリポストした人の表示名、
    自分なら「あなたがリポスト」）。押すとその人のプロフィール。
  - 字を描く画面の「?」は、道具の段の下ではなく、**バーの字の名前（例「k」）のすぐ横**。ほかの画面と同じ置き方。
    2026-08-28「帯の下にしよう」はこれで差し替え。
- Reason: オーナーの言葉「vethの名前の下に〇〇がリポストって入れよう」「？はkの横にしよう」。
- Affected features: フォロー中のタイムライン、プロフィールの投稿、字を描く画面。
- Affected data: 無し（`feed_fo()`・`posts_by()` がリポストした人の名前とハンドルも返す ── 読み方だけ）。
- Implementation status: 実装（r106、`claude/r106-rp`、CODE CONFIRMED のみ）。本番の `schema.sql` を貼り直すまで他人のリポストの印は出ない。

### 2026-09-26 プロフィールに自分のリポストも出す（1.0.3）
- Date: 2026-09-26
- Area: プロフィールの投稿の一覧（`netPostsBy()`、サーバーの一覧）
- Decision: X と同じく、プロフィールの投稿の一覧にその人のリポストも混ぜる。並びはリポストした時刻。「〇〇がリポスト」の印は投稿者の名前の下（上の 2026-09-26「「〇〇がリポスト」は投稿者の名前の下」）。
  書いた投稿とリポストを一つの一覧にして返すのはサーバー（タイムラインが混ぜているのと同じ仕組み）。引用は投稿なので今のまま出る。
...
- Decision:
  - **角丸・枠の残り（box-baseline に載っている古い物）はそのまま。** 新しく足さないルールは今のまま。
  - **決まったマークの無い操作（選択・完了・保存など）は字のまま。**
  - **電波の無い時に作って一度も上がっていない古い言語は、特別に扱わない。** 保存にはサーバーが要るので、上がっていない物は残らない。
  - **端末の設定に残る四つ（`order` `script` ── 古い版の移行の元、`read` `voice` ── 誰も読まない）は消す。**
  - **録音と書き出したシートは端末に残さない。** 録音は投稿・下書きと一緒にサーバーにあり、書き出したシートは渡したら端末に残さない。
    アカウントを削除した時に端末に残る物は無い。
- Reason: オーナーの言葉「角丸は大丈夫」「マークなくていいやつはいい」「4 そもそも保存するのにサーバーが必要だから、消えるよね」
...
- Date: 2026-09-26
- Area: スレッドで開いた投稿（`www/post.js`）、コピー
- Decision:
  - スレッドで開いた投稿の本文と意味を、長押しで選んでコピーできる。タイムラインの一覧では選べない（スクロール・長押しの一覧と取り合わない）。
  - 本文の自作文字は、**自分の言語の投稿**ならアプリ内の欄に貼ると自分の自作文字で出る。**他の人の投稿**はローマ字のつづりで貼られる
    （その人の形を自分の字にすり替えない）。アプリの外へはどちらもローマ字のつづり。
...
- Date: 2026-09-26
- Area: 投稿画面の意味の欄（`www/post.js`、`pwMn()`）
- Decision:
  - 意味のオン・オフは**丸いトグル（設定画面と同じスイッチ）**。「意味」と書いた字の切り替えは消す。
  - 自動の意訳は**プレースホルダーではなく、欄の中身の字として入る**。書き直せる。
  - キーを複数選んで、キーより少ない字を選んで確定した時は、選んだ数の分だけ①から順に入り、残りのキーは今のまま（今の形のまま）。
...
- Date: 2026-09-25
- Area: 投稿画面（`www/post.js`）、投稿の行の文字（`www/index.html` の `.pline`・`.pmn`）
- Decision:
  - **投稿画面に切り替えを一つ**置き、意味の行を出さずに投稿できる（出さない投稿は、タイムライン・スレッド・カードで意味の行が無い）。
    オフの時は Twitter と同じように投稿する ── 意味の欄は無く、意味は投稿に載らない（「意味をオフにした場合はTwitterと同じように投稿できる」）。
  - **投稿の本文の普通の文字は Twitter と同じ大きさ**（本文 15pt 相当）。自作文字の大きさは今のまま
    （「自作文字の表示サイズはちょうどいいんやけど、既存文字はでかい」2026-09-25）。
  - **意味（翻訳）の行はその 0.8 倍くらい。**
- Reason: オーナーの言葉「投稿画面の前にトグル置いて意味を非表示にするってできたら、スッキリした画面にならない？」
...
- Date: 2026-09-25
- Area: 投稿（`www/post.js`）、タイムライン・スレッド（`www/sns.js`）、フォロー（`www/me.js`）、サーバー（`supabase/schema.sql`）
- Decision:
  - **いいね・リポストした人の一覧**: 投稿のハート・リポストの印を長押しすると、押した人の一覧の画面（フォロー一覧と同じ形、新しい順）。
  - **引用リポスト**: リポストの印を押すと「リポスト／引用」。引用は投稿の画面が開き、元の投稿が下に小さく付く。元が消えた・
    ブロックで見えない時は「この投稿は表示できません」。引用されたら元の人に通知。
  - **解除の確認**: リポストを取り消す時とフォローを外す時に「解除しますか？」の確認のポップ。
  - **投稿の詳しい時刻**: スレッドで開いた投稿に、Twitter と同じく本文の下（右下あたり）へ詳しい日時（時刻・年月日）を足す。
    名前の横の「2h」はそのまま。
- Reason: オーナーの言葉「リツイートといいねした人長押しで見れるようにしたいのと引用リツイート追加しない？」「次でいいよ」
  「リツイート解除とかフォロー解除は開錠しますか？みたいなポップつけて欲しい」「ツイートの詳細時刻出るようにして欲しい」
