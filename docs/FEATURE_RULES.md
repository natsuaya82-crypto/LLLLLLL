...
### 2026-09-25 ミュート・ブロックの残り・サインインの着地・前の版のファイル（r82 の問いへの答え）
- Date: 2026-09-25
- Area: 設定の非表示リスト、ブロック、サインイン後の画面、スマホに残ったファイル
- Decision: オーナーの答えをそのまま右に。
  - 「非表示リスト」は何を並べるか → 「a」＝**人をミュートする機能を作り、その一覧にする**（ミュートした人の投稿は
    タイムラインに出ない。ブロックはしない）。解除は設定の非表示リストから。
  - サインインし直した時の着地 → 「プロフィール」（今のまま。開いた時の最初の画面はタイムライン、サインインし直した時は
    プロフィール）
  - ブロックした相手の公開言語 → 「見えなくする」（言語の一覧・検索・人のページから）
  - ブロックした相手からの iPhone の通知 → 「そもそもブロックした相手から見れるのおかしくね」＝ブロックされた側は
    こちらが見えないので、いいね・返信・フォローもできず、**通知も来ない**（サーバーで止める）
  - 前の版でスマホに残った用紙と声のファイル → 「はい」（消す。DELETE REVIEW を CHANGELOG に）
- Reason: オーナーの言葉のまま。
- Affected features: 設定、タイムライン、ブロック、通知（push-send）、言語の一覧、スマホのファイル
- Affected data: ミュートの表（新しい）、`block_hides()` の範囲、スマホの Documents/Sheets・Voices（消す）
- Affected docs: この項、`docs/STATE.md`、`docs/FEATURES.md`・`docs/RECOVERY.md` の Documents の文（直す session が）
- Implementation status: 未。r84 の後に出す。
