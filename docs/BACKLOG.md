...
## `press` の「一つの list は一つの高さ」の数は、同じコードで ±2 揺れます
...
**決定的なのは真ん中の二つです** ── ③を stash した木と、③を入れた木で、
どちらも **2381**。つまり③はこの数を動かしていないのに、その前に取った基準値は
2379 でした。**変更のせいに見えて、変更ではありません。**
...
ですが、**`lists measured` の ±2 は説明のつく揺れであって、変更ではありません。**
...
## 長押しでキーを運ぶと、行が十キーを超えます

**実測です。今日の `master` にあります。** `claude/kb6` が②の調査で測りました
（`docs/reports/kb6-drag-2026-08-27.md`）。

qwerty パターンの盤（各行ちょうど十キー）で、行1のキーを長押しで掴んで行0へ
運ぶと ── 半列で数えて:

```
運ぶ前   [20, 20, 20, 20, 20]     全部ちょうど十キー
運んだ後 [22, 18, 20, 20, 20]     行0が十一キー
```

`kbDragTo()` は `row.insertBefore()` するだけで、**行の幅を一度も訊きません。**
訊く関数は在ります ── `kbRoomIn()`。手で足す `kbCellAdd()` はそれを通っていて、
満杯の行には入りません。**運搬だけが通っていません。**

規則19 は「十が電話の数、十一なら29pt」と言ってそれを禁じています。運搬はその
天井の外に居ます。何も throw せず、盤は描かれ、保存され、拡張は十一キーを
一つずつ細くして出します。

**やらない理由:** 直し方が四通りあり（拒む / 入れ替える / 押しのける / 今のまま）、
**どれにするかは手触りの判断でオーナーのものです。** 四つそれぞれで何が起きるかは
上の報告に書いてあります。`kb6` は決めずに測って出しました。

**巻き戻しは効きます**（測りました）。だから今すぐ誰かの盤が失われる類ではあり
ません。
...
**やらない理由:** 「一緒に動く / 掴めない / 今のまま」のどれかは**オーナーの
決定**で、まだ訊かれていません。`master` に `kbVJoin` が入るまでは起きません。

## キーに置く文字を選ぶ画面の「検索」が、まだ入っていません

絞り込みと並べ替えは `claude/kb6` が入れました（隣のアルファベットの章の
`ltSort` / `ltFil` をそのまま呼ぶ形）。**検索は入っていません。**

検索は新しい状態を一つ増やし、その置き場所が
`www/sound.js`（検索は隣の画面にも要るので、そこが一箇所）、
`www/shell.js`（`viewReset()` が忘れる先）、
`www/act-map.js`（`actIn`）── **三つとも kb6 の持ち物ではありませんでした。**

キーボード側だけに置くこともできますが、それは
**「同じ一覧を二か所で見せていて片方にだけ道具がある」を向きを変えて作り直す**
ことになります（この項目の元の指摘そのものです）。

**入れるための差分は測って書いてあります** ──
`docs/reports/kb6-drag-2026-08-27.md` の付録。決まっていないのは二つだけ:
**名前だけで引くのか読みでも引くのか**（`ltCodes()` がその関数）、
**箱を常に出すのか文字数がある数を超えてからか**。どちらもオーナーのものです。


## AI に相談 ── アカウントを持っていない人は、外で止まります

**直せません。知っていることと知らないことは違うので、書いておきます。**

「AIに相談」は ChatGPT を本文の入った状態で開くだけで、うちは鍵も課金も持ち
ません。**開いた先はその人のアカウントです。** 持っていない人が押すと、
ChatGPT のログイン画面に当たって、そこで止まります。

**アプリの中では何も起きません。** `UIApplication.shared.open()` は相手が何を
表示したかを返しませんし、universal link で外のアプリに渡した時点でこちらから
は何も見えません。**押した人が「壊れている」と思っても、こちらには一件も届き
ません。**

**アプリの中で説明はしません**（`CLAUDE.md`「説明を書かない」）。これは何かが
取り上げられた状態でもないので、狭められた例外にも当たりません ── 押せば外に
出ますし、外で何が起きるかは外の話です。

**やらない理由:** 直すには (a) 相手のログイン状態を知る、(b) うちが API 鍵を
持つ、(c) 押す前に注意を書く、のどれかが要ります。(a) は不可能、(b) は
「うちの鍵も課金も要らない」という設計そのものを捨てること、(c) は説明文です。

**もし変えるなら**、それはオーナーの決定です ── 「持っていない人に何を見せるか」
は判断であって、コードから読めることではありません。
...
下書きは投稿の前段で、投稿はサーバに在る。アカウントを消したときサーバの
投稿は消えるが、**この電話の中の下書きはどうなるか**が決まっていない。

CLAUDE.md「自動の削除・剪定・掃除は、書かれた仕様が求めない限り禁止」なので、
**何も決まっていない今は消さないのが既定**。消すと決めるなら DELETE REVIEW。
