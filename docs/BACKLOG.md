...
## `press` が `g2Move` を一度も押していません ── 文法の章

**私のものではありません。** `claude/kb6` が測って見つけたので書いておきます。

`press` が `239/240` と出して、届いていない名前として `g2Move` を挙げます
（`www/grammar.js`、§14 の語順を並べ替えるキー。`a685d69`「語順は、六択から
選ぶのではなく語を動かして決める」で入ったもの）。

**ボタンが壊れているのではありません。** フィクスチャで `g2Three()` を直接
呼ぶと値を返します ── 主語になる語・動詞・二つ目の名詞が揃っているので、
`g2Sent()` が描かれれば chip は出ます。**歩きがその面に届いていない**という
ことです。

**いつから: master に元からあります。** `claude/kb6` が master を取り込んだ
commit `f9788bf` で `press` を回して確かめました ── **その時点で既に**
`11929 (239/240)`、届いていない名前は `g2Move` 一つ。取り込みより後に私が
書いたコードは、この数を一つも動かしていません（11929 のまま）。

**やらない理由:** 直すなら `tools/fixture.mjs` に §14 の面を足すことになり、
それは文法の章の持ち物です。共有のフィクスチャに面を足すと他の検査の数も
動くので、その章のセッションが自分で足すのが筋です。

`press` 自身が「nothing here is claimed about them」と書いているとおり、
これは**赤ではなく、押さえられていない範囲**です。
...
けば？」── 押す前に、外に出ることが暗い字で一行書いてあります（`www/words.js`、
`t('ask.leave')`）。

残りは変わりません: ChatGPT のアカウントを持っていない人は、開いた先でログイン
画面に当たります。`UIApplication.shared.open()` は相手が何を表示したかを返さない
ので、**こちらには一件も届きません。** それは直せませんが、**押す前に外に出ると
分かる**ようになったので、原因の分からない状態ではなくなりました。

この一行は `CLAUDE.md`「アプリ内に説明を書くの禁止」の例外です。オーナーが今日
決めたもので、規則より新しいので勝ちます。最小限、一行だけ。
