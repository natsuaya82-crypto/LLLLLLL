...
**要るのは確かです。**格子は 21×21、390px 幅の画面で描画面は約 342px なので、
**点と点の間は約 17px、点そのものは直径 6px 前後。**指先の当たり判定は
だいたい 44px と言われているもので、**その 2.5 倍の粗さで狙わされている。**
オーナーの「細かすぎて描きにくい」は、測っても同じことを言っています。

**それでも実装を止めているのは、決まっていないことが四つあり、
どれも「あとで直せる小さな差」ではないからです。**

| | 訊きたいこと | なぜ後から変えられないか |
|---|---|---|
| 1 | **ピンチか、ボタンか** | ピンチなら描画面の pointer event を取り合うことになる ── 指二本を「拡大」と読むか「二本目の線」と読むかは、描く操作そのものの意味が変わる。ボタン（＋/−）なら rail に二つ増え、rail はもう十以上ある |
| 2 | **拡大中に描けるのか、拡大は見るだけか** | 見るだけなら、拡大は描画面の CSS transform 一つで済み、格子も点も座標も一切触らない。描けるなら `geAt()`（指→800の座標）と `geDraw()`（800→画面）が両方ズームを通ることになり、**この二つが食い違った瞬間、点は指の下から外れる** |
| 3 | **格子の点も一緒に拡大するのか** | 一緒に拡大＝点の数はそのまま、狙いやすくなるだけ。拡大しない＝見えている範囲に点が増える、つまり**格子が細かくなる**。後者は文字の形そのものが変わる（21×21 は決めて選ばれた数） |
| 4 | **拡大したまま保存したら、次に開いたとき拡大は残るのか** | 残るなら「どこを見ていたか」を letter ごとに覚えることになり、**保存されるものが増える** ── `docs/DATA_MODEL.md` の話になる。残らないなら何も増えない |

**2 が一番大きい。**「見るだけ」なら一日、「描ける」なら `geAt` と `geDraw`
の座標変換を一箇所にまとめる作業が先に要ります（今は二箇所に同じ式が
書いてあり、`pad + v*k` とその逆。ズームが入ると**式が二つある**こと自体が
バグの置き場所になる）。

**私の見立て（訊かれれば）:** ボタン、描ける、格子ごと拡大、拡大は残さない。
ボタンにするのはピンチが「二本目の線」と取り合うからで、これは
`docs/SESSIONS.md` に書いてある「一つの操作に二つの意味を持たせない」と
同じ話です。ただし**これは推測で、オーナーが決めることです。**
