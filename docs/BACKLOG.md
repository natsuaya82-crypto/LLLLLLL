...
## 下書きと録音が、まだ端末にある ── 2026-08-27

「SNSは全部サーバー」 **OWNER**、この日にまた言われた。**また、というのが
この項目の要点**で、仕様が曖昧なのではなく**コードが古いまま読まれている**。

```
lingua.drafts          端末の平の鍵。サーバーへ行かない
Documents/Voices/*.m4a 端末のファイル。投稿の `vo` は名前だけを持つ
```

**どちらもタイムラインのものなので、サーバーに在るべき。** 投稿と同じ扱い。
今は端末にしか無いので、**機種を変えた人は両方失う。**

**そしてこれは今日の作業に影響している。** `claude/del`（アカウント削除）は
「下書きは端末のもの」という前提で作られた ── リーダーがその前提で指示した。
**下書きがサーバーに在るなら、アカウント削除はサーバー側の下書きも消さないと
いけない。** 今の作りはそこを見ていない。

**サイトのプライバシーポリシーも、ここが決まらないと書けない。** 公開する
契約書に「下書きは端末にあります」と書いて数日後に書き換えるのは、前に違う
ことを言っていたという記録が残る。**移してから書くのが順当。**

やること:
- 下書きをサーバーへ（`post` の下書き状態か、別のテーブルか ── 決まっていない）
- 録音をサーバーへ（Storage。写真が既にそうしている）
- `claude/del` のアカウント削除を、サーバー側の両方も消すように
- そのあとで `tokinets.com/lingua/privacy.html` を書く
