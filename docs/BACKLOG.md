...
## Apple と Google で入ると、管理画面がパスワードなしで開く

`adminLocked()`（`www/mod.js`）:

```js
function adminLocked(){ return !ADMIN_OK && netHow()==='email'; }
```

**メールで入った人にしかパスワードを訊きません。**Apple か Google で入った
場合、七回タップだけで管理画面が開きます。書いた人の理屈は「Apple/Google の
アカウントにはパスワードが無いので、訊くと本人が締め出される」で、理屈自体は
正しい ── その二つの道には、照らし合わせるものが存在しません。

**守っているものの大きさは変わりません。**データを守っているのは
`is_admin()` で、サーバが毎回訊きます。七回タップもパスワードも
「サインインしたままの端末を人に渡したとき」のためのもので、そこが片方の道で
効いていない、という話です。

**直していません。**どうするかは決まりごとです ── 端末側の生体認証を使う、
六桁をメールに送る、その二つの道では最初にパスワードを決めさせる、あるいは
このままでよい。オーナーが決めることなので、勝手に選びません。
2026-09-03、オーナーが実機で気づいたもの。
