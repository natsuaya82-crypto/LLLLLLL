...
## まだ送れていない自分の投稿に「いいね」を押すと、何も起きず何も言わない（2026-09-09）

数はサーバーのものになりました（`post_seen`）。まだ上がっていない投稿には
反応する行が無いので、`netMark()` は何もせずに戻ります ── 心は動かず、
言葉も出ません。

**前も何も起きてはいませんでした** ── 端末の中の数が 1 動くだけで、誰にも
届かず、次の pull で消えていました。だから悪くなってはいませんが、
「押したのに何も言わない」は残ります。

言うとすれば「まだ送られていません」で、**それは文言なのでオーナーのもの**
（`docs/FEATURE_RULES.md` § Deciding）。`netWhy()` の今の文は
「接続できません」で、これは接続の話ではありません。
...
札を押して開いた検索の箱だけ `#今日のお題` のままです。

割れない理由は保存です。`snsQ` は箱に見える字であり、検索語であり、そのまま
`snsRecentAdd()` で `recent`（検索履歴、その人のもの）に入ります。見せ方と
保存を割ると、履歴の行に綴りが入るのか表示言語の字が入るのか ── 端末の表示
言語を変えたら履歴はどう見えるのか ── という問いが立ち、それは
`docs/FEATURE_RULES.md` § Deciding です。

検索そのものは正しく動きます：押した札は綴りで検索され、誰の投稿も出ます。

## 投稿画面の宛先を外す手段がありません ── 頼まれていません（2026-09-08）

「Replying to @〇〇」の行が出たあと、それを取り消す道がありません。本文に
入っていた頃は文字なので消せました。今は `PW.toh` で、押せるのは @ だけ
（その人の頁へ飛ぶ）。

外し方は決めごとです ── ✕ を置くのか、その行を押すのか、行を空にして送ると
ふつうの投稿になるのか。`docs/FEATURE_RULES.md` § Deciding。オーナーに訊く
まで作りません。

回り道はあります：＋ から開いた画面を出たまま放っておくと `PW` は残るので、
本文を消して出ても宛先は残ります。`pwBlank()` を通る道 ── 送る、下書きに
する ── を通ると消えます。
...
写しを消すため。「消していい。そもそも増殖させるな」OWNER 2026-09-07、
`docs/CHANGELOG.md` の DELETE REVIEW）。`kbAdd()` が作る板は
`{id, nm:'', pat, lay:kbBlank(...)}` なので、**同じ型で二枚作ってどちらも
一度も触っていない**と、その二枚は id 以外が同じ ── 次に読んだとき一枚に
なります。

写しと、人が押した二回とを、あとから見分けるものは今ありません（増殖した
写しも別々の乱数 id を着ているので、id では分かれない）。中身を一つでも
変えた板は当たりません。

訊くこと: 空の板が二枚あったら、一枚にしてよいか。それとも空の板だけは
残すか。`tools/plan-check.mjs` のプールの主張がこの形に当たって赤くなった
ので、実際に起こり得る形です。
...
（`docs/CHANGELOG.md`）。残っているのは二つで、どちらもオーナーのものです。

**① 規則の一文が、条件を言いません。**「動詞の末尾に -ta」は `at` と `add` ──
規則の画面（`fmrFormHTML`）が書ける全部 ── を言います。**`when` と `drop` を
持つ古い規則**（二つの欄になる前に書かれたもの）は条件つきで働きますが、文は
それを言わないので、いつでも効くように読めます。**嘘をつかない側に倒して**、
その代わり下の表が「実際にどの語に届いたか」を見せています ── 届かない語は表に
出ません。文に条件の節を足すかどうかは、言葉を決めることなのでオーナーのものです。
...
## 否定語の「動詞の前／後」を決める画面がありません

2026-09-06。「文法はさっき言ったルール。…重複はいらない。」（OWNER）で
`STAGES` から 語順・名詞・動詞・否定・疑問・形容詞・場所 の七段を消しました。
そのうち 否定 段だけが `feats:['negp']` を持っていて、**`STG.gpos.negp` を
書く画面はそこだけでした。**

`gRules()` は今も `gPos('negp')` を読み、既定の `after` で語を並べます。値は
言語のもので、書かれたものはそのまま残り、サーバーにも上がります ── 消えたのは
**書く手段**だけです。

同じ `feats` を持っていた他の三つには行き先があります：語順は `g2Sent()` の板、
形容詞は `g2Adj()`、場所は `g2Adp()`。否定形の章（`g2FmChap`）は規則の一覧で、
側を選ぶところがありません。

**どこに置くかはオーナーのものです。**否定形の章に `g2Side('negp', …)` を足すのが
兄弟三つと同じ形ですが、章の中身を増やすのは決定です。`STG.set.adj` などを読んで
いた `stTouched()` も同じ理由で消えました（`stMarkSet()` は残っていて、書く側は
動いています）。
...
## 名詞クラスを消す道がありません ── オーナーに訊くこと

**2026-09-07。**性・名詞クラスの章は、クラスを作れて、名前を書き換えられます
（`docs/CHANGELOG.md`）。**消す道は付けませんでした。**

消せないままなのは、打ち間違いを直せないという意味ではありません ── 名前は
書き換えられます。消せないのは、**消したときにそのクラスに入っている名詞を
どうするのかが決まっていない**からです:

- そのクラスに入っていた名詞は「なし」になるのか
- そのクラスの一致の規則（`STG.fm` の `ncls~<番号>`）はどうなるのか
- 番号は詰めるのか、空けたままにするのか ── 詰めると、他のクラスの
  `ncls~<番号>` と `of` の番号が全部ずれます

`docs/FEATURE_RULES.md` § 決めること は、削除はセッションが決めないと言って
います。**三つとも、その人のデータについての判断です。**
