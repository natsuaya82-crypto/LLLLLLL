...
## ダウンロードした言語を返す道 ── 切り替え画面の行をスライドして消す（2026-09-09、メモ）

OWNER 2026-09-09「言語変更画面をスライドで消せる、メモしといて」。作る日の形は
これ：言語切り替え画面の `mine:false` の行だけがスライドで「削除」を出し、押すと
`language_take` の行を DELETE（サーバーが先）→ 通ってから端末の行と slice を落とす
（`netTakenDown` が「答えに無い行を落とす」のと同じ道）。自分の言語の行は
スライドしない（自分の言語は設定の真ん中の行から）。**DELETE REVIEW が要る。**
