...
**閉じた。** 2026-08-27 にオーナーが出す相手を決めたので、この項が待って
いたものは無くなった。決定は `docs/CHANGELOG.md` に、仕様は
`docs/FEATURES.md` に、列と関数は `supabase/schema.sql` にある。

一つだけ、まだ決まっていないものがここに残る ── **企業でなくなった後、
その人が昔書いた投稿の金の印はどうなるか。** オーナーの決定であって、
コードでは決められない。両側の下調べは
