...
（`r10-kb`・`r10-dl` ほかを取り込んだところ）で赤になります。**`claude/r10-wire`
の変更を一つも入れていない木で測って 2, 2, 2** です ── 人が数える板が一枚では
なく二枚あります。
...
## 起動のバイト数を止めるものがありません（2026-09-09）

`tools/slow-check.mjs` に `MAXB` ができ、`save` のバイト数は上限が持っています
（`docs/CHANGELOG.md` 2026-09-09 の三本）。**起動には無い**、というのが
`MAXB` の上のコメントに書いてある話です ── この check の偽サーバーは、ページが
開いた時点で slice を一つも持っていません。だから「言語を降ろした起動」と
「何も降ろさなかった起動」が同じ重さになり、二度読みが戻ってきても緑のままです。

測ったのは `node tools/measure-cost.mjs`（憶える偽サーバー）で、そちらは
gate に入っていません ── 落ちるものが無いためです。

**塞ぐには**、`slow-check` の偽サーバーが起動の前から言語の slice を持っている
必要があります。`fixture.mjs` が書き出したものを `fakeNet` に渡すのが素直な形
ですが、`fakeNet` はページが動く前に入るので、その受け渡しをどう作るかは
一仕事です。**ここでやらなかったのは、それがこの枝の仕事ではないから**です。

## 圏外で作って一度も上げていない古い言語は、サインインすると誰のものでもありません（2026-09-09、オーナーの判断が要ります）

`language.owner` が答えになった日（`e44d3a65`）から、**印の無い言語は誰のもの
でもありません** ── これは決まっていることです（OWNER 2026-09-02「1アドレス
1アカウント」、09-03「アカウントごとに言語情報も違う」、`acct-check` 10・35）。

**そこに一つだけ、決まっていない形が混ざっています。** 印の仕組みができる前の
版で作られ、一度もサーバーへ上がっていない言語です。索引には `mine:true` と
書いてありますが、印も `sid` もありません。その端末でサインインすると：
...
`acct-check` 35 は「次に入った人のもの」にならないことを押さえていて、それは
**作った本人が戻ってきた場合も同じ**です。だから今の形は決定どおりで、
`gramlang-check` の種はその決定に合わせて印を書きました（この日）。

**残っているのは「では本人はどうやって取り戻すのか」で、それはオーナーのもの。**
選択肢は少なくとも三つ：(a) 何もしない ── 端末に残るが触れない、
(b) サインインした時に一度だけ、印の無い `mine:true` の言語をその人のものに
する（09-03 に禁じた「端末の一人目」に戻ります）、
(c) 画面に一つ道を作る ──「この言語を自分のものにする」を人に押させる。

9-04 以降は保存のたびに上がるので、**当てはまるのはオンラインになって以来
...
写すと、**一台の端末の一つの答えが、その人の全部の言語に配られます** ──
`SET.wsys` は人の設定で、言語ごとの答えを持っていなかったからです。二つ
言語を持っている人には、どちらに配っても間違いになり得ます。

写さないと、前に「音節文字」を選んでいた人の言語は空になり、`wsGuess()` が
答えます（一文字が二音を書いていれば syll、など）。**選び直せば直ります。**

選択肢は三つ: (a) このまま、(b) 開いている言語一つにだけ写す（`lingua.cur` が
どれかを言える）、(c) 何もしないと決める。**どれくらいの人がそうなっているかは
実アカウントでしか測れません。**

## 言語の名前 ── 列とスライスが食い違う言語は、どちらが勝つか決まっていません（2026-09-08）

名前の答えは `language.name` 列一本になりました（`www/core.js` § LNAME）。
2026-09-08 より前に改名された言語は、**列に作った日の名前が入ったまま**で、
改名の結果は `lang` スライスの側にあります ── 改名が列に届いていなかったからです。

起動で降りてくるときに列を**埋める**のは、列が**空**のときだけです
（`netLangsDown`、「fills in and stops」）。**食い違うときは何もしません。**
スライスを勝たせると、今日から先の改名（列にしか書かない）を次の起動が
巻き戻します ── 二つが食い違う理由が「まだ移していない」なのか「今さっき
改名した」なのか、印が無いと区別できません。

これは衝突の解決なのでオーナーのものです（`docs/FEATURE_RULES.md` § Deciding）。
選択肢は三つ：(a) このまま ── 人が一度改名すれば直る、(b) 一度だけスライスを
勝たせる印を `language` に足す（列一本、SQL の流し直しが要る）、(c) 何もしない
と決める。**どれくらいの言語がそうなっているかは実アカウントでしか測れません。**
...
## 星をつけた検索と絞り込みの札は、お題でも綴りのまま ── 頼まれていません（2026-09-09）
...
## ~~「この言語は非公開か」の答えが二か所にある~~ ── 閉じた（2026-09-10）

2026-09-08/09 に「非公開か」は `language.published_at` の列一つになり、
`wldPubOf()`（`www/core.js`）が「自分の／他人の／まだ訊いていない」の三つの
状態を持つ。「無い」と「公開」は同じ枝ではなくなった（CLAUDE.md 規則 22）。
下は当時の記録。


実機 143 で「非公開にしていたのに、ログアウト→ログインで公開に戻っていた」
という報告があり、headless で五本の道を測りました（`claude/r7-acct`）。

| 測った道 | 画面の `wldHidden()` |
|---|---|
| 同じ画面でログアウト→ログイン | 非公開のまま |
| ログアウト→アプリを立ち上げなおす→ドアからログイン | 非公開のまま |
| サインインしたまま立ち上げなおす（メモリも写しも消して） | 非公開のまま |
| 端末を入れ直す→サインイン→もう一度起動 | 非公開のまま |
| **`slice` が降りてこない起動** | **公開と答える** |

サーバーの `language.published_at` はどの道でも `null` のままでした。**戻るのは
...
なおしてログインしたあと、そして**端末の写しも無い状態でサーバーだけが
答えたとき**。三本目は `netLangsDown()` が降ろすスライスから `wld` を外すと
赤になります（見ました）。

**訊くこと。** 二つの答えのどちらが正か。

- `wld.hide` が正なら、「まだ降りてきていない」は公開でも非公開でもない第三の
  状態で、その間スイッチが何を出すかは決めごとです（画面に出るものが増える
  ので `CLAUDE.md` の禁止事項にも触ります）。
- `language.published_at` が正なら、`netLangsDown()` がそれを降ろして
  `wld.hide` を要らなくする形になります ── 一つのことを一つの仕組みで、
  という書き換えです。

どちらもコードを消す話なので、決まるまで触っていません。
## 起動が Google Fonts を待って止まる ── 触っていません（2026-09-08）

`www/index.html:11` に `fonts.googleapis.com` への `<link rel="stylesheet">`
が在ります。**同期で読み込みを止めるもの**なので、DOMContentLoaded がそれを
待ちます。

測りました（`tools/slow-check.mjs` を書いている途中で出てきた）：このコンテナ
は proxy がそのホストを塞ぐので、**12.7 秒**待って諦め、その間アプリの
スクリプトは一行も走りません。起動全体 14.2 秒のうち 12.7 秒がそれでした。
潰したら 2.6 秒。CPU が遅いのではありません（同じ機械で 3 億回のループが
2.2 秒）。

**実機で同じことが起きるかは測っていません。**電波の悪い場所、機内モード、
学校や職場の網の中では、同じ待ちが入る形になっています。「なんか全体的に
遅くない？」（実機 143）の、通信の段数とは別の半分かもしれません。

やらなかった理由：`www/index.html` は別のセッションが持っているファイルで、
一度に一人しか触れません（`docs/SESSIONS.md`）。それと、字の指定を
どうするか（同梱するのか、`--face-*` の後ろの fallback で行くのか）は
規則 17 の「顔は一箇所」に触るので、判断が要ります。

`slow-check` はこの `<link>` に空の CSS を返して測っています ── そうしないと
表の「実測 ms」がアプリではなく第三者を測ることになるので。

## 本物のサーバーへの往復が、この容れ物からは測れません（2026-09-08）

`SB_URL`（Supabase）へは agent proxy が CONNECT を 403 で断ります。だから
`slow-check` の 300ms は**置いた値**で、実測ではありません。段数の比べ方
（4 段 → 2 段）は 300 が何であっても正しいけれど、「実機で何 ms 縮むか」は
この容れ物からは言えません。

DNS は Cloudflare（172.64.x / 104.18.x）を指していて、Supabase の REST は
Cloudflare の前置きの後ろに在るので、どのリージョンかは名前解決からは
分かりません。**知りたければ実機か、外に出られる機械から一本投げるしか
ありません。**


## post-check の下書きの主張が、ときどき赤くなる ── 原因は分かっていません

2026-09-08。`post-check` の 14 章「下書きは録音そのものを localStorage に
入れない」（`/b64/` を `lingua.drafts` の生の文字列に当てる）が、**同じコード
のまま赤くなったり緑になったりします。**

**測った数：probe 無しで 13 回中 2 回赤。** 文言は毎回同じ
（`` `b64` is in lingua.drafts ``）。

**原因は分かっていません。何が一致したのかを出す probe を入れて 14 回
走らせましたが、一度も当たりませんでした。** 立てた仮説は一つで、**測って
外れました**：写真のデータ URL（`data:image/gif;base64,`）が `b64` を含む、
というもの。写真だけの下書きを作って `lingua.drafts` を読むと `b64` は
入っていません。

疑わしいのは、この主張が**部分文字列**を聞いていることです。焼いた写真の
base64 は毎回ちがうバイト列なので、`b64` という三文字がたまたま並ぶことが
あり得ます ── ですがそれは**確かめていない推測**で、probe が当たらなかった
以上、ここに書けるのは「分かっていない」だけです。

**触っていません。** 下書きと録音は `claude/r6-post` の担当ではなく、他人が
書いた主張です。直すなら訊く内容を「`b64` という**欄**があるか」に変える形
（生の文字列ではなく、読み直した中身を歩く）ですが、それは主張の書き直しなので
リーダーの指名が要ります。

## @〇〇 で始めた投稿は、プロフィールの「返信」に出るべきか

2026-09-07。本文が `@jjj ` で始まる投稿は、その人への投稿になりました
（`docs/CHANGELOG.md`）。その投稿は `toh` を持ち、`to`（答えた投稿の id）は
持ちません ── 答えている投稿が無いからです。

プロフィールの「返信」の欄は `pfList()` が `!!p.to` で選んでいるので、この
投稿はそこに出ず、「投稿」の側に出ます。Twitter は @ で始まる投稿を「返信」に
入れます。どちらが正しいかは**この app が何を「返信」と呼ぶか**の話で、
オーナーのものです。

**触っていません。** 直すなら `pfList()` の一行を `postToWho(p)` に変えるだけ
ですが、それは「返信」の意味を変える変更で、検査が緑になるかどうかとは別の
ことです。

## 名詞クラスの画面に、前からあるバグが二つ（触っていません）

**2026-09-09。**クラスを消す道を付けたときにスクショで見えたもので、
**どちらも前からあり、頼まれた三つではないので直していません。**

**① 名前を書き換える画面に、いまの名前が出ません。**`nclsForm()`
（`www/grammar.js`）が

```
lnField('ncls-n', t('g2.ncls.name'), nclsName(i), '')
```

と呼んでいますが、`lnField(id, ph, attrs, val, cls)` の三つ目は **`attrs`**
です（`www/shell.js`）。だから名前は**タグの属性の位置に素で入り**、欄の中身は
空のまま開きます。書き換えようとした人は、毎回まっさらな欄に打ち直しています。
属性の位置なので `esc()` も通っていません。

**② 保存のボタンが `form.save` と出ます。**十言語のどこにもその鍵がなく、
`t()` は見つからない鍵をそのまま返すので、画面に鍵の名前が出ています。
`i18n-check` は「en が定義した鍵を他の九つが答えるか」を見るので、
**en にも無い鍵は緑のまま通ります。**

どちらも `shots/half-a-noun-class-that-exists-ja.png` に写っています。

## 規則の一文の行は、縮まないので長いと画面が横にずれます

**2026-09-09。**規則の一文が入るのは `.stslot .psm` で、`www/index.html:3541`
が `flex:0 0 auto` と言っています ── **縮まないし折り返しません。**だから
ラベルが画面より広いと、行ではなく**ページが広がって画面全体が横スクロール
します。**

測りました。条件を足した文 `y で終わるとき、末尾の 1 文字を落として、動詞の
末尾に -ied` で、スクショの幅が 390 → 461 になりました。

**いま入っている手当ては折り返しです** ── `g2FmSent()`（`www/grammar.js`）が
条件の節と本体の間に `<br>` を入れ、二行にします。両方の行はいまの十言語では
収まります（ja と en で測定、780px のまま）。

**これは手当てであって直しではありません。**節が三文字ぶん長い言語、あるいは
`wend` に長い綴りを書いた人で、また横にずれます。**直すのは
`.stslot .psm` の一行**（縮んで折り返せるようにする）で、`www/index.html` は
このセッションの持ち場ではありませんでした。**元からある性質です** ── 長い
接辞を書けば条件が無くてもずれます。条件が付いたことで、ずれる場面が増えました。

## 文法書の章 ── 一つ、決まっていないことがあります

## 文法書の章 ── 二つ、決まっていないことがあります

2026-09-06。章のページは規則の一文と、辞書の実物で作った表と、例文になりました
（`docs/CHANGELOG.md`）。残っているのは一つで、オーナーのものです。

**表に上限がありません。**規則が効く語を全部並べます。辞書の画面
（`wordsSeen()`）と同じものを歩くので、無料プランなら 100 語が上限、有料なら
辞書の大きさぶんです。文法書としては「全部」が正しい形に見えますが、二千語の
辞書で二千行になります。**何行までにするかは閾値＝判断なので決めていません。**


## オンライン一本化 ── 残っている一行はオーナーのものです

2026-09-04。四歩とも入りました（`docs/CHANGELOG.md`）。**残っているのは
「消すかどうか」だけで、それはオーナーが決めることです。**

**① 古い `lingua.<id>.*` を、いつ読むのをやめるか。**言語のスライスは
メモリになりましたが、**古い鍵はまだ読みます**（`slRd()` が localStorage に
落ちます）。**書きはしません。**アプリを更新した人の言語がその場で空になるのを
防ぐためで、`migrate-check` が二十五行の赤でそれを言いました
──「the language somebody already has did not survive」。

**移行の規則どおりです** ── 写して、読んだものは消さない（`DATA_SAFETY` 規則 2）。
サーバーから降りてきたスライスは `LSL` に入り、そちらが先に訊かれます。

**やめる日はオーナーのものです。**電波の無い起動でやめると、まだ送れていない
言語をその場で失います。**安全な形は「サーバーに上がったことを確かめてから、
その言語のぶんだけ」ですが、それは設計が要ります。**

**② もう iPhone に在るバックアップのファイルを掃除するか。**同じ形です。
`Documents/Languages/` はアプリが触らなくなっただけで、ファイルアプリからは
見えます。人が消せます。

**どちらもやるなら DELETE REVIEW を別に書きます。**

**そして電波が無いときの文言がまだありません。**「電波が無いときはログイン
できない。そう出す」はオーナーの決定ですが、鍵が一つ要ります。`www/i18n/` は
このセッションの持ち場ではありませんでした。
...
二つのことを決めているからで、落ちるものは何もありません ── 気づくのは、
Plus の人が用紙を使えるようになった日です。

**今は分けません。**「4は同じ弾って何の話？」OWNER 2026-09-03 ── 決定では
なくコードの中の名前の話なので、専用の能力は作らない。`file` の段を動かす人が
この一行を読めば足ります。

## 録音と書き出したシートが、アカウント削除で残る
...
つまり「アカウント削除で残るものねえ」OWNER 2026-08-27 が、その二つについては
成り立っていません。残るのは Files アプリから人が消せるファイルで、消して
しまうと他人のものが戻りません ── 小さいほうの誤りを採っています。

閉じるには、録音とシートのファイル名にアカウント（か言語）を入れる必要があり、
それは既にある人のファイルの名前を変える話なので、決定が要ります。
...
## ~~場所を名詞の側に印として付ける言語が、まだ書けません~~ ── 閉じました

**2026-09-07。オーナーが枠を決めました。**「格 ── 主語・目的語・渡す相手 に
加えて 所有（〜の）・場所（〜で／に）・道具（〜で）・共同（〜と）」。助詞の章の
枠が三つから七つになり、`house-LOC` が書けるようになりました
（`docs/CHANGELOG.md` 2026-09-07）。

**「に」という語と「場所の印」の両方を持てるか**も、同じ決定が答えています:
完成の定義の 17 は 場所 の章（前置詞／後置詞の語）を今のまま残すと言い、4 は
場所の印を足すと言っているので、**両方ありえます。**片方だけの言語も、両方
持つ言語もあります。二箇所が同じことを言っているのではありません ── 印は名詞に
付くもの、語は横に立つ別の語で、別のものです。

`FM_INF` に格を足す話（下の項）は**そのまま残っています。**足していません。
...
なる ── 起動した瞬間は言語も文字もキーボードも無い。**ただし本体はサーバー
なので、サインインし直せば `netLangSync()` が全部のスライスを戻します。**
`lingua.sess` もオリジンと一緒に消えるため、**入り直しは要ります**。
`backup.js` が Documents に書いたファイルはオリジンに縛られないので、
サーバーに届かないときの最後の道はそちら ── ただしそれは「バックアップが
勝つ」形の復元で、`docs/DATA_SAFETY.md` に真正面から関わります。
...
## ~~「決めること」の下の二択が、誰も選んでいないのに選ばれて見える~~ ── 閉じた（2026-09-11）

OWNER 2026-09-10「文法の各段は最初は何も置かれてない状態」。`gPosSaid()`
（`www/grammar.js`）が「この言語はこの側に答えたか」を訊く一箇所で、答えて
いなければどちらも光らない。`stTouched()` ではなく**値があるか**を訊く ──
`migrateGramLang()` が電話から写した側はその人が押した答えで、エンジンは
全ての文をそれで組んでいるから。`gramlang` の 8 番が両方向を見ている。

**下段の「決めごと」のほうも、否定については決まった。** 「否定は結構細かく
作れるようにして」OWNER 2026-09-10 ── 否定と疑問は二択ではなく、文を二つ
作って差分から出す規則（接辞・語・語順・組み合わせ、相手ごとに別）になった
（`docs/GRAMMAR-V2-SPEC.md` §4.4/§4.5、`www/grammar.js` § 否定）。
**`adj` と `adp` は二択のまま**で、そこは決まっていない。下は当時の記録。
...
## ~~「黙って課金画面へ飛ばす」が二箇所にあり、オーナーの言葉が両側にある~~ ── 閉じた（2026-09-10）

「黙って課金に飛ばすのなし」OWNER 2026-09-10。コードは既にそうなっている：
`www/post.js` の編集は `popAsk(t('post.editplan'), …)` で訊いてから飛び、
`www/sound.js` に `go('plans')` は一つも無い（`grep -n "go('plans')" www/*.js`
で出るのは全部 `popAsk` の中）。下は当時の記録。
...
危ないのは、タイムラインから他人の言語を読む機能が入った日。**印が外れた
2026-09-05 以降、移行は毎回走ります**が、写す相手は「この端末のディスクに素の
スライス鍵がある言語」だけです（`langUnderSet()`）── 降りてきた他人の言語は
今の版が書くので素の鍵を持たず、届きません。**古い版の時代に降りてきていた
言語だけ**が、他人の言語に自分の語順を刻まれる道に残ります。
...
## ~~`plan-check` が赤い ── 直せるが、直すと文言をアプリが決めることになる~~ ── 閉じた（2026-09-10）

`plan-check` は 44 本の中で緑（2026-09-10、`982ae856`）。編集は `popAsk` で
訊いてから課金の画面へ行き、検査もそれを問う形になっている。下は当時の記録。
...
## 一度もサーバーに上がっていない言語が、どこからも戻せなくなる ── **オーナーの決めごと**

**2026-08-28、`claude/acct2`。リーダーへ上げてあります。決めていません。**

「端末のデータを消す」がバックアップファイルも消すようになると
（OWNER「全部消えるって」）、戻す道はサーバーだけになります。
**サーバーに上がったことのある言語だけです。**

**上がっていない言語は実在しえます。**`netLangSync()` を呼ぶ場所は
アプリ全体で**二つだけ**で、どちらも通らない道があります:

```
  www/boot.js:68     bootSession() の中。netResume() が成功した時だけ走る
  www/onboard.js:1019  obFinish()。オンボーディングを抜けた瞬間、一度きり
```

`langOpen()` も `langNew()` も呼びません。定期的に走るものもありません
（`setInterval` はレコーダーのものだけ）。そして `netLangSync()` は
**開いている言語しか見ません** ── `langKey(kind)` と `netLangRow()` の
`LANGS[langId]` です。
...
```
  ① 電波の無いところだけで使った
     netResume() が失敗 → bootSession() が走らない → 同期しない。
     オンボーディングを抜けた時の一回も、そこが圏外なら空振りする

  ② 二つ目以降の言語
     言語Bを作り、言語Aに戻して、Bを開いたまま起動する日が来なければ、
     Bは一度も上がらない。電波は関係ない
```

**目印は既に保存されています。**`LANGS[id].sid` は、その言語が一度でも
上がったときに `netLangRow()` が付けます（`www/core.js` のコメント
「an entry with no `sid` has never been up」）。**新しく保存するものは要りません。**

**決まりました。何もしません。**OWNER 2026-08-28、四つ並べて訊いた答え:
...
**「全部消える」を字義どおりに取ります。**上がっていない言語を守る条件は
入れません ── sid を見て言うのも、先に同期を回すのも、バックアップを残すのも、
**どれも無い。**押した人が消したものは消えます。

「最初からずっと言ってる」はそのとおりで、オーナーは 2026-08-28 に
「全部」を三度言っています（「アカウント削除はもう全部消えるの。全部。」
「言語データが全部消える」「全部消えるって」）。リーダーが三度訊き直しました。

**この項はもう BACKLOG ではありません。**残してあるのは、②（電波と無関係に
二つ目の言語が上がらない）が**それ自体は本当の穴**で、消すボタンとは別に
直す価値があるからです ── `netLangSync()` を呼ぶ場所がアプリに二つしかなく、
開いている言語しか見ない、という事実は消すボタンが無くても効いています。
**それを直すのは別の仕事で、まだ誰にも渡していません。**
...
その中に `read : mkApprox(word_xx, syl_xx)` と `rdName : "respelling"` が
あります。**`dead-check` はこの二つについて何も言えません。**理由:

- `mkApprox`（`www/core.js:1064`）は十回**呼ばれて**いるので生きている
- `word_en` `syl_en` などは `mkApprox` に**渡されて**いるので名指されている
...
**今日やらない理由 ── そして「消せる」ではありません。**

1. **`read` は消せません。生きています。** `www/settings.js:201` が
   `LANG[k].read.word(S.rom)` を呼んでいて、インターフェース言語を選ぶ画面が
   十行それぞれをその言語自身の読み替えで書いた見本語にしています。**十行が
   十個のラベルではなく十個の読みになっているのはこれです。**このセッションは
   最初「誰も読まなくなった」と報告し、`www/reading.js` のコメントにもそう
   書きました。**間違いでした**。同じ枝の後のコミットで直してあります。
2. **`rdName` は、アプリの中では誰も読んでいません。**読んでいるのは
   `tools/i18n-check.mjs` の二箇所だけ（十言語が持っていることを主張し、
   疑似言語の語彙として覚える）。つまり検査に押さえられてはいるが、画面には
   一度も出ません。**消してよいかは誰も確かめていません。**
3. **一コミット一事。**死んだ鍵を消すことと、十ファイルにまたがるエンジンを
   落とすことと、検査の穴を塞ぐことは、三つとも別の種類です。

**誰かが手をつけるとき、先に確かめること:**

```
  LANG[k].read を読む場所を全部数える
    ── 今日は www/settings.js:201 の一箇所。ここが消えるまで read は生きている
  rdName を画面に出す道が本当に無いか
    ── tools/i18n-check.mjs の二箇所以外に無ければ、消すのは i18n-check も一緒
  dead-check に「返した object のプロパティ」を数えさせるかどうか
    ── 数えさせるなら、i18n の `pos` や `str` の中身まで巻き込まないこと。
       あれは t() が prefix から組み立てて訊くもので、i18n-check の
...
## 既に二行で載ってしまった文字は、二行のまま残ります（2026-09-10、削除の判断が要ります）

その日の直し（`docs/CHANGELOG.md`「どの枠の文字か」を一箇所にする）は、
**これ以上増えないこと**を止めるものです。**既に増えてしまったものは片づけ
ません。**

何が二行かは言えます ── 同じ枠を指す文字が二つあり、片方は `ab` を持たず
（一覧から入れた・書き取りの紙・オンボーディング・音から作った文字）、もう
片方は `ltStart()` が作った空の枠（id が `lt.k` の形）です。測った例では
`lt.k`／`l1`、`lt.t`／`l2`、`lt.qm`／`l4` の三組。
...
`ltSeen()` が三十八までしか見せないので、二行目は隠れます）、(b) 空の枠のほう
だけを取る DELETE REVIEW を書く（`ltJoinSlots()` が既にその形をしていて、
「描いてある方が残る」と決まっています）、(c) 画面に道を作って人に選ばせる。

**オーナーのものです。ここでは訊いていません。**

## 写しの無い端末で保存すると、サーバーの文字が 39 → 42 に増える ── r11 の残り、二つ

2026-09-10、`claude/r11-letters` が「a〜z が消えた・保存できない」の原因を測る
途中で見つけたもの。**150 の直しとは別の道で、147 でも 149 でも同じに出ます。**
測り方と表は `docs/scope/r11-letters.md`（偽サーバーの上で 8 通りの端末状態を
147 と 149 で流したもの）。

1. **写し（`lingua.<id>.got`）が無い端末で保存すると、サーバーの letters が
...
2. **入れ直した端末（147）で、起動のあとサーバーの letters が 42** ── その
   端末に無い言語が上がっている。149 では起きない（39 のまま）ので、150 以降は
   起きないはずだが、**その理由を一行で言える人がいない**（149 で `netLangsWalk`
   に変わったからだと思われるが、測っていない）。

どちらも「保存で何が増えるか」を問う claim が `again-check` にありません。
