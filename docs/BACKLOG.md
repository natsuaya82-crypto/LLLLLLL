...
## 起動の道では、板は id を打たれてからサーバーの行に出会う ── 判断待ち（2026-09-09）

`again-check` 1432「起動してもキーボードは増えない」が **2, 2, 2** で赤のまま
です。ディスクに id 無しの写し、サーバーに 2026-09-07 以前のランダム id を着た
同じ板、という端末で、アップグレード後の最初の起動で一度だけ二枚になります
（そのあとは増えません）。

**測った順（推測ではありません）。** 同じ端末で `kbRead()` → `saveKb()` →
`slMine()` を順に見ました。

| | 板の id |
|---|---|
| ディスクの写し | `(none)` |
| `kbRead()` のあと | `bfzcp3y_1714`（`kbHash`） |
| `saveKb()` のあと `slMine()` が返すもの | `bfzcp3y_1714` |
| `syMerge` の結果 | `bfzcp3y_1714`, `k1788700000_1` |
| `kbIded` のあと | 二枚のまま |

`kbIded()` の移行の組は「**id を持たない板**が、中身の同じ **id 付き**の板に
出会ったとき」だけ働きます。実機の道では出会う前に `saveKb()` が打たれた版を
`LSL` に書くので、`syMerge` に渡る二枚は両方 id を持っています。`saveKb()` を
通らない場合（`langLocked()` が真）は、同じ道で一枚になることを測りました。

**三つの形があり、どれもオーナー／リーダーの判断です。**

- 最初の合流までは保存しない。
- id を打つのを**読むとき**ではなく**保存するとき**にする。
- id を持つ板どうしも中身で突き合わせる ── 2026-09-09 の決定が消したもの。
  「人が二回押した空の板が一枚になる」が戻るので、そのままでは採れません。
