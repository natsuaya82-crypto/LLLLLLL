...
AdMob は入っていた（~~`LinguaAds.swift`~~、2026-09-25 にビルドから外した ── 広告を出す日に戻す）が、Google の同意の画面（User
Messaging Platform）は入れていない ── 土台にした jpel に無いため（リーダー
2026-09-23）。無い間、その地域では個人向けでない広告しか出ない。入れるなら
戻した LinguaAds.swift の `start` の前に UMP の同意を取り、AdMob の管理画面で
...
**二つ目**の引数で、省くと `form` です（`www/shell.js`）。この並びは
`keepKeyOf(r, a)` `go(r, a)` `page(r, v)` と逆で、読む人の罠になります。

揃えるには `form` を名指す四つ ── `www/words.js` `www/wordsheet.js`
`www/import.js` `www/grammar.js` ── を同時に直す必要があり、**`grammar.js` は
別の枝が使用中**なので 2026-09-11 の枝ではできませんでした。名前を変えるだけの
commit で、振る舞いは一つも変わりません。

## 星の行から一件ずつ外す道（2026-09-11、オーナーの判断が要ります）

検索の空欄の下に「保存した検索」が並ぶようになりましたが、その行に ×
（一件ずつ外す）は**付けていません**。「最近の検索」には付いています
（「1件づつ消せるでいいよ」OWNER 2026-09-03 ── あちらは決まっています）。

星を外す道は今のところ**欄の★を押し直す一つだけ**で、そのためには同じ言葉を
もう一度検索しないといけません。一覧から直接外せた方が早いのは確かですが、
削除の道を増やすのはオーナーの判断です（`docs/FEATURE_RULES.md` § Deciding）。
訊くこと：**星の行にも × を付けますか。**
## hunt #6 ── 作った言語に文字が無く、単語を足せない（2026-09-11、持っていない file）

有料の段で ＋ で作った言語は `LETTERS=0` で、これは仕様です
（`ltStart()` の `if(can('letters')) return;` ── 二十八の枠は無料の段のもの）。
打てないのは綴りの欄が**この言語の文字**で出来ているからで、文字が無ければ
変換結果は空、断りは `toast.hw2`「つづりは2文字以上必要です」。三文字打った
人には意味が通りません。

直す所は `www/wordsheet.js`、**何と言うかはオーナーのもの**。測った形は
`docs/scope/r24-lang.md` 六章。

## hunt #9 ── できたばかりのアカウントに「非表示 2」（2026-09-11、持っていない file）

`vLangs()`（`www/home.js`）が、**他人の言語を「取ったが見せられない言語」
として数えています**。三つに分かれるべきところが二つになっている ── 自分の
もの（`language.owner`）、取ったもの（`language_take`）、**どちらでもない
もの**（前のアカウントが残した行 ── どちらにも出さず数えない）。測った形は
`docs/scope/r24-lang.md` 六章。

## hunt #12 ── 「言語を追加」がプロフィールへ飛ぶ（2026-09-11、決めごと）

`langOpen()` の最後が `goTab('profile')` で、切り替えと新規の両方が通ります。
書かれた規則が二つあってぶつかります ──「アカウントが変わるイメージ」
（OWNER 2026-08-25、切り替え）と「保存とか確定は変更して戻る」
（OWNER 2026-09-05、確定）。**オーナーへ。**（送らない半分は直しました。）

## 既に二本に割れてしまったアカウントを、一本に戻す道がありません（2026-09-11）

`claude/r24-lang` が直したのは **これから登録する人** です。今日までに登録した
人のサーバーには、名前の無い行がもう一本あり、**歩きで描いた字はそちらに入って
います**（`docs/CHANGELOG.md` 2026-09-11、`docs/scope/r24-lang.md` に測った形）。
名前が付いているのは中身の空なほうです。

直しませんでした。二本を一本にするのは **どちらを残してどちらを畳むか** の
決めごとで、`docs/DATA_SAFETY.md` の DELETE REVIEW が要ります。人が作ったものが
片方に入っているので、間違えると字が消えます。**オーナーへ。**

見分け方だけ書いておきます: 同じアカウントの `language` が二行あり、片方は
`name` が空で `slice` に `letters` が入っていて、もう片方は `name` があって
`slice` がほとんど無い ── その対がこのバグの跡です。

## 二台目の歩きが作った言語は、そのアカウントの二本目として上がります ── 決まりました（2026-09-11）

端末 B で歩きを通ってから既にあるアカウントでサインインすると、B の歩きが
作った言語がそのアカウントの新しい言語として上がります。**OWNER 2026-09-11
（リーダー経由）: 上げる。人が描いた物は捨てない**（`docs/DATA_SAFETY.md`）。
そのアカウントの二本目として、歩きで付けた名前のまま一覧に出ます。**直す
ことは何もありません** ── 今そうなっているので。この項は「決まったことの
記録」として残します。

**ただし歩きを飛ばした端末**（`obSkipAll`）は、中身の無い名前の無い言語を
上げます。何も描いていないので「人が描いた物」ではありません。`langFirst()`
が起動ごとに誰も頼んでいない言語を一本作るのが元で、**これはまだ決まって
いません**（下の項）。

## `langFirst()` は、誰も頼んでいないのに起動ごとに言語を一本作ります（2026-09-11）

`www/core.js` の一番上、`index.html` が `net.js` を読む前に走るので、
セッションのことは何も訊けません。歩きにはそれが要る（アカウントより先に
物を作る唯一の場所）のですが、**歩きを飛ばした端末や、既にアカウントを
持っている人の二台目でも同じように一本できます**。その言語は中身も名前も
無く、扉を通ると二本目として上がります。

「端末は何も決めない」（`docs/FEATURE_RULES.md`）を最後まで通すなら、
言語が出来るのは**人が作ったとき**（歩き、＋）と**サーバーが「無い」と
答えたとき**（`langForAcct()`）の二つだけになります。`langId` が空の状態を
アプリが通れるかを測ってから。**オーナー／リーダーへ。**

## `netLangRow()` の四つ目の状態は、まだオーナーのものです（2026-09-11）

「誰のものとも言われていない・一度も上がっていない言語は、訊いた人のものとして
上げる」── その関数のコメント自身が「THE FOURTH IS THE ONE THE OWNER HAS TO
DECIDE」と書いています。扉で印が付くようになった（2026-09-11）ので歩きの言語は
もうここを通りませんが、**古い端末に残っている印の無い言語**は通ります。触って
いません。
...
道具の行は投稿の画面の一番下の子で、画面はキーボードの上で終わります
（`ios/App/App/MainViewController.swift` の `keepStill()`、2026-09-25）。
キーボードの高さには**この帯も入る**ので、帯が消えれば行はキーの上に降ります。
**`www/` 側に直すところはありません。**
