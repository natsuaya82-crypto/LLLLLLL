...
**どう防ぐかはオーナーのものです。**思いつく形を並べるだけにします
（どれも実装していません。条件を勝手に足さないこと）:

```
  a. sid の無い言語があるときは、消す前にそう言う
  b. 消す前に netLangSync() を回して、上がりきってから消す
     ── 圏外だと上がらないので、そのとき何をするかがまた決めごと
  c. バックアップファイルは消さない（今日までの形）
  d. 何もしない ── 「全部消える」を字義どおりに取る
```
