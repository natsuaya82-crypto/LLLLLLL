...
## 二台目の歩きが作った言語が、そのアカウントの二本目として上がります（2026-09-11）

端末 B で歩きを通ってから、既にあるアカウントでサインインすると、
`obIn()` が `SET.walked` の偽で `obFinish()` を呼び、B の歩きが作った言語
（描いた字と打った名前つき）がそのアカウントの新しい言語として上がります。
**これは今日もそうで、`claude/r24-lang` では変えていません。**

「二台目の歩きで作ったものを上げるか、捨てるか」は決めごとです ── 捨てる側は
人が描いた字を捨てることになるので、勝手には決められません。**オーナーへ。**
