...
**未解決です。** 一度「押す前の一行」で閉じましたが、**その一行は
2026-08-27 に消えました**（オーナー、実機 #96 を見て「キモいわ消せよ」）。
答えが無くなったので、問いを元の姿に戻します。

**何が起きるか。** 「AIに相談」を押すと ChatGPT がその人の端末で開きます。
アカウントを持っていない人は、**開いた先のログイン画面で止まります。**
そこから先はアプリの外なので、**こちらには一件も返りません** ──
`UIApplication.shared.open()` は相手が何を表示したかを返さないからです。
アプリの側から見ると「押された」で終わっていて、止まった人が居ることは
**測ることもできません。**

**直す道は三つしかなく、三つとも塞がっています:**

| | | |
|---|---|---|
| (a) | 相手のログイン状態を知る | **不可能。** 他のアプリの中は見えません |
| (b) | Lingua が API 鍵を持って自分で訊く | **設計を捨てることになります。** 「アプリは生成せず鍵も持たない。開く先はその人のアカウント」（`docs/FEATURES.md`）。持った瞬間、生成の費用と責任がこちらに移ります |
| (c) | 押す前に、外に出ると説明を書く | **2026-08-27 にオーナーが却下しました。** 一度入れて、実機で見て消させています |

**だから今は何もしません。** ここに書いてあるのは「見つけたが直さない」の
記録であって、次に誰かが (c) を思いつくためのものではありません ──
(c) は試して却下された道です。
