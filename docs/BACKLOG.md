...
## 元が消えた DL 言語の行が、端末の切り替えに残ります（2026-09-09）

OWNER 2026-09-09「dl元が言語を削除したり、アカウントを消してその言語自体が
消えた場合は、dlユーザーからも削除される」。サーバー側はそうなっています
（`language_take` は `language` の削除で cascade、`schema.sql:315`）。**端末
は違います**：索引（`lingua.langs`）の `mine:false` の行は起動の walk
（`netLangsWalk`）では消えません ── walk は「無いものを埋めて止まる」ので、
`language_take` に無くなった言語の行が切り替えに残り、開くと空です。

作るなら：起動で `language_take` の答えが来た時に、索引の `mine:false` の行の
うちその答えに無い sid のものを落とす（slice も `slRm`）。**端末のものを
消すので DELETE REVIEW が要ります**。「答えが来ていない」（null）と「無い」
（空の配列）を同じ枝に入れないこと ── 圏外で開いた起動で取った言語を消して
はいけない。押さえるのは `again-check`（取った言語を再起動で戻す claim の
隣に「元が消えた言語は次の起動で切り替えから消える」を一本）。
