...
`plan-check` が Keychain の側を持つ ── 読み取り失敗で一バイトも書かれないこと、
本当に空なら移行の道は今までどおり動くこと、Keychain が持っていればそれが勝つ
こと。赤を見てある。

**StoreKit の側は誰も持っていない。**Swift は Linux でコンパイルできず、ゲートは
`.swift` を一行も読まない。`writeDown(mayLower:)` を守るのは**人が読むこと**だけ
で、それはこの行が言っている以上のものではない。新しく `writeDown()` を呼ぶ道を
足す時は、そこが上の三つのどれかかを自分で答えること。
