...
- **古い鍵は残します。** `lingua.L….<slice>` と、その `.was`、その下の
  写しは**一バイトも削りません**。読まれるのは写した先で、古い鍵はそのまま
  そこに在ります。
- **動くのは索引の行だけで、それは移動です。** `lingua.langs` の
  `'L…'` の行は `U` の行になります ── 同じ行が、その言語の本当の番号の下へ
  移るということで、行が持っていた欄は一つ残らず一緒に移ります（`sid` の欄
  だけは、その番号そのものになったので消えます）。二つ在ると切り替えに同じ
  言語が二行並ぶので、一つでなければなりません。**人が作ったものは索引には
  入っていません** ── 言語の中身は `lingua.<id>.<slice>` の側で、そちらは
  上のとおり何も消えません。
