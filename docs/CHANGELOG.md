...
### 金の印は「エンタープライズ」に与えるものになりました ── 買えません

> 「公式の印は返事こないから言語検討する人にも入れれるように」
> 「例えばエンタープライズとして、金の印を与えるTwitterの金印と同じ扱い」
> **OWNER 2026-08-27**

**2026-08-25 に色と形はもう決まっていました** ── 青の塗りが「払っている人」、
**金の塗りが「公式」**、形は両方とも同じダイヤ（`MARK_PLUS`）で、色だけで分ける。
入れていなかったのは**出す相手を決めるものが無かったから**で、作者からの返事を
待っていました。

**今日その待ちが終わり、相手が変わりました。** 返事を待つのをやめて、
**エンタープライズ**（企業・団体のアカウント）に与えるものになります。Twitter の
金バッジと同じ扱いで、「言語を検討する人」もここに入ります。

**買えません。運営が与えます。** これはプランではないので **`can()` を一つも
通りません** ── `CAN` に鍵は増えず、値段表の行にも出ません。青の印
（`can('badge')`、Pro から）とは**別の問いに答える別のもの**で、たまたま同じ
形をしているだけです。

**新しく保存するもの:** `profile.gold`（boolean、既定 false）。誰も自分では
立てられません ── `staff` とまったく同じ形で、アプリがログインに使うロールから
この列の UPDATE と INSERT の権限を取り上げてあります。立てられるのは
`gold_add()` / `gold_drop()` という関数だけで、その中で `is_admin()` を訊いて
います。`staff_add()` / `staff_drop()` と一行ずつ同じです。

**`staff` とは別の列です。** `staff` は「通報を読めるか」、`gold` は「企業として
表示されるか」。同じ人であることも、片方だけであることもあります。`staff` の上の
註が「二つ目の問いに答える日が来たら表を作れ」と書いているのは `staff` の話で、
`gold` はその二つ目ではなく**三つ目の、別の表に属する問い**です ── boolean を
一つ足したのは、`is_staff()`（二つの policy・一つの view・四つの関数が訊いて
いる）に一文字も触らないためです。

**消したもの・移したものはありません。** 既存の投稿も、既存の印も、
`can('badge')` の答えも変わりません。

**アプリの中に「これは公式アカウントです」という説明は書いていません。** 印だけです。
