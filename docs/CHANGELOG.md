...
### 2026-09-05 新しく作った言語は、語順を持たずに生まれる

> 「当たり前でしょ」 OWNER 2026-09-05

**人が気づくこと。**今日つくった言語は、語順も三つの位置も**持たずに**始まり
ます。画面は既定の SOV と after を出します ── 前の言語で選んだ語順を着せられる
ことは、もうありません。**前からある言語は何も変わりません。**引っ越し
（2026-08-25）で写された語順はそのまま残り、次の起動でも消えません。

**保存されるもの。**`migrateGramLang()`（`www/phases.js`）が `phases` スライス
に書く相手が変わりました。**「いま持っている言語ぜんぶ」から「引っ越しの前から
この端末にあった言語」へ。**新しく作った言語には `phases` スライスが**作られ
ません**（今までは `{"order":…,"gpos":{…}}` が一つ書かれていました）。誰の
データも消えていません ── `SET.order` と `SET.gpos` は読むだけで、そのまま
残ります（`docs/DATA_SAFETY.md` 規則2）。

**どうやって数えるか。**`migrateGramOld()`。`SET.order` は**古いアプリが**
`lingua.set` に書いた値で、言語を端末のディスクに置いていた頃のものです。だから
その値が語順であるところの言語は、**その同じアプリがここに残していった言語** ──
`lingua.<id>.<slice>` ── です。2026-09-04 以降、そこには何も書かれません
（規則22：`slWr()` はメモリ、サーバーから降りてきたものは `.got`）。**新しい
言語はそこに何も持っていません。**

**印（`SET.gramLang`）は戻していません。**印はディスクに残るのに写し先はメモリ
なので、人が選んだ語順が一度写されて次の起動で永久に消えます。印がついでに
やっていたこと ── 新しい言語を巻き込まない ── が `migrateGramOld()` で、
これは「もう走ったか」を訊きません。**いつ訊いても同じ答えです。**

**確かめたこと。**`gramlang-check` 緑（主張1〜10）。**二つのバグを入れ直して、
別々の主張が赤くなるのを見ました** ── 走査を `LANGS` 全部に戻すと主張5が5本赤、
印を戻すと主張4が2本赤。`migrate-check`・`es5`・`dead-check` 緑。**端末では
確かめていません。**画面の見た目は変わっていません（既定を読む画面は前から
SOV/after を描いています）。
...
**`NET_HEARD` も一緒に消えました。**この段落は最初「残ります」と書かれて
いましたが、実際に消してみると読む人がいませんでした ── `netLangsDown()` の
足でそれが使われていたのは「答えが来た瞬間に描き直す」ためで、描き直しが要る
のは通信の一文が画面に立っていたからです。その一文が無いので、何も作っていない
人の画面は描き直す理由がありません。残せば片端の外れた配線になります。
**保存されるものは何も変わりません。**
