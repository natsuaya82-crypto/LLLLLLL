...
### キーボードの ？ に、その画面が何かを ── **IMPLEMENTED**

オーナー決定 2026-08-28「キーボードも？に説明入れよう」。

この ？ は前から在り、中身は端末に入れる四手順でした。答えていたのは
「どう使うか」だけで、「今見ているものは何か」には何も答えていません ──
**無料の面は打てない QWERTY** なので、最初にすることは押して何も起きないことです。

二行だけ足しました。一行目は両プラン（描いた文字を置いた QWERTY で、それが
そのまま端末に入る）。二行目は**無料だけ**（ここでは打てない）── 有料はこの画面で
押して編集するので、両方に出すと嘘になります。

**保存されるものは何もありません。**新しい文言キーは2つ、10言語すべてに。

見出しは `kb.sys.h`「キーボードの設定方法」のままです。中身が二本立てになったので
合わなくなりましたが、言葉はオーナーのものなので変えていません。
