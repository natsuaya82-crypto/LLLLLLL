...
`save()` は `bkTouch()` で始まり、`backup.js` は `shell.js` より後に読まれます。
古い品詞ラベルを一つでも持っている端末では、`shell.js` がその行で投げて
**その下の定義が全部消えました**。2026-09-03 に呼ぶ場所を `boot.js` へ移して
直しましたが、**次の一件を止めるものは何もありませんでした**。

`tools/order-check.mjs`（新、`npm run order`、ブラウザ不要、1 秒未満）が
持ちます。読み込み時に走る呼び出しを 402 箇所、**三段先まで**追います。
入れた時点で赤は 0 件。`migratePos -> save -> bkTouch` を戻して赤を見ました。

- 保存するもの: 変わりません
- 移行 / 削除 / プラン: 無関係
- ゲート: `FAST` に入りました（検査は入れた同じコミットでゲートに入る）


### 2026-09-03 「今日のお題」が、今日を訊く

「今日のお題はちゃんと今日を聞いてください」OWNER 2026-09-03。

`netDay()` は表を `on_day` の新しい順に一行だけ読んでいました。**新しい行は
今日の行ではありません。**書き手が一日でも書き損ねれば、古い一文が
「今日のお題」として出続け、書き込み画面がその番号に答えを結びつけます。
何も投げず、画面も正しく、日付だけが違う状態でした。

**開くと変わること:**

- 今日の行が無い日は、タイムラインの頭が**ただの書き込み欄に戻ります**。
  昨日の一文が今日の見出しの下に出ることはもうありません。
- 今日の行がある日は、これまでと同じに見えます。

**保存するものは変わりません。**`prompt` 表も `post.prompt` 列もそのままです。

- 移行: 無し
- 削除: 無し
- プラン: 無関係
- **サーバー側**: `supabase/schema.sql` に `day_prompt()` が増えました。
  ダッシュボードに流すまでお題は出ません（流していない現状と同じです）。
  日付の計算は**サーバーの側だけ**にあります ── 端末は時間帯を一つも知りません。
  `feed_slot()` が時間帯を名乗るのと同じ理由で、同じ場所を読んでいます。
- 検査: `rls-check` に三件。うち二件はバグを戻して赤を見ました
