...
中に写真を持たない（線として書かれている）ので、**必ず電話の PDFKit で描く道**を
...
⭐️はサーバーを待ちません（電車の中でも光る）。ただしその言葉はサーバーに
無いので、次の取り直しで消えます ──「サーバーが本体」の値段です。
...
#### 見つけた本物のバグ ── 電話は指一本ずつしか動かしません

最初は「指の間の変化」と「真ん中の移動」を毎回比べていました。**電話は
