...
`buttons pressed` は 11468 → 11633（+165）、名前は 236 → 238。全部測って帰属を
つけました ── 名前は `kbSetNew` と `kbPick` が消えて `kbCellAdd` `kbTapKey`
`kbJoinSel` `kbOpenSel` が入って +2。ボタンは面ごとに:

| 面 | 差 | 中身 |
|---|---|---|
| キーが選ばれている（新設） | +69 ×3 | 面そのもの |
| 二層のキーボード | +5 ×3 | 幅タイル −3、空きマス +8 |
| 列を切って「どこに」 | +2 ×3 | 幅タイル −3、空きマス +5 |
| 編集画面の他の7面 | −3 ×3 | 幅タイル三つ |

`screens built` は 707 → 710 ── 増えた面は1つで、halfDone の面は三つの立場で
歩かれるので3画面。qwerty の板はどの行も十列使い切っているので空きマスが無く、
そこは −3 だけになります。

**`press` はこの枝では赤で、直す場所は私の file ではありません。** タイルが
消えたので `www/index.html` の `.kbnew` `.kbnewl` `.kbnewr` `.kbnewt` が着る者を
失いました（1888・1889・1894・1901・1904行）。その file は他の session のもの
なので触っていません。`docs/reports/kb4-2026-08-27.md` に外す一手を書いてあります。
