...
### 行と列が、その番号・記号を押すと消える — DELETE REVIEW

**何が消えるか。** 行の番号を押すとその行、列の記号を押すとその列が、その面の
配置から消える。消えるのは置かれたキー ── そのキーが何を打つか、四方向のスロット、
幅。**それ以外は何も消えない**: そのキーが指していた文字も、このキーボードの他の面も、
他のキーボードも、単語も、言語も。
...
**確認を出さない。代わりに巻き戻しがある。** 「巻き戻し」「進む」を盤の上に置いた。
...
10 キーに満たない行は、余りを左右に等分して真ん中に置きます。「揃えて欲しい」

    ・・・・・
    　・・・

升目が半キー刻みなので、これは必ず割り切れます ── 5キーの盤に3キーの行なら
左右に1キーずつで、行が列の上にちょうど乗ります。
