...
- **はい** — 下書きに入れて 1 画面戻る。作文欄は空になる
- **いいえ** — 保存せず 1 画面戻る。作文欄も空になる
- **キャンセル** — 何も起きない。書き続けられる

#### 形について

**新しい箱ではありません。** 投稿の「…」を押したときに出るメニューと同じ
`.pmenu` です。角丸も枠も一つも足していない（`box` 緑、baseline 113 のまま）。
下からせり上がるシートでもなく、押したものにぶら下がる形。

新しい規則は `.pmq`（問いの一行）と `.bkw`（矢印の入れ物）の二つだけで、
どちらも枠も角も持ちません。
