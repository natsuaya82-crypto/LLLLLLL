...
**どこに住むかは決まっていません。** 今日はその人の設定（端末側）です。
docs/ は設定を端末のものと書いており、保存した言葉は「SNSは全部サーバー」が
並べているもののどれでもありません。**アカウントのもので新しい端末について
行くべきなら、読み書きは `snsSaved()` と `snsSaveQ()` の二つだけ**です。
オーナー／リーダーの決定待ち。
