...
### 無料の QWERTY の下に ？ ── 打てない鍵盤が何なのか ── **IMPLEMENTED**

オーナー決定 2026-08-28「設定方法だけでいいや」「キーボードの中に入った無料の
qwartyの中に？つけて」。

バーの ？ は前からあり、中身は端末に入れる四手順です ── **そのままにしました**。
一度そこへ「これは何か」を足しましたが、同じ日に戻しています。

代わりに、**QWERTY のすぐ下に ？ を一つ**。無料の面はこの鍵盤に押せるものが
何もないので、最初にすることは押して何も起きないことです。押すと二行 ──
描いた文字を置いた QWERTY でそれがそのまま端末に入ること、ここでは打てないこと。

同じ鍵盤を見せる面は二つあり（無料プランのこの画面と、有料で board 0 を開いた
とき）、**両方に付いています**。中身は `HELP.kbfree` 一つです。

**保存されるものは何もありません。**文言キーは `kb.is` / `kb.is.free` の2つ、
10言語すべてに。`kb.what` と書いて i18n-check に二重だと止められました ──
その名前はキーの種類の見出しのもので、先にあったほうは動かしていません。
