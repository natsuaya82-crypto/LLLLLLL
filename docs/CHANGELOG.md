...
- 用紙の画面で名前を打ったとき、字が**作られる**のか、既にある字から**選ぶ**だけか
- 電子で来た字（線として入る・編集できる）と紙で来た字（形として入る・編集
  できない）を、`write` の中で区別する必要があるか。今のところ、それは字の中身
  （塗った閉じた輪郭か、芯か）を見れば分かるので、欄を増やす理由が見当たりません
