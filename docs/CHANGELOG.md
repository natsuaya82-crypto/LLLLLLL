...
中身の id 無しの板三枚は三枚のまま、`at` も三枚目のまま」「起動しても増えない」
「すでに増えた端末は三度の起動で一枚」。前の三本はバグを入れたまま赤を見ています。

**まだ緑になっていないもの。** `again-check` の「起動してもキーボードは増えない」
（1432 行）は **2, 2, 2** のままです。実機の道では、板がサーバーの行に出会う前に
既に id を打たれているためで、測った順は `kbRead()` が打つ → `saveKb()` がそれを
`LSL` に書く → `netSlice1()` の `slMine()` がその打たれた版を返す、です。
`syMerge` に渡る二枚は**両方 id を持って**いるので、上の組には当たりません。
`saveKb()` を通らない場合（`langLocked()` が真）は同じ道で一枚になることを測って
います。**どう直すかは決まっていません** ── `docs/BACKLOG.md`。
