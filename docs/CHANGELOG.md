...
#### やり残し ── 別の担当のファイルに残っています

このセッションが持っているのは `www/letters.js` `www/sound.js` `www/phases.js`
だけなので、下の三つは触っていません。**どれも「今は無害だが、片付いていない」
の側で、動きは変わりません。**

1. `sndStart()` は**何もしない関数として残っています。**呼んでいるのは
   `www/core.js`（`langOpen`）、`www/boot.js`（2 箇所）、`www/settings.js`
   （2 箇所）の 5 箇所で、どれもこのセッションのファイルではありません。
   関数ごと消すには、その 5 行を一緒に消す必要があります ── 先に消すと、
   起動で落ちて端末が真っ白になります。
2. `www/assist.js` の `AS_CHARS` `asChar()` `asReal()` `asSounds()` は、
   これで**誰も呼ばなくなりました。**`sndStart()` が唯一の読み手でした。
   `npm run dead` はこれを赤にします ── **正しく赤です。**消すのは
   `www/assist.js` を持っている担当です。
3. `tools/migrate-check.mjs` の 4 番目のブロックが
   `want('and sounds to start from', addedSndLen(c.snd) > 0, true)` で、
   **この規則違反そのものを緑で固定しています。**その上のコメントも、音の無い
   言語は全部の文字が何も読まない言語だ、と理由を書いています ── `ltNew()` が
   `snd:[]` を作り、新しい文字が勝手に音を取る道はもう無いので、その理由は
   今のコードに残っていません。この一本は、この変更で**赤になります。**
   `tools/` はこのセッションのファイルではないので直していません。
