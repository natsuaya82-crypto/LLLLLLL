...
### 2026-09-06 プライバシーの一覧表を二つ入れた

`PrivacyInfo.xcprivacy` が本体（`ios/App/App/`）とキーボード拡張
（`ios/App/LinguaKeyboard/`）に入り、それぞれのターゲットの Resources
フェーズに載りました。**保存されるものは何も変わりません** ── 申告であって、
...
「アプリの機能のため」です。拡張は何も集めません ── 外へ出す道が一本も
ありません。
