...
**断りに文が付くのは一箇所だけです。** 一番大きい天井（Pro の三つ）を既に
持っている人が押したときだけ、`langs.full`「言語は最大 3 つです」と一文出ます。
そこは料金の画面が何も答えないので、原因も出口も無い画面になるためです ──
CLAUDE.md の 2026-08-22 の狭められた例外で、凍結画面が唯一の先例。他の段では
一言も出ません。押すと料金の画面へ行くので、それが答えです。
