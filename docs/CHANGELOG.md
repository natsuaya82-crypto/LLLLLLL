...
**`HOLD_SLOP` は 10 を仮に置いています。決めごとなのでオーナーの確認が
要ります。**指がどれだけ彷徨っても「まだ握っている」かはしきい値で、
`docs/FEATURE_RULES.md` § Deciding はしきい値をオーナーのものだと
書いています。動くようにするために置いた数で、値そのものは未確認です。
