...
### 無料の ？ は、キーが動かない理由から答える ── **IMPLEMENTED**

オーナー決定 2026-08-28。無料プランで ？ を押した人が最初に訊いているのは
「なぜ押しても何も起きないのか」なので、そこから答えます。

  無料プランのキーボードは編集ができません。
  自作キーボードを作りたい場合はアップグレードしてください。
  ［アップグレードする］

**無料のときだけ**出ます。端末にキーボードを入れる四手順はその下に残っていて、
順番が変わっただけです ── あれは有料の機能ではなく、無料こそ他に行き先が
ありません。

「＋ キーボードを追加」のポップは「アップグレードが必要です。」に。

**保存されるものは何もありません。**文言は `kb.free.no` `kb.free.up`
`kb.up.go` が新しく3つ、`kb.up.d` は書き換え、10言語すべてに。
