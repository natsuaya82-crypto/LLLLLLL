...
### 2026-09-07 id の無い板につく id を、中身から決まるものにした ── アップデートのたびに増えるのを止める

**保存されるものが変わったこと。** id を持たない板（2026-09-06 より前に作られ
たもの）に `kbIded()` がつける id が、**その板の中身（名前・型・配列）から決ま
る値**になりました。すでに id を持っている板は一切触りません。新しく作る板
（`kbAdd` → `kbId()`）も今までどおりです。
...
**消えたデータはありません。** すでに増えてしまった板は**自動では掃除しません**
── 同じ中身の板を 1 枚にまとめるのも、隠すのも、人が作ったものを消すことです。
**重複は手で消してください。** 増えるのが止まるだけで、減りはしません。

**すでに増えている端末で何が起きるか。** サーバーの写しは前の版のランダムな id
を持っているので、**この直しのあと一度だけ**ディスクの写しの分（多くは 2 枚）が
足されて、そこで止まります。二度目からはディスクの写しの id がサーバーにも在る
ので、突き合って増えません。

**同じ中身の板が二枚あるときの id。** 中身から決まる値なので衝突します。配列の
中で二枚目以降は決まった形で区別する（`kbHash` のあとに順番を足す）ので、二つの
写しの並びが同じなら両側で同じ id になります。**まとめることはしません。**

**押さえているもの。** `kb-check`:「id 無しの写しとサーバーの id 付きの写しを
突き合わせても板は増えない」「二度起動しても増えない」。バグを戻して赤を見てい
ます。
