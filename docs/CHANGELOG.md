...
**数えました**（240文字を入れて `scrollWidth` が `clientWidth` を超えるかを
読む。route と form と半端な状態の顔を歩く）:

```
はじめ   入力欄 <input> 30 / <textarea> 17    横に流れた <input> 30
いま     入力欄 <input> 19 / <textarea> 28    横に流れた <input> 19
```

**残り 19 のうち 8 つはこの枝の持ち物ですが、直していません** ── 打っている
最中に伸ばす道が無い欄で、そのまま変えると横に消えるかわりに下に隠れます。
残り 11 は他のセッションの持ち物です（合言葉の欄 4 つを含む）。
