...
#### 残っているもの ── このセッションの持ち物ではありません

- `tools/ask-check.mjs:28` の主張4の注釈が「**All four of ASK_TO route**」
  と言っています。四つではなくなったので、この一文は嘘になりました。
  検査自体は `Object.keys(ASK_TO)` を歩くので緑のままです
- `www/i18n/*.js` の `ask.to.gemini` `ask.to.grok` `ask.to.perplexity`
  （十言語ぶん）が誰にも訊かれなくなります。**元から** `i18n-check` が
  毎回「三つ宛先不明」と報告していたのがこれで、失格ではありません
- `docs/FEATURES.md:50` が「**相手は設定で選ぶ**は decided (2026-08-27)」と
  書いています。**今回の決定と食い違います。** 古い方を消すのはこの
  セッションの持ち物ではないので、リーダーに上げます
