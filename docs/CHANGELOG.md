...
### 2026-09-09 空の板を二枚作ったら二枚 ── 同じ中身の板を一枚にまとめるのをやめた
...
**消したもの ── `kbIded()` の中身比較。** 2026-09-07 に入れた「id を除いた中身が
同じ板は一枚」（`87a2e2d8`、当日の DELETE REVIEW）を削除しました。まとめるのは
**id が同じ板だけ**です ── それは写しではなく、同じ板が二度書かれているもので、
`syKeyOf()` から見て一行にしかなりません。**中身を見て決めることは、もう何も
ありません。**

**DELETE REVIEW は要りません。** 板の数が減る道が無くなったためです。**逆に、
すでに増えてしまった端末の写しは減らなくなりました** ── まとめはそれを掃除する
ために入っていたので、そこに残ります。掃除できなかったのは、増殖が作った写しと、
人が作った二枚目の空の板とを、あとから見分けるものが無いからです（どちらも同じ
バイト）。**止まっているのは増えることのほうで、それは元を断って止めてあります。**
...
**その元は 2026-09-07 の `95bc95bb` で消えています** ── id が**中身から決まる
値**（`kbHash`）になり、同じ写しは何度読んでも同じ id になりました。増殖はそこで
止まっており、そのあとに足された「中身で一枚にまとめる」は**すでに増えた板を
減らす**ための別の道です。今回消したのはその後者だけで、元は消えたままです。
...
ないので、何度読んでも同じ二つの id になります。ここも中身の比較ではありません。

**押さえているもの。** `kb-check`:「＋を二回押した空の板は、保存して読み直しても
二枚」「id 無しの写し二枚は、毎回同じ二つの id で二枚」「同じ中身の板三枚は三枚の
まま、`at` も三枚目のまま」「起動しても増えない（サーバー 2, 2）」「すでに増えた
端末の枚数は三度の起動で動かない」。前の二本はバグを入れたまま赤を見ています
（1 枚、id が一つだけ）。

**書きません。** id を打つのも、id が同じものを一つにするのも読むときだけで、
ディスクとサーバーへ届くのは次の保存です。
