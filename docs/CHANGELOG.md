...
「返信元／返信スペース／キーボード。ここのスペース空きすぎだからもっと広く
使え」OWNER 2026-09-01。行の欄は**打った文字のぶんだけ**の高さで、余りは下に
落ちて何もしていませんでした ── 一言だけの返信は、一行の文字と手のひら一枚分の
空きガラス。行の欄は `fitin` になり、**空いている分の高さを取って中でスクロール**
します（`lnFit()` は `fitin` の欄に触りません）。

402×874・キーボード336pt で測って:

```
返信元    y  60-138  (78)   x  24-378 (354)
行の欄    y 142-423 (281)   x  76-378 (302)    ← 51 だったもの
意味      y 435-481  (46)   x  76-378 (302)
帯        y 485-538  (53)   x   0-402 (402)
```
