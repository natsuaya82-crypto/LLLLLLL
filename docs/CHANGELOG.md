...
**一括編集で直せるのは品詞と語調（`reg`）の二つだけです。**
どちらも短い一覧から一つ選ぶもので、複数の単語に同じ値を入れて意味が
あります。意味・読み・つづりは一語ごとに違うので載せません。タグは
「入れる」ではなく「足す」で、動詞が違うので今回は載せません。

**一括編集も同じ行で取り消せます。**二十語に一つの品詞を書くのは
二十の答えを捨てることで、失われ方の形は消すのと同じです。同じ
`wUndo`、同じ「元に戻す」の行、同じ寿命。二つの仕組みではありません。
