...
| §7 Morpheme | 枠はある。`rule.form` があれば読まれないので**空のまま** |
| §11 Semantic IR | ある。言語固有情報は入っていない |
| §12 Parser / §13 Generator | ある（`parseSentence` / `fromSemantic`） |
| §14 **Grammar Page の UI** | **一つも作られていない** |
| §9 Sentence Builder | **無い** |
| §1 語順をドラッグで導出 | **無い。六択がそのまま残っている** |
| §3 動詞の活用を人が定義 | **画面はある。エンジンに繋がっていない**（下記） |
...
`www/wordsheet.js` の語形のページ（`vForms` / `fmrFormHTML`）で、品詞・何の形・足す綴り・
前か後か・末尾を何文字落とすか・母音の後だけか、まで指定できます。保存先は `STG.fm`。
