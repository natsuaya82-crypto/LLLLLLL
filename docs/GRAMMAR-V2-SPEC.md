...
| 3 | 代名詞 | `pron` の六つ | ある |
| 4 | 数詞 | `count` | ある |
| 5 | 動詞 | 人称変化（`person` ── `p1s` `p2s` `p3s` `p1p` `p2p` `p3p`）。一頁に六つの形 | **出来た**（`FM_INF` の末尾六つ・`PERSON`） |
| | | 時制（`tense` ── `prs` `pst` `fut` `plp` `prg` `prf`）。一頁に六つの形 | ある |
| | | 命令・条件・可能・〜しなければならない・〜したい（`mood` ── `imp` `cnd` `pot` `obl` `des`）── 命令の否定と疑問はこの頁の中 | **出来た**（`MOOD` が五つ） |
| | | 受け身・使役（`voice` ── `pas` `cau`） | ある |
| | | 否定（`neg:v`）・疑問（`q:v`）── 疑問詞の位置は語順の板 | **出来た**（`STG.gr` の Rule ── 接辞／語／語順／組み合わせ、否定する相手ごとに一本。`gPolPut()`） |
| 6 | 形容詞と副詞 | 形容詞の側（`adj`） | ある |
| | | 比較（`degree` ── `cmp` `sup`）。一頁に二つの形 | **出来た**（`DEGREE`・`than` の語と側・語順の `STD`） |
...
## 節の単位は「形」ではなく「節」 ── オーナー決定 2026-09-11

「開いたらそんな分け方してるの意味わからない。**人称でまとめて設定できれば
いいやん**」

動詞の章を開くと二十一の扉が並んでいた ── 一人称単数、二人称単数、三人称単数…
それぞれが一頁で、「主語によって動詞がどう変わるか」を書きたい人は六つ歩いて
同じ種類の規則を六回書くことになる。**節が頁の単位になった。**人称変化は
**一頁**で、その中に六つの形が見出しとして並ぶ。

表は `G2FM_CHAPS`（`www/grammar.js`）── 節の id、名前の鍵、品詞、形の並び。
`G2BOOK` の `has` は節 id の並びだけになり、**群の見出しは消えた**：見出しが
節そのものになったから。一つの考えを「扉の上の見出し」と「扉の先の頁」の二段に
書いていたのをやめた、というのがこの変更の全部。

**形が一つの節も特別扱いしない。**複数形も比較も人称変化と同じ関数が描く。
違うのは表の一行だけ。

**規則が一つも無い形は「作成」の行だけで、見出しは付けない** ── 行が既に形の
名前を書いているので、その上の見出しは名前を二度書いているだけ。規則がある形は
見出しを持ち、＋ はその見出しに乗る。

**行の名は学校文法の用語。**「日本語はちゃんとしてくれ。主語で変わる形なんか
聞いたことない」 OWNER 2026-09-11 ── 「私の形」「君たちの形」は
一人称単数・二人称複数に、`obl` は「〜しなければならない」、`des` は「〜したい」
になった。他の九言語も同じ意味。説明（`.d`／`.e`）はそのまま。
