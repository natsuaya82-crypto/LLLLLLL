...
| | 章 | 状態 |
|---|---|---|
| 4 | 格 ── 主語・目的語・渡す相手 に **所有・場所・道具・共同** を足す | **出来た**（`part` の枠が七つ・`GCASE`） |
| 5 | 複数 | ある |
| 6 | **性・名詞クラス** ── 無し／2 つ／3 つ…、名前は自由、語ごとにどれか、一致 | **出来た**（`STG.ncls`・`CLASS` の rule）。**消す道は無い**（下） |
| 7 | **冠詞・指示詞** ── a／the／this／that に当たる語と位置 | **出来た**（`CHAP_SLOTS.det`・位置は §2 の板） |
| 8 | 代名詞 | ある |
| 9 | 数詞 | ある |
...
| | 章 | 状態 |
|---|---|---|
| 10 | **人称・数の変化** ── 私／君／彼・彼女／私たち／君たち／彼ら | **出来た**（`FM_INF` の末尾六つ・`PERSON`） |
...
| | 章 | 状態 |
|---|---|---|
| 16 | **形容詞の比較** ── 比較・最上級の規則（fmr の形）と位置 | **出来た**（`DEGREE`・`than` の語と側・語順の `STD`） |
...
- **コピュラの位置**は §1 の語順の板（`CMP` の札）。§15 が持つのは語だけ
- **比べられる句が文のどこに立つか**は §1 の板（`STD` の札）。§16 が持つのは
  「より」の語とその側だけで、形容詞そのものの側は §2 と 形容詞 の章のまま
