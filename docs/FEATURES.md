...
| Reading a downloaded language | **shipped 2026-09-02**, **not device confirmed** — the row in the language switcher is a button and `langOpen()` takes it; `langLocked()` in `core.js` is what every saver asks, so nothing writes to it. The ↓ itself stays on the article and does not switch (`wldGet`, `www/home.js`). **Not held by any check and not written anywhere until 2026-09-09: the slices are in memory (rule 22) and `netLangsDown()` pulls only `owner=eq.<me>`, so after the app is closed a downloaded language is a row in the switcher with nothing in it until its article's ↓ is pressed again** | — | — | nothing new | decided — 「ダウンロード言語にしようよ。編集不可でそのアカウントに切り替えたらダウンロードした人の言語が使える」 OWNER 2026-09-02. Whether ↓ switches at once, and whether a taken language comes back down on launch, are the owner's (asked 2026-09-09) |
