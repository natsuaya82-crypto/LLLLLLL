...
| Writing system | shipped | alphabet only | `wsys`: syllabary, abjad, abugida, logography | the `language.wsys` column (`langWsysOf()`), slice `script` | decided |
...
| Font file out of the app (.otf, the share sheet) -- the share mark at the top right of the letters screen | code confirmed (r95; moved to the letters r96), not on a device | — | `font`: Plus | none — a file in the app's tmp folder, handed to iOS (`ltFontOut()`) | decided — 2026-09-25 |
