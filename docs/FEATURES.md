...
| The blue mark beside a name | shipped | no | `badge`: **Pro** | none — asked of `can()` now, never frozen | decided — 「バッチはplusから」 (2026-08-23), moved to Pro with the rename. It says what is true NOW, so it comes off when somebody stops paying 「バッジは消える」, and `postBadge()` therefore answers it for `p.mine` only |
| **The gold mark — エンタープライズ** | **planned** — **OWNER DECISION 2026-08-27**, 未実装 | no | **not a plan** | `profile.gold`, boolean, set by `gold_add()`/`gold_drop()` behind `is_admin()` | decided — 「例えばエンタープライズとして、金の印を与えるTwitterの金印と同じ扱い」. **It cannot be bought and goes through no `can()`**; the operator gives it, the same way `staff` is given. Same shape as the blue one (`MARK_PLUS`), **colour alone tells them apart**. 「言語検討する人」are inside it. **Still open:** what happens to the mark on posts somebody wrote while they were an enterprise, after they stop being one — `docs/reports/badge-past-2026-08-27.md` |
