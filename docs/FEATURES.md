...
| Font file out of the app (.otf, the share sheet) -- the share mark at the top right of the letters screen opens the export screen (`vLtOut()`), and its フォント row is the export | code confirmed (r95; moved to the letters r96), not on a device | — | `font`: Plus | none — a file in the app's tmp folder, handed to iOS (`ltFontOut()`) | decided — 2026-09-25 |
...
| Letters put together in one square (block, like Hangul) -- the sixth writing system; a syllable's onset, vowel and final drawn into parts of the square (`wsStrokes()`, `wsParts()`), each vowel cut side by side, one over the other, or in four like 田 (the vowel's own letter page → 置き方); four parts at most | code confirmed (r102), not on a device | — | `wsys`, as the other five | the `language.wsys` column (`block`); `SCRIPT.blk` in slice `script` | decided — 2026-09-26 |
