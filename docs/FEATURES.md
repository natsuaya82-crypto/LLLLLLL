...
| A long post folds in the timeline | shipped 2026-09-15, **the height is NOT decided** | yes | — | none new | 「もっと読むで開くTwitterと同じ方式で頼む。」 OWNER 2026-09-15. Folded at `POST_FOLD` lines with 「もっと読む」 under it, which opens the post's thread — the road the row already carries, not a second one. **Whether a row is cut is MEASURED** (`postFolds()`), never counted in characters: 「3 lines」 is a different number of characters on every phone. A column (ttb) is never folded. **The number of lines is the owner's and is still open** — `shots/r39-fold-3/5/8-ja.png` go to them; 5 is what is in the build. Held by `post-check` 28 |
