...
- **スクリーンショット**: 3〜10 枚。サイズと作り方は下の「スクリーンショット」
...
### スクリーンショット（`node tools/storeshot.mjs`）

**サイズは 1242 × 2688。** 6.9 インチ枠の 1320×2868 は**弾かれます** ──
推測ではなく、`natsuaya82-crypto/jjjj` が実際に弾かれた値で、あちらが通した
7枚はすべて 1242×2688 です。App Store Connect が受け取るのは 1242×2688 /
2688×1242 / 1284×2778 / 2778×1284。**この数字はこの repo の外の事実なので、
弾かれたらその場で書き換えること。**

このアプリには都合よく、390 × 844 を 3.1848 倍すると 1242 × 2688 ちょうどに
なります。

**国ごとに別の組を持てます。** product ページのローカライズごとに、
スクリーンショットも App プレビューも別の組が入る ── ビルドは1つのまま、
日本の人とフランスの人に違う絵を見せられます。`--lang` が**両方**を動かす
（板の上の文字は `tools/pv/store-copy.mjs`、アプリ自身の言語は `www/i18n/`）。

```bash
node tools/storeshot.mjs              # 英語 6枚  pv/store/01..06.png
node tools/storeshot.mjs --lang ja    # 日本語 6枚 ..-ja.png
```

**文字が書かれていない言語では止まります。** 英語の板の下に日本語のアプリ、
のような半分だけの組は作りません。**文面はオーナーのもの**なので、増やすときは
`store-copy.mjs` に書き足すこと。

**見出しは全部2行にそろえること**（jjjj の store/README.md が踏んだ穴）。
1枚だけ1行だと、そこだけ文字と端末の間が空いて、横に並べたときに段差が出ます。

---

## 5b. App プレビュー（product ページの動画）

スクショの前に並んで**自動再生される動画**。インスタの1枚目が動画なのと同じ
位置で、検索結果の中でも動く。ここが空だと、最初に目に入るのは静止画になる。

**Apple が決めていること**（App Store Connect が弾くので、迷ったら上げてみる）

- 各端末サイズ・各言語につき **最大3本**
- 1本 **15〜30秒**。29秒台で作ること（30.42秒を上げようとして気づいた）
- **音は消えた状態で流れる**。だから字幕が本体で、声と音楽はおまけ
- **ポスターフレーム**（止まっているときに出る1コマ）を自分で選ぶ
- サイズは端末サイズごとに決まっている。**受け取る値は App Store Connect が
  出すので、弾かれたらその数字で撮り直す** ── `tools/pv.mjs` は `--w` `--h` で
  どのサイズでも出せる

**Apple のガイドラインで気をつけるところ**

「端末で撮った映像であること」を求めている。文字・音楽・ナレーション・場面の
つなぎを足すのは認められているが、**アプリの外の映像で埋めるのは駄目**。

`tools/pv.mjs` が作るものは、**本物の `index.html` を実機と同じ 390×844 で
動かして撮っている**ので中身はアプリそのものだが、**実機で録画したものでは
ない**。技術的に見分けはつかないはずだが、規約はそう書いてある。落ちたら実機
（またはシミュレータ）の画面収録で同じ流れを撮り直すのが確実な道。ここは
**審査に出してみるまで分からない**ので、落ちたときの手が要る。

**どちらの動画を上げるか**

- **縦（9:16、28.7秒）が App プレビュー用**。画面いっぱいで、字幕が下に大きく、
  音を消しても意味が通る
- **横（16:9、57.5秒）は Store 用ではない**。端末を横に置いて左に文章を出す
  作りなので、上の「アプリの外の映像で埋めない」に引っかかる。こちらは SNS・
  web・説明用
