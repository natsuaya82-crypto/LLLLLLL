...
- **App のプライバシー**（データ収集の申告）── **下の 5b にそのまま押す順で
  書きました。**ここに二行だけ書いてあった頃のものは実物より短く、
  申告が実物と違うアプリは審査で止まるか、出たあとで下げられます
...
## 5b. App のプライバシー（データ収集の申告）── 押す順

App Store Connect → Lingua → **App のプライバシー** → 「編集」。
Apple の区分を一つずつ選んでいく画面です。**下の八つにチェックを入れ、
それ以外には入れない。**
...
| # | 区分 | 何のことか | どこに在るか |
|---|---|---|---|
| 1 | 連絡先情報 → **メールアドレス** | サインインに使うアドレス | Supabase の `auth.users` |
| 2 | 連絡先情報 → **氏名** | 画面に出る名前。本人が自分で打つ自由な文字列で、本名を入れる人がいます | `profile.display` |
| 3 | 識別子 → **ユーザー ID** | ハンドル（`@`のあと）と、アカウントの uuid | `profile.handle`、全部の表の uuid |
| 4 | ユーザーコンテンツ → **写真またはビデオ** | 投稿に付ける写真、一つの投稿に最大 4 枚 | `post-media` バケット |
| 5 | ユーザーコンテンツ → **オーディオデータ** | 投稿に付ける声、30 秒 | `post-media` バケットの `vo.m4a` |
| 6 | ユーザーコンテンツ → **その他のユーザーコンテンツ** | 投稿の本文、下書き、自己紹介、プロフィールの顔、作った言語そのもの、通報に添える一言 | `post` `draft` `profile.bio` `profile.av` `slice` `report.note` |
| 7 | **検索履歴** | 打った言葉の直近 5 件と、★を付けた言葉 | `recent_search` `saved_search` |
| 8 | 購入 → **購入履歴** | このアカウントがどの段を買ったか | `plan` |

**入れないもの。**位置情報、連絡先（アドレス帳）、健康、金融情報、
ブラウズ履歴、診断、広告データ。**位置情報は一件もありません** ──
`www/` と `ios/` に `geolocation` も `CoreLocation` も 0 件で、あるのは
「使わない」と書いたコメント一行だけです。カード番号は Apple が持つので
こちらには来ません。

**7 が入っていなかったのが一番危ない所でした。**検索履歴は Apple が別枠で
訊く区分で、`recent_search` は 2026-09-03 に入ったばかりです。

### この節に出てこないもの ── `PrivacyInfo.xcprivacy`

**同じ「プライバシー」でも別のものです。押すところはありません。**
上の表は「何を集めているか」で、答えるのはこの画面。もう一つ、Apple は
「アプリの中で使っている一部の API について、なぜ使うのか」を**アプリの中の
ファイル**で訊きます。それが `ios/App/App/PrivacyInfo.xcprivacy` で、
**リポジトリに入れました（2026-09-04）。**

これが無いと、**アップロードは成功してワークフローも緑になり、一時間後に
メールで断られます**（ITMS-91053）。build 86 の ITMS-90158 と同じ、
赤い印にならない唯一の落ち方です。

`npm run assets` が両方向で押さえています ── 触っているのに書いていない、
書いてあるのに誰も触っていない、そして**ファイルが Xcode の Resources に
入っていない**。三つ目は置いただけでは入らないという意味で、Archive も
Export も Upload も全部緑のまま落ちます。
