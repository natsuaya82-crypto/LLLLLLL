...
1. **基準の国**。主な客が日本なら日本を基準にすると、円で切りのいい数字を
   自分で決められます（USD 基準だと円は Apple が丸めた数字になります）。
2. 個別に上書きしたい国があるか。普通はありません。
...
**足りないのは繋ぎです。** いまの「プラン」画面のボタンは `SET.plan` を
書き換えるだけ。お金は 1 円も動かず、誰でも押せます。
...
4. **「購入を復元」ボタン**（**必須**。ないと落ちます）。Swift 側の
