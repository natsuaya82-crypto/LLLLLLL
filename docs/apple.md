...
（`ios/App/App/LinguaStore.swift` の `ids`）。無い商品を聞いても RevenueCat
（その下の App Store）はそれを返さないだけなので、**先に作った分から順に売り物として出てきます**。
...
`localizedPriceString`（RevenueCat が App Store から受け取る値段の文字列）—
その人の通貨で、その地域の書式で、Apple が組み立てた文字列です。年額が何％安いかも、Apple が返した数値から計算します。**Apple は国ごとに
...
`restore` / `current` / `manage` があります。**買う・復元・後から届く更新
（親の承認など）は RevenueCat の SDK を通ります**（Shipaton 2026、2026-09-25）
── finish も、アプリを閉じている間に届く更新を聞くのも RevenueCat です。
サーバーへ送るのは今までどおり Apple が署名した取引で、署名が端末で通らない
取引も**わざと**送ります（通るかを決めるのは `verify-plan`）。プラグインは使っていません（このアプリは `@capacitor/core` を
...
**レシートの検証は出来ています（2026-09-06）。段を決めているのはサーバーです。**
「だから端末でやるわけねえだろ」OWNER 2026-09-03。

端末が送るのは **Apple が署名した取引**（`jwsRepresentation`）で、
`supabase/functions/verify-plan` が x5c の鎖を Apple の根まで辿って署名を見て、
`appAccountToken` がそのアカウントかを見て、`purchase` 表に束縛し、その uid に
ついて検証できた取引全部から段を決め、service role で `plan` に書きます。
`plan` も `purchase` も API からは**読むだけ**なので、改造した端末が自分を Pro
だと言う先がありません。

**そして購入は、買った時にサインインしていた Lingua のアカウントのものです。**
「アカウントごとなんだから、違うアカウントで復元できるのおかしいだろ。検証して」
OWNER 2026-09-06。別のアカウントで「購入を復元」を押しても付きません。

**そちらでやることが一つ増えています** ── `supabase/setup.md` § verify-plan：
函数の deploy と、環境変数 `APPLE_ROOT_CA_G3`（Apple の公開ルート証明書。
repo には置いていません）。
...
- **商品 4 つは App Store Connect に在ります**（OWNER 2026-09-06「2あるやんけ」）。
  RevenueCat の公開キーが入れば値段を返せるので、画面が打ち込みの数字に落ちる
  ことはありません。**キーが空の間は、値段・購入・復元が「App Store に
  つながりませんでした」になります**（既に払っている人の段は落ちません）。

コードとして残っている作業は、公開キーの一行だけです（§ 6b）。**実機で一度も走っていない**のが
残っているものです ── 買う／同じアカウントで復元して付く／別のアカウントで
復元して付かない、の三つはサンドボックスでしか答えが出ません。

**そちらでやることは 4 節（商品 4 つ）と、6b 節（RevenueCat）と、
`supabase/setup.md` § verify-plan（函数の deploy と `APPLE_ROOT_CA_G3`）です。** 作った商品から順に
...
## 9. 広告 ── 出さない（2026-09-25）

Lingua は広告を出さず、誰も追跡しません（`docs/FEATURE_RULES.md` 2026-09-25「広告は出さない」）。
アプリに広告の SDK は無く、「追跡を許可しますか」も訊きません。§ 5 のとおり
`PrivacyInfo.xcprivacy` は追跡 false で、`tools/assets-check.mjs` が
「出荷する物のどこにも広告・追跡の SDK の名前が無い」ことと「三枚とも追跡 false」を見ています。

オーナーがやること:

1. App Store Connect →「App のプライバシー」: **「トラッキングに使用」は「いいえ」**。AdMob のために
   足した項目（デバイス ID・広告データ・製品の操作・おおよその場所・診断）があれば外し、§ 5 の
   「集めるもの」だけにする。
2. GitHub の Secrets の ADMOB_APP_ID・ADMOB_NATIVE_UNIT はもう読まれません（消しても、残しても同じ）。
3. app-ads.txt は要りません。
