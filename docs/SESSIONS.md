...
```
  ...
  netDay が今日を訊かないのはバグ    その日は違った。理由つきの決定だった（下）
```

**四件とも「コミット本文か報告を、コードの現状だと信じた」。**

**四件目はその後ひっくり返りました。**2026-09-03、オーナーが
「今日のお題はちゃんと今日を聞いてください」と言い、`netDay()` は
`day_prompt()` を訊くようになりました。**学びは変わりません** ── あの日の
リーダーは、理由が書いてある決定を読まずにバグと呼びました。読んでから
オーナーに出していれば、一日で済んだ話です。
