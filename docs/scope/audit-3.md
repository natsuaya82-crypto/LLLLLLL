...
## 追記（2026-09-03、リーダーの指示で決定ログを直した後）

**直したもの** ── `docs/FEATURE_RULES.md` の 11 件（72 74 78 79 80 81 84 85 86
88 89）。見出しの下に superseded の印と何が置き換えたか、古くなった段落は削除、
実装状況はいまの事実に。コードも検査も一行も触っていません。

**直さなかったもの、その一** ── 61 と 70 の実装状況の行（§ A）。どちらも事実として
古い（「Basic は売っていない」「クラウドは未着手」）のですが、リーダーが名指しした
11 件に入っていません。一行ずつで直せます。**やってよいか訊きます。**

**直さなかったもの、その二** ── § A-2（`www/core.js:1226`）と § H-2
（`www/post.js:2947`）のコードの注、§ G の `openPick` の横並びチップと `.sheet` の
残骸。「`docs/FEATURE_RULES.md` 以外は一行も触らない」の内側です。72 の**不明**も
埋めていません ── 2026-08-26 の原文が無いままなので、決定ログにも「決まって
いない」と書いて残しました。

**言っておくこと** ── この直し方は、決定 59 と 77 が自分について書いていること
（「上の決定は書かれたまま残す。今日に合わせて編集された記録は記録ではない」）と
真正面からぶつかります。私は本文の古い段落を消しました。リーダーの指示が
CLAUDE.md の「歴史とかいいから消せよ」を根拠にしていて、そちらが新しいからです。
**ぶつかっている事実は消えないので、ここに書いておきます。**元の文面は
`36e65562` にあります。
