...
終わったら最後のメッセージを「r64-card 完了」で始め、規則 8 の形で報告する。
オーナーには話しかけない（リーダーが伝える）。

── 並行セッションの規則 ──────────────────────────────
あなたの領域: カードは投稿の絵（r63 §2-2 CD1〜CD5 と C）
あなたのブランチ: claude/r64-card
あなたが持つファイル:
  www/card.js tools/card-check.mjs tools/fixture.mjs（要る面だけ）
...
── r64 の担当 ──────────────────────────────
r63 §2-2 の CD1〜CD5 と card.js の C。CLAUDE.md ルール 8・12 が仕様。
- CD1 ink の無い他人の投稿を自分の辞書と字で描く。ルール 12 の「まだ起きない」は今は偽（r63 が測った）。
  他人の投稿は投稿から描く ── ink が無ければ文字（ルール 12「描けない ink は投稿の text」と同じ答え）。
  自分の古い投稿を辞書から描く道（migratePostInk 前）を残すかは、ルール 12 が「意図的」と書いている範囲
  （自分の投稿だけ）に閉じる。**card-check が緑のままだったのは試験が `qq ww`（どの辞書にも無い）だったから**
  ── 代理の試験を、実際に辞書にある語の ink 無し他人投稿に書き直す。
- CD2 `cardUnit()` が `l.st` しか読まない ── 形の答えは `inkGeo()`（glyph.js）一か所に訊く。
- CD3 空白と改行の二つ目の答え ── `postRuns()`（post.js）を読む（post.js は r60 の物なので**呼ぶだけ**、書き換えない）。
- CD4 カードの意味 `po.mn` 対 タイムラインの `postSay(p)` ── 一つの答えに。
- CD5 消えた投稿のカードが自分の一番新しい単語になる ── 無い物は無いと描く（viewGone と同じ考え）。
見た目が変わるので、カードの前後を撮る。