...
**2026-09-03 の追加決定**（`docs/FEATURE_RULES.md`、`9cc255d`）を受けて、
四冊から**置き換えられた記述を消しました。**印を付けて残すのも残したことに
なるので、私が最初に書いた「ここに七つの表がありました、消しました」
「三つは直っていたので消しました」という**除去の跡そのものも消しました。**

消したものの形:

- **取り消し線 14 箇所**（`~~…~~ Done`）── 済んだ項目は消し、
  まだ開いているものだけを残しました
- **「以前はこう書いてあった」の類**。`docs/STATE.md` §1 の
  「この段落は三度そう言って二度間違えた」、「66 コミットのあいだ
  古い sha を載せていた」、「2026-08-22 にあるセッションが二つのゼロを信じて
  208 コミット遅れの写しを一日いじった」など ── **残っているのは、
  そこから出てきた規則だけ**です（`--all` で取る、`git branch -r` は嘘を
  つきうる、二つのゼロは master が今のアプリであることを証明しない）
- **日付付きの出来事**。§4b の「2026-08-25 に三つのセッションがあった」と
  その日のファイル分担表、§7 の「Landed on 2026-08-25」「Corrected on
  2026-08-22」「Landed on 2026-08-22」── 残したのは
  **ファイルで割る**という規則と、一人一ファイルの原則だけ
- **直った罠の説明**。`docs/keyboard.md` の「押した瞬間に入る、という例外は
  もうありません」も消しました ── いまの習わし（押して選ぶ）だけが残って
  います

**残した「なぜ」は、今日の判断に要るものだけ**です。たとえば
「`Capacitor.Plugins` は使えない、bundler が無いから」は理由ですが、
これを消すと次の人がまた `registerPlugin` を書きます。
「2026-08-22 にゲートが十四本だった」は消しました ── 今日の判断に要りません。

`docs/CHANGELOG.md` は一行も触っていません。
...
**私は一つも決めていません。**下は全部、コードを読んでも数えても動かしても
答えが出ないものです。調べれば分かるものは混ぜていません ── それは本文の
1〜68 で確かめました。

### 食い違い ── どちらが正しいか私は決めません

```
Q1. キーボード拡張は、今のコード一式でビルドが通っているか
    A: 通っている ── docs/STATE.md § 2「It is built, it is on TestFlight,
       and a person has typed their own letters on it on a real phone」
    B: 通っていない ── docs/keyboard-extension.md が五箇所で
       「#41 以降ビルドが一度も回っていない／まだ一度もコンパイルされて
       いません」(2026-08 に書かれたもの)
    コードは今: 六つの Swift が project.pbxproj の Sources に入っています。
       ビルドが回ったかどうかは GitHub Actions の履歴で、私からは読めません
    訊きたいこと: 変換の入った今の拡張(Compose.swift と CandidateBar.swift を
       含む六つ)が入ったビルドは、TestFlight に上がりましたか？

Q2. `tools/token-check.mjs` をゲートに入れますか
    A: 入れる ── docs/STATE.md § 7 の規則「a check enters the gate in the
       same commit that adds it, or it does not enter at all」。
       入れると 35 本 → 36 本
    B: 入れない ── ゲートの本数は CLAUDE.md にも書かれていて、
       CLAUDE.md は claude/aud-claude が持っています
    コードは今: tools/ にあり、緑に通ります。npm の名前を私が足したので
       `npm run token` で回せますが、`npm test` では回りません
    訊きたいこと: token-check をゲートに入れて 36 本にしますか？

Q3. 拡張の `RequestsOpenAccess` を `false` に落としますか
    A: 落とす ── docs/keyboard-extension.md § 7 が「拡張はこれを一つも
       使っていません」「false に落とせるはずです」と書いています。
       落とせば審査でフルアクセスの理由を訊かれません
    B: 落とさない ── 落として間違っていたら、キーボードが全員で死にます
    コードは今: Info.plist は `true`。拡張の Swift 六つに
       UserDefaults / URLSession / UIPasteboard / openURL は一つもありません
       (私が grep しました)。**ただしフルアクセス無しで App Group が実際に
       読めるかは、実機でしか分かりません。**落とすなら
       `www/` の手順書(kb.step4)からフルアクセスの段も同時に消す必要があり、
       `www/` は別のセッションのものです
    訊きたいこと: 実機でフルアクセスを切って、キーボードに自作の字が
       出たままかを見てもらえますか？

Q4. 端末がレシート無しで自分の段を書けるままにしますか
    A: このままでよい ── docs/STATE.md は「決めごと」と書いています
    B: 閉じる ── 誰でも自分を pro にできます
    コードは今: `plan` テーブルに端末が自分で書き、RLS が止めるのは
       「他人の段を触ること」までです(www/net.js の netPlanUp() の上に
       同じことが書いてあります)。段がサーバーに載ったのは今日で、
       「決めごと」と書かれたのはそれより前です
    訊きたいこと: 段がサーバーに載った今も、レシートの検証は要りませんか？

Q5. `press` は一番狭い端末を歩きますか
    A: 歩くべき ── docs/STATE.md は「オーナーは iPhone SE2 と 17 で
       確認している。press が測っているのは 402pt の一台だけで、SE2 の
       320pt はそこに入っていない」と書いています
    B: 今のまま ── 一台ぶん時間が倍になります
    コードは今: `press` は一つの幅で歩きます。キーボードの高さの上限だけは
       320×568 から割り出しています(www/keyboard.js の KB_REF_W/KB_REF_H)
    訊きたいこと: 44pt の床を 320pt 幅でも測りますか？
```

### 実機でしか分からないこと ── オーナーしか答えられません

```
Q6.  キーボード拡張は、実際に U+E000 以降を挿していますか。
     挿したものは LinguaType の形で出ていますか
Q7.  自作文字の用紙(PDF)は、印が 18.3〜73.2px に乗っていますか。
     `LinguaPdf.swift` は今 g.scaleBy(x:k, y:-k) を掛けています ──
     直っているはずですが、刷って読ませた人がいません
Q8.  フリックの 18px しきい値は、実機の指で効いていますか
Q9.  購入 ── 買う、消して入れ直して復元、アプリを閉じている間の更新、
     そして Plus のレシートが Plus として読まれるか(Pro ではなく)
Q10. フォントのシステム登録(LinguaShare.registerFont)。
     まだボタンがありません。作りますか
Q11. ウィジェットは Xcode で開いて建ちますか。ホーム画面に時計は出ますか
Q12. Documents への書き込みと、バックアップの世代(.1 / .2)、
     一番新しいファイルが壊れていたとき
Q13. 「接続できません」の三つの道のどれか
     (profile 0 / mkprofile − / token ≠)。画面写真一枚で決まります
```

### ダッシュボードでしか分からないこと ── オーナーしか答えられません

```
Q14. `supabase/schema.sql` はダッシュボードに流してありますか
     (流すまで、人の言語のページの ↓ は何も落としません)
Q15. メールの送信設定と DNS は済んでいますか
     (六桁が飛ばないと審査用のデモアカウントも作れません)
Q16. 自分を staff にする SQL 一行は流しましたか
     (流すまで、通報は誰の画面にも出ません)
Q17. Spend Cap は ON になっていますか
     (250GB を超えると $0.09/GB が青天井です)
Q18. Authentication → Sessions の二つの値は何ですか
     (「久しぶりに開いたらサインアウトされるか」に、この repo は答えを
     持っていません)
Q19. 審査用のデモアカウントは作りましたか
Q20. ウィジェットのプロビジョニングプロファイルは作りましたか
     (`Lingua Widget Distribution`。無いとビルドがそこで止まります)
Q21. いま出ているビルドは何番ですか
     (STATE.md には #91 と #122 と #126 が同時に載っていました。
     run number は Actions の履歴にしかありません)
```

### リーダーへ ── オーナーではなく、あなたが答えられるもの

```
Q22. ゲート全体の所要時間。docs/TESTING.md は十分と書いていて、
     別の行は十六分と書いていました。私は十分に揃えましたが、
     **どちらも測っていません。**次に全ゲートを回すとき、
     時間と screens walked / mirror / buttons の三つを控えてください ──
     その三つの数は、今どの文書にも書いてありません(わざと消しました)
Q23. `.github/workflows/i18n.yml` が「twenty-six checks」と二箇所で
     言っています。私の担当外です。持ち主を決めてください
```
