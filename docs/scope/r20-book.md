...
1〜4 は「全部そのままで進めてよい」。`st` は作らない。疑問詞六語は `q:v` の頁で
よい。＋ と隠れた段は付録の頁でよい。目次の右の数もよい。

そのうえで二つ足された ── **写真は pop 無しで撮り直す（原因を見て）**、
**章の頁は見出し（`.sec`）で節をまとめる（区切りは章の表が持つ）**。どちらも
入れた。見出しは `G2BOOK` の群の `t`（`g2.g.*`）で、**節が一つの群には付けない**
（「複数」の見出しの下に「複数形」一行は行の名を二度書いているだけ）。

### 「接続できません」が写真に写る原因

**リクエストは投げた時に失敗せず、この容器の前に立っているものが諦めた時に
失敗する ── 起動から約 15 秒後、二本同時。**`tools/shot.mjs` は `popOff()` の
あと 260ms 待って撮るので、その 380ms の窓に当たるかどうかで写る。走りの中の
どの画面に当たるかは運で、三枚に一枚に出ていた。

直し: 撮る直前に `popOn()`（アプリ自身の答え）を訊いて、消えるまで最大四回
`popOff()` する。`tools/shot.mjs` を触ったのはここだけで、scope の
「shot の指定だけ」を一行超えている。
...
7. **名詞の章で「格」の下の行が「名詞」と出る。**その節の名は `posLabel('n')`
   なので、章の名（名詞）と同じ語になる ── 「名詞 › 格 › 名詞」。「格の印」の
   ような名の方がよいが、`posLabel` は他の画面も使うので改名はこの枝でやらない。
8. **文の章（語順・名詞句の並び・です／ある）に見出しを付けていない。**三行で、
   まとめる名が思い当たらなかった。名を言ってもらえれば入れる。

## 測った数（`npm run press`）

| | integ-0905 | この枝 |
|---|---|---|
| screens built | 1059 | 1082 |
| buttons pressed | 16300 | 15987 |
| rows in one list | 3235 | 3308 |
| classes worn | 619 | 619 |
...
一覧が 73 増えたのは、章の頁が九つ増え、そのうち三つが群ごとに分かれた
一覧を持つため。
