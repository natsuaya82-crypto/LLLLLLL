...
リーダーの「commit は『一つの commit は一つの種類』で分ける（140 の上限／
頭の @／もっと読む／docs）」に**従えていません** ── 全部を一本
（`6795ace7`）にまとめました。リーダーの三点の答えが届く前に作り終えて
push していて、そのまま報告まで書いてしまいました。

**分け直すには**、push 済みの履歴を書き換えたうえで `www/post.js` の
中間状態を三つ手で組み直すことになります（上限・頭の @・畳みは同じ file の
別の場所です）。どれも check を通していない木になるので、**勝手にはやりません。**

- **分け直してほしい**なら言ってください。この枝は私しか居ないので
  `--force-with-lease` で組み直します。
- **このままでよい**なら、commit message に四つの種類が全部書いてあります。

次から分けます。

---

## オーナーが写真を見て言った三点（2026-09-15、リーダー経由）

**⑫**

> もっと読むで開いたら折り畳まないとダメでは？
>
> あとこの機能を使うにはの背景真っ白なのはなぜ？ツイートされちゃってない？
>
> なんでタグが翻訳の横にいるの？

### 1. 「もっと読む」で開いて、たたみ直せる

**その場で開く形に変えました。**前はスレッドへ飛ぶ形で、**行った先は元に
戻せません** ── 着いてしまえばその投稿はただ長いだけでした。開くのは
「取り消せること」なので**状態**にして、開いている間は「たたむ」と出ます。
一つの状態を一つの関数が反転するだけで、閉じる用の関数はありません。

**名前がぶつかっていました。**`postMore` は既に **⋯ メニューを開く関数**で、
私はそれと同じ名前で二つ目を宣言していました ── JavaScript は後の宣言を
採るので、**「もっと読む」を押すとメニューが開いていました**。`act-map.js`
にも同じ名前が二度入っていて、`act-check` がちゃんと撥ねます（赤を見ました：
`act 'postMore' -- registered twice` と、名前の無いボタン 128 本）。
`postUnfold` / `PUNFOLD` / `postFoldBtn` / `.punfold` に改名しました。

### 2. ポップの写真 ── 背景が真っ白だった理由

**投稿はされていません。**写真が白かったのは私の顔がポップだけを返していた
からで、**断られた画面が写っていなかった**だけです。オーナーの読み方のほうが
正しい ── 画面が写っていないのに「されていない」とは読めません。

**書く画面を後ろに残したまま撮り直しました**（`shots/r39-cap-pop-ja.png`）。
赤い輪の **−13** が下に写っています。

**投稿が出ていないことは `post-check` 26 が押さえています。**バグを入れ直した
ときの出力：
...
`sent` は `POSTS.length` が増えたかで、**増えていません**。逆向き（断られる
べきが出て行った）も同じ主張が見ます：
...
### 3. fixture の投稿を全部、今の形に

`shots/r39-feed-4tags-ja.png` の二本目（Iri）が、本文の中に `#今日のお題` を
持つ**古い形**でした。だからタグが翻訳の横に出ていました。
「昔の投稿は加味しなくていい」なので、**fixture の投稿は全部 `body.tags` の
形**にして、本文から `#` を抜きました。

**古い形を描く道（`tagHTML()`）は残しています** ── 今日より前に書かれた投稿は
今も本文の中にタグを持っているからです。fixture に描く物が無くなっただけです。

`post-check` 28 が「fixture のどの投稿の本文にも `#〜` が無い」を見ます
（赤も見ました）。
...
`post` `press` `i18n` `act` `es5` `dead` ── リーダーの名指しぶん。ほかに
