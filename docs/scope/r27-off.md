...
**#11（`www/sns.js`）** 表に列を一つ足す（`PULL_OFF`）。**一つの値に畳めない**
のがこれを書いていて分かったところで、理由は測ればすぐ出る：十分前の答えを持って
いる画面で引いて落ちたら、その画面は「答えを持っている」と「届いていない」の
両方であり、**どちらの半分も読む人がいる** ── 答えは電波なしでも下書きと通知を
画面に残す半分（規則 22）、落ちたことは「まだ何もない」を言わせない半分。一つの
値にすると、落ちた瞬間に下書きが画面から消えます。だから `PULL_ON`・`PULL_HAS`・
`PULL_OUT`・`PULL_GOT` と同じ**表の列**で、二本目の仕組みではありません。
**描く側が訊くのは `pullSay(r)` 一つ**：0 まだ何も言えない・1 サーバーが答えた・
−1 訊けなかった。
...
`keepBack()` で端末を押す前に戻す。**紙はそのまま** ── これは覚えて戻すのでは
なく、**片付けるのを答えの後ろへ動かす**（`addW`・`addFms`・`addFrom`・`addSlot`
を空にする四行。「打ったものは欄に残る」）。失敗のときは**描き直さない**：
`render()` はポップを畳むので（`www/glyph.js` § render）、描き直すと
「接続できません」が同じ frame で消える。新しい部品も新しい文言も無し。
