...
終わったら最後のメッセージを「r63-audit 完了」で始め、規則 8 の形で報告する。
オーナーには話しかけない（リーダーが伝える）。

── 並行セッションの規則 ──────────────────────────────
あなたの領域: 二重確認（r46 が見ていない所）── 読んで報告だけ、コードは変えない
あなたのブランチ: claude/r63-audit
あなたが持つファイル:
  docs/scope/r63-audit.md だけ
  （コードも docs もこれ以外は一行も変えない）
...
── r63 の担当 ──────────────────────────────
オーナーの「この一覧以外にも無いか、二重に確認する」。r46-audit の「見ていないもの」を
**同じ三つの問い**で洗う:
  A. 端末の写しがサーバーに勝つ／人が押していないのにサーバーへ書く／丸ごと送る
  B. 一つの事を二つ以上の仕組みでやっている（§ Simple）、穴を一つずつ塞いだ継ぎ当て
  C. 書いてある事（コメント・docs）が今のコードと違う
対象: `www/grammar-engine/`、`www/keyboard.js`、`www/card.js`、`www/sheet.js`、
`www/sync.js`（syMerge 以外）、`www/push.js`、`www/cal.js`、`www/rec.js`、`www/store.js`、
`www/mod.js`、`ios/`（キーボード拡張・課金・広告 LinguaAds.swift）、
`supabase/schema.sql` の policy と RPC と `supabase/functions/`、`www/index.html` の CSS、
`localStorage` に書くキーのうち後でサーバーへ送られるもの（`tools/store-check.mjs` の
一覧から一つずつ）。

さらに **r46 が見た所の読み直し**も一回: r46 の A〜C の各項目について、r46 の読みが
合っているかを別の目で読む（違っていたら書く）。

一件ごとに: ファイル:行、どのルール（CLAUDE.md の節名・ルール番号）、起きること、
**測ったか読んだだけか**。測れるものは測る（`node tools/shot.mjs`、検査の fixture、
`npm run rls` で schema は実物の PostgreSQL に当てられる）。

**直さない。** 直すのは別のセッション。結果を `docs/scope/r63-audit.md` に書いて push し、
「r63-audit 完了」で報告する。r60（§A）・r61（§B1/B2/B4）・r62（§C）が並行して直して
いるので、それらと重なるものは「r60 の面」のように印を付ける。
