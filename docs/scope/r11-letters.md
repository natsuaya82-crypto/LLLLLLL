...
## 訂正 ── 上の「差ゼロ」の表は取り消しです（2026-09-10）

**測る側に欠陥がありました。**`tools/again-check.mjs` の偽サーバーを
テンプレートリテラルの**生のファイル文字列のまま**取り出して `eval` していた
ので、その中の
...
が `=eq\\.` という正規表現になり、`language=eq.srv1` に一つも当たりません
でした。**GET が毎回 `[]` を返していた**ので、「147 と 149 で差ゼロ」は
**何も答えないサーバー相手の測り**です。`again-check` 本体は正しく動いて
います（あちらはリテラルとして評価される）。壊れていたのはこの枝の写しだけ
です。
...
| 電話の状態 | 147 | 149 |
|---|---|---|
| 使い込んだ端末 | L=39 言語=1 鯖 39→39 | 同じ |
| 写し（`.got`）が無い | L=38 言語=1 鯖 39→42 | 同じ |
| 入れ直し | L=38 **言語=3** 鯖 42→**38** | L=39 **言語=2** 鯖 39→39 |
| 空の端末 | L=38 **言語=3** 鯖 42→**38** | L=39 **言語=2** 鯖 39→39 |

**149 の方が綺麗です。**147 は入れ直し・空の端末で言語を三つ作り、保存で
サーバーの letters を 42 から 38 へ**縮めます**。149 はどちらも一つの言語、
39 のままです。**どの道でも 149 で a〜z は消えず、保存も通ります。**

残る二つは 147/149 のどちらにも在り、この枝の受け持ちではありません
（リーダーへ）:

- 写しの無い端末で保存すると、サーバーの letters が **39 → 42** に増える。
- 入れ直しの端末で、起動のあとサーバーの letters が **42**（147）── 端末に
  無い言語が上がっている。
