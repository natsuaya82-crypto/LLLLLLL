...
## L. ⑮ 人の言語の詳細への入口 ── 言語の **id** が端末に届いていません

**サーバーは開いています。**そこは確かめました:

```
language_read : published_at is not null or owner = auth.uid()
slice_read    : 持ち主 or (kind in ('wld','script','snd','letters','kb')
                           and その言語が published)
```

**足りないのは一つだけで、`www/net.js` の select です。**

```
netLangNames()  /rest/v1/language?select=owner,name&owner=in.(...)
                                        ^^^^^^^^^^^ id も published_at も無い
```

`netSlices(sid, ok, bad)` は既に在って言語 id で全スライスを取れますが、
**その id が端末のどこにも来ていません。**`whoOf()` が持っているのは `lname`
（名前の文字列）だけです。

  www/net.js  `select=owner,name` に `id` と `published_at` を足す。
              **渡されていない**

**投稿から取る道は取りません。**投稿は `lang:langId` を運んでいるので、その人の
投稿をこの端末が持っていれば id は分かります ── **が、持っていなければ入口が
消えます。**「持っている人には出て、持っていない人には出ない入口」は、
CLAUDE.md の言う「半分だけ動く画面」そのものです。

**描く側はこの枝の持ち物で、id が来たら書きます。**いまは書きません
（`dead-check` に引っかかる、届かない関数になるので）。大きさだけ先に:

```
wldPage()  var w=world(), drawn=LETTERS.filter(...), ... langName ...
```

開いている言語のグローバルを三つ、頭の行で読んでいます。**これが元の不具合**
（押すと自分の言語が出た）で、規則8そのものです。直し方は、取ってきた
スライスの束を引数で受ける形にするか、読む専用の描画を別に置くかの二択で、
**どちらを選ぶかは id が来てから決めます。**非公開の言語は `language_read` が
行ごと返さないので、閉じたままが自動的に成り立ちます。
...
## M. ⑯ 自己紹介 ── 描く側は入りました（446db67）

`whoOf()` が `bio:''` と直に書いて潰していたのを `got.bio||''` に。**列が来た日
に、コードを一行も足さずに人のページとフォロー中の一覧の両方に出ます。**
`claude/acct2` が `profile` の列と `netWho()` の読みを入れる、で噛み合います。

**フォロー数・フォロワー数は 0 のままです**（上の F）。net.js が要ります。
