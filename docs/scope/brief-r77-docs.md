あなたはリーダーから出された作業セッション r77-docs です。土台は `integ-0905`。規則 6 の「master を入れる」は
...
`node tools/shot.mjs --lang ja` で前後を撮って shots/ に。終わったら「r77-docs 完了」で始め、規則 8 の形で報告。
オーナーには話しかけない。

── 並行セッションの規則 ──────────────────────────────
あなたの領域: 書いてある事が今と違う・決定が規則を置き換えたのに直っていない（r73 §2-17・付録 C・§0 #10）
あなたのブランチ: claude/r77-docs
あなたが持つファイル:
  CLAUDE.md docs/*.md（CHANGELOG と scope と reports は書き換えない。STATE.md は付録 C の文だけ直してよい）
  README.md tools/docs-check.mjs tools/docs-baseline.txt
  （www/ と supabase/ は一行も持たない。コードのコメントの古い文は一覧に書いて、そのファイルの持ち主へ）
...
── r77 の担当 ──────────────────────────────
r73 §2-17 全部、付録 C、§0 #10（「決定が規則を置き換えても規則・状態の文が直っていない」）。
- CLAUDE.md「And when a decision replaces a rule, FIX THE RULE — in the same commit」「Fixing means deleting」。
  置き換えられた古い文は消す（「これは歴史」と前置きして残さない）。決定ログの superseded の印が無い組は、
  **新しい方が後から言われた物なら**古い方に superseded を付けて規則の文を今に直す。どちらが新しいか分からない
  二つの書かれた決定の食い違い（r73 §5-10・§5-11・§5-14）は直さず「オーナーへ」。
- docs-check の穴（r73 §2-17 が書いた、識別子と検査名を数えない等）を面で覆う: 文書が名指す**検査名**
  （`npm run x`、`x-check`）と**数**（「the fast nine」「39 checks」「28本」）も数える形に。数を文書に書かない
  （CLAUDE.md の規則どおり、gate の最後の行から読む）。
- docs-baseline に凍っている 32 は、直せる物は直して行を外す。README.md は今のアプリの説明に書き直す
  （古い 21 名）。
- STATE.md は付録 C の文だけ（リーダーの物だが、今夜はこの節の分を任せる）。