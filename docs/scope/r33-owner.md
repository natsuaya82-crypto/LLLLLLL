...
**追記 2（リーダー 2026-09-12 04:11）**：`plan-check` の 700ms の固定待ちを、
待つものそのものに取り替えました（`7ab6d9f7` のあと、commit 一つ）。
`boot()` は「verify-plan の要求が偽サーバーの log に現れるまで」と
「`planKnown()` が真になるまで」を別々に待ちます（上限 5 秒、越えたら赤）。
主張も二つに割れています ──「訊いたか」は要求の有無、「答えが着いたか」は
`planKnown()` と読んだ値。**測って分かって直したことが一つ**：電波が無い起動は
verify-plan を**一度も訊きません**（一時間前のトークンを更新できないので、
訊くための session が無い）。リーダーの指示は「電波なしでも要求は出る」という
見立てでしたが、出ていません ── その主張は「一度も訊かない」を測る形に直して
あります。赤は三つの形で見ました：電波を戻さないと「訊き直さない」が赤、
答えを別の段にすると「答えを取る」だけが赤（「訊いた」は緑のまま）、
`planTook()` を潰すと手前の主張から赤。二回続けて緑。
