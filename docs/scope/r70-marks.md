...
|---|---|---|---|---|
| カード | `cardSave` | 共有 | `ICON_SHARE`、**バーの右上へ移した** | card.js |
...
## 右上（OWNER 2026-09-23 の続き）

「右上にしてね。送信も　紙飛行機右上、共有も共有マークを右上。その位置に書くものはない。」
「ルールの徹底なんだからそこだけ直すのやめろよ」

**送る・共有をしている画面を全部数えた**（en の字で send/post/reply/share を名乗るボタン、
字でも印でも、全画面・全顔）：

| 画面 | 操作 | 今 | 位置 |
|---|---|---|---|
| カード | `cardSave` 共有 | 共有の印 | **右上へ移した**（前はカードの下） |
| お問い合わせ | `contactGo` 送信 | 紙飛行機 | もとから右上（settings.js のコメント「送信右上にして」） |
| 投稿画面 | `pwSend` 投稿する | **字**（r60） | 右上。印にするのは r60 の後 |

これ以外に送る・共有を名乗るボタンは無い。投稿の行（`postCard`）・語のページと例文の行
（`cardOpen`）の共有の印は「カード」と名乗り、カード画面を開くもの ── 共有そのものは
カード画面の右上で起きる。サインインの「コードを送る」「もう一度送る」は目的語つきの行で、
扉の中の手順（下の一覧に残した）。

検査：送る・共有が右上以外に出たら（印でも字でも）赤。直す前のカードで赤を見た
（`cardSave ("Share") is a share in the page`）。

**右上に字で出ているもの（17）── 印が決まっていないので替えていない。**
「その位置に書くものはない」をリーダーは「右上に字を置かない」と読んだが、原文は「（カードの）
その位置には他に置く物が無い ＝ 空いている」とも読める。右上の字を全部なくすには、完了・選択・
保存・確定に印を決める必要があり、それはオーナーのもの。検査は数を毎回出す（赤にはしない）。

| 操作 | 字 | 画面 |
|---|---|---|
| `wSelOn` `ntSelOn` `kbSelOn` `g2SelOn` `dfSelOn` | 選択 | 辞書・メモ・キーボード・規則・下書き |
| `wSelOff` `ntSelOff` `kbSelOff` `g2SelOff` `dfSelOff` | 完了 | 同じ五つの、選んでいる間 |
| `ltWobEnd` | 完了 | 字を並べ替えている間 |
| `kbLtPut` | 確定 | キーに字を選んだとき |
| `keepPress` | 保存 | 言語の記事を書く顔 |
| `pwSend`（編集中） | 保存 | 投稿を直している時（r60） |
| `go` | フォロー中／おすすめ／最新 | タイムラインの二つのタブ（選ぶもの。値） |
