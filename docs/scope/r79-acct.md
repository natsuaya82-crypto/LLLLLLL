...
- **3**「`langMineIds` の道を消す」── 索引がアカウントの物になると、持ち主の無い枝に届くのは歩きの言語だけ
  （指示書自身が残せと言う例外）。コードは残し、コメントを今の本当にした。acct-check 86 が持つ。
- **8**「netOut が device の行を残す」── 押したサインアウトではもう落ちていた（acct-check 84）。足りなかったのは
  refresh を断られた道と削除の道。
- **11-2**「`netLangsWalk` の `netLangNamePut` の段落」── r71 で `netLangFill` に移っていた。そこを消した。
- **1**「`setFor` と同じ形で預ける」── 預ける形そのものが穴だった（戻った時に古い預けが残り、生きている鍵は印が無い）。
  預けずに、書く時に uid の鍵へ書く形にした。
- **12**「9 画面」── page から集めると保存ボタンのある画面は 37。K1 の覆う一文は、キーの面の「確定」の決定
  （2026-09-03）と食い違う（上の「オーナーへ」）。
- **ゲートの赤 gramlang**── 読み（`langLocked()`）は合っていた。直すのは検査の前提（種の `owner.got` は写しで、書いてよいかを決めない）。
