...
**消しました**（`docs/CHANGELOG.md` は触っていません）。食い違いは決めずに
**§ オーナーに訊くこと** にまとめました ── 七件です。
...
**七件。**下の § 見つけたもの の五十件は、数える・読む・動かせば分かったので
こちらには入れていません。ここに在るのは**決めないと分からないものだけ**です。
Q1 と Q3 は下の 48 と 49 と同じもので、そちらから移しました。

```
Q1. 言語の上限は「この端末に何個あるか」で数えるのか、「このアカウントに
    何個あるか」で数えるのか。
    A: 端末で数える ── docs/FEATURE_RULES.md:2794「**Not a loophole,
       decided:** the language count is what is on THIS PHONE … signing in as
       somebody else neither adds a language nor resets the count」。
       2026-08-23 の段の決定の Implementation status に書かれている
    B: アカウントで数える ── www/core.js:900 `langOwned()` が `SESS.uid` を
       見る。2026-09-02「1アドレス1アカウント」「これは絶対課金もアカウント
       ごと」
    コードは今 B。A で入っていたものが 2026-09-02 に B へ書き換わり、
    A の文だけが残った
    訊きたいこと: 一台の端末で、A さんが 1 個作ったあと B さんでログインしたら、
    B さんは無料のまま 1 個作れてよいですか？（はい＝B のまま／いいえ＝A に戻す）

Q2. Plus の「DL 1 個」の 1 は、言語 1 個ですか、章 1 個ですか。
    A: 言語 1 個 ── www/core.js:986 `dlCount()` は index の行を数える。
       トキポナの単語・文字・文法・キーボードを全部取っても 1
    B: 章 1 個 ── docs/FEATURE_RULES.md:842「単語・文字・キーボードは
       一つずつ DL できる」、2026-09-01「いや一つづつdlでいいよ」。
       章ごとに解放する `wldSecDl()` があり、章が単位に読める
    コードは今 A
    訊きたいこと: Plus の人は、トキポナの単語と文字を両方取れますか？
    （はい＝A／いいえ、どちらか一つ＝B）

Q3. 買う画面の Pro の五行に、言語 3 個と DL 3 個が入っていません。
    A: 今のまま五行 ── www/i18n の `plan.pro.1`〜`5` は「Plus の全部と、
       単語無制限・キーボード無制限・文法と向き・ファイル」
    B: 書く ── docs/PAID_FEATURES.md の表と `langCap()` `dlCap()` は、
       Pro が言語 3 個と DL 3 個を売っていると言っている
    コードは今 A。**Pro が売っているものの一部が、買う画面のどこにも
    書かれていません**（Plus の DL 1 個も同じ）
    訊きたいこと: Pro の五行に「言語を三つ」を入れますか？（はい／いいえ）

Q4. シートで文字を入れる道（章 26）は、CSV とファイル取り込みと同じ
    `can('file')` で開いています。専用の名前がありません。
    A: 同じ門でよい ── 今日はどちらも Pro なので、見え方は決定どおり
    B: 専用の名前が要る ── 決定は 2026-08-23「シートは Pro」で、
       `file` は「一覧をファイルで入れる」。いつか `file` を Plus に
       下ろすと、シートも黙って一緒に下りる
    コードは今 A（www/sheet.js:1299, 1533）
    訊きたいこと: CSV の取り込みとシートは、これからも必ず同じ段ですか？
    （はい＝A のまま／いいえ＝分ける）

Q5. 単語の天井に当たったとき（`capStop()`）、いま画面はポップで訊いてから
    飛びます。2026-08-25 の「閉じている扉は押したら課金ページへ」と
    同じ形にするかどうかが、決定ログの中で**未回答のまま**です。
    A: 今のまま訊く ── docs/FEATURE_RULES.md:2392「`capStop()` は含まない、
       と読んでいる … **この読みが違うなら `capStop()` も直す。
       オーナーに聞くこと。**」
    B: 訊かずに飛ぶ ── 扉と同じ形にする
    コードは今 A
    訊きたいこと: 単語を打っている途中で百語目に当たった人は、訊かれてから
    料金表へ行く（今のまま）でよいですか？（はい／いいえ）

Q6. 広告は、まだ来ますか。
    A: 来る ── docs/FEATURE_RULES.md:2910「`noads` is in that table and is
       NOT in `CAN` yet, deliberately」、2922「ベーシックも広告表示させる
       よ？＋から広告非表示で考えてた」
    B: 来ない
    コードは今 ── 広告は一つも無く、`noads` も無い。`CAN` は `dl` が入って
    十二になっているので、「広告が来たら十二」と書いてあった文はもう
    数として合いません
    訊きたいこと: 無料と Plus に広告を出して、Pro を広告なしにする話は
    まだ生きていますか？（はい／いいえ）

Q7. サーバーがレシートを確かめないまま、課金を App Store に出しますか。
    A: 出さない ── docs/apple.md § 6 と docs/FEATURES.md § 1 は
       「レシート検証は本番」と書いている
    B: 出す
    コードは今 ── 段はアカウントの `plan` 行に乗るが、**その行に載るのは
    端末が言ったこと**で、誰も Apple に訊いていない。改造した端末は自分を
    Pro だと言えて、サーバーはそれを書き留める
    訊きたいこと: レシート検証より先に、課金つきで審査に出しますか？
    （はい／いいえ）
```

**Q1 と Q2 は上限の話なので、答えが来たらコードが動きます。**Q3 は言葉、
Q4 は境目、Q5 は画面、Q6 と Q7 は順番です。
...
48. docs/FEATURE_RULES.md:2794 「**Not a loophole, decided:** the language
    ...
    実際: 2026-09-02 の「1アドレス1アカウント」で `langOwned()`
          ...
          **このアカウントの**言語だけを数える。別のアカウントで入ると数は
          変わる
    判定: **オーナーが決めること** ── 書かれた決定が二つ、逆を向いている。
          有料の上限が抜けるかどうかの話なので、日付だけで片付けません
    やったこと: 直さない。**Q1 に出しました**
