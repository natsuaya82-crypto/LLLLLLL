...
- 日付: 2026-09-03
- 枝: `claude/aud-pay`
- 基準にした木: `master` の `44a9e75`（報告のたびに取り込んでいます。他の枝は
  一つも取り込んでいません）
- 担当: `docs/PAID_FEATURES.md`、`docs/FEATURES.md`、`docs/apple.md` の
  一文ずつを、いまのコードに当てる。答えが返ってからは `www/i18n/*.js` も

**「全部確かめました」とは書きません。**確かめていないものは § 確かめていない
もの に名前で並べてあります。

**2026-09-03 の追加決定の二つに従っています。**置き換えられた文は印を付けずに
**消しました**（`docs/CHANGELOG.md` は書き換えず、足すだけ）。食い違いは
決めずに七件出し、**七件とも答えが返っています**（§ オーナーに訊いた七件）。
...
**七件出して、七件とも返っています**（2026-09-03）。下の § 見つけたもの の
五十件は数える・読む・動かせば分かったので、こちらには入れていません。

```
Q1. 言語の上限は何を単位に数えるのか
    答え: **アカウント。**「は？端末の話なんかしてねえだろ」「だから端末で
          やるわけねえだろ」。この app にその単位は無い（CLAUDE.md § Online）
    コードは既にそうなっていた（`langOwned()` が `SESS.uid` を見る）
    やったこと: docs/FEATURE_RULES.md:2877 の逆を書いた一段落を消して、
          アカウントで数えると書いた

Q2. 「DL 1 個」の 1 は言語か、章か
    答え: **言語。**「DLしたらDL言語がアカウントに出てくるだろ？それはなにを
          ダウンロードしてもその言語からDLしてんだから1個だろ」
    コードは既にそうなっていた（`dlCount()` が index の行を数える）
    やったこと: docs/FEATURE_RULES.md の「単語・文字・キーボードは一つずつ
          DL できる」を、章は一つずつ取るが数えるのは言語、と書き換えた

Q3. 買う画面に、言語と DL の数が書かれていない
    答え: **入れる。**「何で入ってないの？」
    やったこと: **コードを直した。**下の § Q3 でやったこと

Q4. シートの門が CSV と同じ `can('file')` である
    答え: **取り下げ。**「4は同じ弾って何の話？」── 決定ではなく名前の話で、
          出すべき問いではなかった
    やったこと: `can('file')` のまま。将来 `file` を下ろすと用紙も一緒に
          下りる、という一行だけ docs/BACKLOG.md に置いた

Q5. 単語の天井（`capStop()`）は訊いてから飛ぶままでよいか
    答え: **今のまま。**「5はいあってる」
    やったこと: 決定ログの「この読みが違うなら直す。オーナーに聞くこと」を
          消して、天井は訊いてから飛ぶ・扉は押したら飛ぶ、と決まった形で
          書いた

Q6. 広告はまだ来るか
    答え: **今はやらない。**「6いまはいい」
    やったこと: `noads` は `CAN` に入れない。決定ログの「広告が来たら十二」の
          ような数を予告する文を二箇所消した

Q7. レシート検証より先に、課金つきで審査に出すか
    答え: **サーバーが Apple に訊く。端末に決めさせない。**「だから端末で
          やるわけねえだろ馬鹿じゃねえの」
    コードは別のセッションが持つ。**触っていません**
    やったこと: docs/apple.md § 6 と docs/FEATURES.md § 1 の「レシート検証は
          本番でやる」という書き方を、「いま端末が決めていて、これから直す」に
          書き換えた
```

## Q3 でやったこと ── 買う画面に上限を書く

`langCap()` は Pro に言語を三つ、`dlCap()` は Pro に DL を三つ、Plus に一つ
渡していて、**その三つはどのカードにも書かれていませんでした。**

**行を足しました。**既存の行に足すのではなく足したのは、どの行も一つのことを
言う形で揃っているからです ── 「単語は1000語まで」「自作キーボード4つまで」の
隣に並ぶのが自然で、二つを一行に詰めると説明文の側に寄ります。

```
  Plus  ＋1 行   plan.plus.6   ダウンロードした言語1つまで
  Pro   ＋2 行   plan.pro.6    自分の言語3つまで
                 plan.pro.7    ダウンロードした言語3つまで
```

**Plus に「自分の言語」の行はありません。**Plus の言語は無料と同じ一つで、
`langCap()` が `FREE_LANGS` を返します。無料と同じ数を有料のカードに書くと、
買うと増えるように読めます。

**画面は崩れません。**`.plpage` は幅 80% の flex 項目で、`.pllines` は高さの
決まっていない縦並びです（`www/index.html:1758,1830`）。カードは下に伸びるだけ
で、行数は今も 4 / 5 / 6 と揃っていません。足して 4 / 6 / 8 になります。

**絵は既定の ✓ が付きます。**`planMark()`（`www/settings.js`）に三つの鍵は
無く、あの関数は `m[key] || ICON_TICK` で落ちます。`www/settings.js` は
`claude/plannow` のものなので触っていません ── **絵を選ぶのはそちらです。**

**赤を見てあります。**`www/i18n/ko.js` から `plan.pro.7` を抜いて
`npm run i18n` を回すと、`keys: ko is missing plan.pro.7` と
`the walk: fell back to English: ko plan.pro.7` の二行で落ちます。戻して緑。

決定ログに項を立て（「買う画面には、そのプランが売っているものを全部書く」）、
`docs/CHANGELOG.md` にも同じコミットで書きました。
...
46. package.json:「"ask": "node tools/ask-check.mjs"」
    実際: `tools/ask-check.mjs` が無い。四十本のスクリプトのうちこれだけ。
          ...
          呼ばないのでゲートは緑のまま
    判定: 規則が守られていない
    やったこと: **master で解決済み。**行は消え、`assets-check` が両向きを
          持つようになった ── 無いツールを指すスクリプトも、`npm run` の
          alias が無いゲートの行も落ちる
...
47. www/settings.js / www/store.js / ios/App/App/LinguaStore.swift
    ...
    やったこと: **master で解決済み。**`claude/plannow` が入り、
          `plan.now` `plan.now.only` `plan.date` が十言語に在る

48. docs/FEATURE_RULES.md:2794 「**Not a loophole, decided:** the language
    ...
    実際: 2026-09-02 の「1アドレス1アカウント」で `langOwned()`
          ...
          **このアカウントの**言語だけを数える。別のアカウントで入ると数は
          変わる
    判定: **オーナーが決めること** ── 書かれた決定が二つ、逆を向いていた
    やったこと: **Q1 で答えが出た。**単位はアカウント。逆を書いていた
          一段落を消して、アカウントで数えると書いた
...
49. www/i18n の `plan.plus.*` / `plan.pro.*`（各五行の売り文句）
    ...
    やったこと: **Q3 で「入れる」と返ってきた。直した** ── 上の § Q3
...
docs/apple.md           直した
docs/FEATURE_RULES.md   答えが返った四件（Q1 Q2 Q5 Q6）と、Q3 の決定の項
docs/BACKLOG.md         Q4 の一行
docs/CHANGELOG.md       Q3 の項を足した（書き換えていません）
www/i18n/*.js           十本ぜんぶに三つの鍵（Q3）
www/core.js             `PLANS` の Plus に一行、Pro に二行（Q3）。
                        ほかはコメントだけ ── CAN の数、段が売りに出ているか、
                        古い段の名前、`langCount()` の上に二重に残っていた段落
tools/gate.mjs          FAST に paid-check。取り込みで見出しの注釈が三重に
                        重なっていたので、いま効いている一つに書き直した
tools/paid-check.mjs    新規
docs/scope/aud-pay.md   これ
```

`www/settings.js` `www/store.js` `ios/App/App/LinguaStore.swift`
`CLAUDE.md` `package.json` `supabase/` には触っていません。**Q7 のコードにも
触っていません。**
