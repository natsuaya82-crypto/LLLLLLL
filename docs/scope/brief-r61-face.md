...
終わったら最後のメッセージを「r61-face 完了」で始め、規則 8 の形で報告する。
オーナーには話しかけない（リーダーが伝える）。

── 並行セッションの規則 ──────────────────────────────
あなたの領域: 自作文字で文を出す仕組みと、SET を決める所（r46-audit § B1・B2・B4）
あなたのブランチ: claude/r61-face
あなたが持つファイル:
  www/glyph.js www/index.html（CSS） www/grammar.js www/numbers.js www/wordsheet.js
  www/words.js www/onboard.js www/settings.js www/home.js www/letters.js www/sound.js
  www/act-map.js www/i18n/*.js
  tools/fixture.mjs、新しい検査一本（要れば。package.json と tools/gate.mjs に登録）、face-check / line-check（要るなら）
  docs/CHANGELOG.md docs/FEATURE_RULES.md docs/scope/r61-face.md
  （net.js・core.js・me.js・post.js は r60 のもの。要るなら止めて報告）
...
── r61 の担当 ──────────────────────────────
r46-audit § B1（自作文字で文を出す仕組みが三通り残っているか：`.sfont`/`myFontOn()`、
`sfontHTML()`、r53 の `.tfont`/`.pline` の LinguaType）、§ B2（`SET.myfont` を決める所が
三つ ── 特に `geKeep`/`geKeepOn` が文字を保存するたびに本人のオフを上書きする）、
§ B4（同じ SET / ME の項目を複数のファイルが書いている ── `SET.showScript`・`SET.ui`・
`SET.walked`・`ME.handle`/`ME.name` の onboard.js 側）。

**B1 はまず測ること。** 三つが「同じこと（この文字列を自作の字で出す）」を別々に
やっているのか、別のこと（要素全体か、描いた所だけか）なのかを、画面ごとに実際に
描いて確かめる。同じことなら一つに書き直す ── r53 が投稿の一行でやったのと同じ形
（`CLAUDE.md` ルール 8「A line is drawn by ONE mechanism」）。違うことなら、なぜ違うかを
報告に書いて触らない。

**B2/B4 の覆う一文（リーダーの読み）:** 「SET の一つの項目を決めるのは一か所で、人が
決めたことを、別の操作が黙って書き換えない。」面を数える検査 ── SET / ME の各項目に
ついて、書いている場所を数えて、二か所以上なら理由（別の瞬間の初期値など）が
書いてあることを求める、のような形。二か所が正当な場合もある（オンボーディングの初期値と
設定のスイッチ）ので、測ってから決める。

`SET.myfont` が今オンで何を決めているか・既定がどちらかは**オーナーの仕様**。
`CLAUDE.md` の「SET.myfont is off until somebody turns it on」が書かれた仕様。
それと食い違うコードを見つけたら、仕様に合わせる（コードに合わせて仕様を読み替えない）。
