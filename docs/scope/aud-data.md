...
- 基準にした木: `master` の `9cc255d0`（「古い規則は残さない。食い違いは
  オーナーに訊く」を取り込んだあと）
...
### 消す方向の変更 ── 手を付けていません。オーナーの判断待ち

**7・8・9 と 26 は「データを消す」側です。見つけただけで直していません。**
`docs/DATA_SAFETY.md` § DELETE REVIEW がその理由です。
...
- **7・8・9**（`lingua.notices.<uid>` / `lingua.set.<uid>` / `lingua.<id>.bkn`
  が消し残る）── 消す方向。オーナーの判断待ち
