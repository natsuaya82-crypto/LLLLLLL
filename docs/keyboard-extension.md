...
**ビルド時に決めます。**設定 → キーボードの一覧と、🌐 の長押しに出る名前で、
拡張ターゲットの `CFBundleDisplayName` です。

- **こちらで好きな文字列にできます**（日本語も絵文字も入ります）
- **使う人があとから変えることはできません。** iOS にキーボードの名前を
  変える仕組みがありません
- **言語ごとに変えることもできません。**拡張は1つで、名前も1つです。
  10個の言語を持っていても、キーボードの名前は同じ1つです

決めていなければ `Lingua` にします。

### 「Lingua の中だけで使う」

**それは、拡張を作らないことそのものです。** いまがその状態です。

iOS のキーボード拡張は、**いったん有効にすると全部のアプリで出ます。**
特定のアプリだけに限る方法はありません — 拡張には「いまどのアプリの中にいるか」
が渡されないからです（Apple がプライバシーのために渡していません）。
ホストのバンドル ID を取る非公開の手はありますが、審査で落ちます。

逆方向は問題ありません。**拡張を入れても、Lingua の中は今まで通りです。**
言語を書く欄は `readonly` にしてあってアプリ内キーボードが出るので、
システムキーボードとぶつかりません。

つまり選べるのは2つで、その中間はありません:

| | 出る範囲 |
|---|---|
| 拡張を作らない | Lingua の中だけ |
| 拡張を作る | 全部のアプリ（Lingua の中は今まで通りアプリ内キーボード） |

### フルアクセスについて

拡張が App Group からあなたの文字を読むのに**必須**です。オフだと空の
キーボードになります。何もサーバーには送りませんが、iOS は全部の他社製
キーボードに同じ強い警告を出すので、これは消せません。
