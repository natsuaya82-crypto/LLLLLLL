# Lingua キーボード（iOS Keyboard Extension）— 必要なもの全部

**書きました。動いています。**この文書は仕様書であり、書いたものの説明であり、
実機で打てているものの説明でもあります。自作の字がキーに並び、打てるところまで
確認済みです。**その一度はフルアクセスを与えて開いたときのもので**、要求は
2026-09-18 に落としました（§ plist）── **オフのままで打てるかは実機未確認**。

**どこで使うものか。** 仕組みは iOS のキーボード拡張だが、**使う場所は
Lingua の中**である。自作文字で書くのは Lingua の中の入力欄 — 投稿の作成欄、
単語、例文 — であって、Messages で使えるようにはしていない。
**SNS が Lingua の中にあるのはそのためである。** 自作文字で書ける場所が
Lingua の中しか無いのだから、書いたものを見せる場所も中にある。

拡張という形を取っているのは、iOS が「アプリが自分の入力欄に自前の鍵盤を
出す」ことを webview の中で素直にやらせないからであって、端末じゅうで使わせ
たいからではない。この一行が無かったせいで、この文書は「Messages で自作文字
を打つためのもの」と読まれた。
...
- **App のプライバシー** / プライバシーポリシー に、キーボードが
  「入力内容をどこにも送らない」ことを1文書いておく。**この拡張は
  フルアクセスを取らない**ので理由は聞かれませんが、1文はそれでも書きます
...
- 打っている面がローマ字の面（`b.rom`、`onRoman`）なら、押しても
  **何も挿さず**溜める側。その人の字の面は押した文字がそのまま挿さり、
  溜まりは「続きの候補」を出すためだけに存在します。決めるのは書記体系では
  なく**面**です（`KeyboardViewController.swift` の `compose?.onRoman`）
...
**`www/otf5.js` の `bar` / `hull` を Swift に写さないでください。**写すと
b スプライン・角丸め・ペン先の凸包が Swift 側にもう一組できて、`GPEN` を
触った日にキーの上の文字とアプリの中の文字が黙って食い違います ──
**どちらの側の check にも見えません。**だから `st` は線ではなく塗り終わった
形で渡します。
...
**規則は2つあり、どちらになるかは渡されたもので決まります。**

- **キー**は正方形です。`box` を辺とする正方形をビューの短辺に合わせて中央に
  置きます。タイルとキーは正方形というのが本体の規則で（`inkCanvases`）。
- **1行**は違います。1文字の幅は自分のインクに1歩を足したもので、両端に半歩。
  どの2文字が隣り合っても間は1歩になります（`inkAdv`）。`CandidateBar` は
  1行なので、こちらです。

**バーを正方形に収めてはいけません。**細い文字が2つ並ぶと1マス分空いて、
単語が間延びします。「キーボード内のプレビューのアルファベットいちいち
全角のスペース開くのうざい」

**`inkAdv` の計算は Swift に写していません。**規則が住んでいるのは
`www/glyph.js` の `inkAdv()` 1か所で、`shareFace()` がそこに聞いて答えを
`w`（進む幅）と `dx`（その中でインクが始まる位置）として `keyboard.json` に
乗せます。ペン先を変えた日に2組目が黙って食い違う、という §F と同じ話です。

`w`／`dx` はどちらも省略可です。形の無い面には付かず、これより前のビルドが
書いた `keyboard.json` にも入っていません。無い時 Swift は正方形に落ちます
——今まで全部の面がそうだった状態で、詰めが悪いだけで落ちはしません。
...
- `PrimaryLanguage = mul`（多言語）。作った言語が何語かは Apple の一覧に
  無いので、これが正直な答えです
- `RequestsOpenAccess = false`。**拡張はフルアクセスを一つも使っていません**
  ── `UserDefaults` もネットワークもペーストボードも `openURL` もカスタム
  フォントも無く、App Group は**読むだけ**で、読むのはフルアクセス無しで
  通ります（上の KeyboardViewController の節）。**2026-09-18 に落としました**
  ── 「切っていい」OWNER 2026-09-18。同じ着地で `www/` の案内の手順 4
  （~~`kb.step4`~~、`www/keyboard.js`）も消えています。存在しないスイッチを
  押せと書いた画面は出ません。
  **DEVICE UNCONFIRMED** ── Linux では Swift が動かず、フルアクセス無しで
  実際に読めることをここでは確かめられません。実機で**オフのまま**開いて
  自作の字が出ることを見るまでは、文書と grep に基づく推論です
  （`docs/CHECK-0907.md` ビルド 162）
...
- フルアクセスを取る理由を聞かれます。**取っていないので聞かれません** ──
  `RequestsOpenAccess` は `false` です（2026-09-18、plist の節）
...
| I | こちら | 拡張ターゲット・Swift・Info.plist・entitlements・CI（§3,4,6,7,8） | 済（#41 でコンパイルが通り、埋め込まれて TestFlight に上がった） |
| J | あなた | 実機でフルアクセスを与え、キーボードを開いて打ってみる | 済（打てている） |
...
数字の段も自分の数字で出ています。**フルアクセスを与えて、です** ── 要求は
2026-09-18 に落としたので、**オフのままで同じことが起きるかは誰も見ていません**
（`docs/CHECK-0907.md` ビルド 162）。
