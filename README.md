...
読みは **IPA（発音記号）が本体**で、カタカナは日本語話者むけの近似。設定 →「読みの表記」で
**発音記号／カタカナ／両方**（既定は両方）を選ぶ。`/ /` の中の `.` は音節の切れ目、`ː` は長音。
一語の詳細では音節ごとに IPA とカナを並べて出し、II 音 では在庫の子音・母音の一つひとつに IPA を添える。
...
- **I ことば** … 辞書。語をタップすると詳細（IPA・カナ・音節の切れ目とその音・意味・品詞）が開き、**なおす／消す**ができる
...
- `kana()` … つづりから読みを機械的に導出（人が読みを考えなくていい）
- `ipa()` / `ipaSyl()` … つづりから発音記号を導出。`.` は音節境界、同じ母音の連続は `ː`
- `readOut()` / `readLink()` / `readMode()` … 設定の「読みの表記」に応じて IPA／カナ／両方を出し分ける単一の窓口
- `loadVoices()` / `pickVoice()` / `speak()` … 声まわり。`voiceschanged` 待ち・言語優先順位・
  手動選択・日本語声へのカナ差し替え・ネイティブTTSプラグインへの分岐が全部ここ
- `linked()` … 語末子音を次の語の頭にくっつけて再音節化＝リンキングの可視化。IPAとカナの両方を返す
