...
| `tools/pv.mjs` | the film. A minute of the app as one mp4, shot a frame at a time off the real `index.html` in an iframe — `tools/pv/` holds the stage, the language it is about, and what happens second by second. Not a gate, and nothing in `www/` knows it exists |
| `tools/storeshot.mjs` | the pictures on the App Store page. The same seeded app as the film, in a phone frame, at 1242×2688 — the size App Store Connect actually took. `--lang` moves the words and the app together, because the product page holds one set per country |
